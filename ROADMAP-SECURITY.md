...
## Changelog
- **2026-07-31** — **S10 — trade-alert endpoint auth + abuse guardrails (2490 green).** _(synced master first — S9 now merged `4f895fc` + PR #2 Journaling `9ae004d`; committed local on `security`, awaiting PM merge)_
  - **What it is:** `POST /api/vapi/trade-alert` — Derrick's separate trade-monitor service POSTs `{reason, headline, context, idempotencyKey}` with an `x-trade-alert-key` header; on accept, Edge places an outbound call to Derrick with the alert. This is the **trust surface** (S10 half of the joint C14 ticket); Core (Darren) owns the call-variant.
  - **Machine-authed, not a session route** — like the Vapi webhook, it's authenticated by a shared secret, not `getSession()` (documented in-file so the OWASP sweep doesn't flag it).
  - **Gate order (a leaked key = robocall vector, so gate hard):** (1) constant-time key compare — SHA-256 both sides + `crypto.timingSafeEqual`, so a length mismatch neither throws nor short-circuits; key never logged. → (2) body validation, length-capped (`context`≤500, headline≤200, reason≤100) to stop prompt/dedupe stuffing. → (3) resolve target user (`TRADE_ALERT_USER_ID` env, else sole user). → (4) per-user kill switch (`users.trade_alerts_enabled`, default ON = opt-out). → (5) market-hours window (09:30–16:00 America/New_York, Mon–Fri; DST-aware via Intl; holidays intentionally not modeled). → (6) `idempotencyKey` dedupe **before** the cap so an upstream retry short-circuits and never burns budget. → (7) ≤3/day hard cap (`rateLimit` bucket `tradeAlert`, per target user) — the backstop, consumed only on genuine accepts. → (8) dispatch to the Core seam + audit. Responses: `200 {queued:true}` / `200 {queued:false, reason}` (disabled / off-hours / duplicate / no-target / dispatch-failed) / `401` (bad key) / `429` (daily cap).
  - **Every branch audit-logged** — `auditLogQueries.logTradeAlert({userId, outcome, reason, idempotencyKey})`, action `trade_alert`, `ok=1` accept / `0` reject; unauthenticated rejects use sentinel user 0 (mirrors `inbound_call_attempt`). The key itself is never passed to audit.
  - **Core handoff seam:** `dispatchTradeAlertCall(user, alert)` lazily imports `initiateTradeAlertCall` from `lib/vapi.ts`; if absent (pre-C14) it returns `'pending'` (endpoint accepts + audits, no call placed) so this surface ships and is fully tested independently. **Darren:** export `initiateTradeAlertCall(user: User, alert: TradeAlertBody): Promise<unknown>` (short-call variant, `maxDurationSeconds` 300, prompt seeded with `headline`+`context`) and it plugs in with zero endpoint changes. `TradeAlertBody` is exported from `lib/tradeAlert.ts`.
  - **Files (all additive):** new `app/api/vapi/trade-alert/route.ts`, new `lib/tradeAlert.ts` (pure guardrails + seam), `lib/idempotency.ts` (`claimTradeAlert`), `lib/rateLimit.ts` (`tradeAlert` bucket), `lib/db.ts` (`users.trade_alerts_enabled` column + `getTradeAlertsEnabled`/`setTradeAlertsEnabled`/`getTradeAlertTarget` + `logTradeAlert`).
  - **Tests:** `lib/tradeAlert.test.ts` (key compare incl. wrong-length, market-hours EST+EDT+weekend boundaries, body validation) + `app/api/vapi/trade-alert/route.test.ts` (full gating matrix incl. the retry-doesn't-consume-cap invariant + dispatch pending/error paths). +27. 170 files / 2490 green, tsc + build clean.
  - **⚠️ Config (Derrick, go-live):** set `TRADE_ALERT_KEY` on **both** Railway services (Edg3 + trade-monitor); optional `TRADE_ALERT_USER_ID`. Kill-switch dashboard toggle = Core/Design UI.
  - **Scope update (same day — C14 revised per Derrick, two endpoints now):** the trade-monitor also POLLs a watch-list feed, so there are two externally-hit endpoints, not one. (a) Extracted `guardTradeAlertKey(req)` — the shared constant-time auth gate (bad_key audit + ready 401) — so the POST fire path AND Core's `GET /api/vapi/trade-alerts?status=active` feed gate identically; the GET leaks Derrick's watch-list if unauthenticated, so Core's handler must call it first. (b) Added a `tradeAlertPoll` rate-limit bucket (120/min per IP) as the GET's DoS backstop (auth is the real guard; the watcher polls legitimately). (c) POST body now accepts an optional `alertId` (validated positive integer), threaded through the dispatch seam so Core's call-variant marks the `trade_alerts` row `fired`. +5 net tests (alertId parse + gating). 170 files / 2495 green. **Pushed to `origin/security`** (Derrick's request: git is the canonical status channel — PM polls + merges).
- **2026-07-02** — **S9 — production DB schema-drift repair (2446 green). URGENT — PM cross-session dispatch.** _(synced master first; committed local on `security`, awaiting PM merge + deploy — MERGED `4f895fc` + deployed 2026-07-30)_
  - **Root cause:** three prod write paths were failing *invisibly*. The migration runner's `catch { /* skip */ }` swallowed every failure, and SQLite can **never** `ALTER` a `CHECK` constraint or a column `DEFAULT` — so any table created before a CHECK/DEFAULT change stays drifted from the fresh `CREATE` schema and rejects inserts the current code assumes are legal. Fresh test/local DBs get the current schema via `CREATE TABLE`, which is why 2435 tests stayed green while prod threw.
  - **S9a — `facts.last_confirmed_at` missing in prod.** It was added by `ALTER TABLE facts ADD COLUMN last_confirmed_at TEXT DEFAULT (datetime('now'))` — a *non-constant* default. Some SQLite builds (older than prod's) reject that outright, so the column never landed and every memory-freshness upsert threw `no such column`. Removed the broken migration entirely; the column (with its `DEFAULT`) is now guaranteed by the facts table rebuild below. (Note: this machine's SQLite 3.53.1 happens to *accept* the non-constant default — that's why it was never caught locally.)
  - **S9b — `briefings.status` CHECK lacked `'missed'`.** Prod predates the value, so the cloud-branch declined/short-call streak gate threw `CHECK constraint failed`. New `rebuildBriefingsIfStale()` — rebuilds the table with the current CHECK **only if** the stored SQL doesn't already list `'missed'`.
  - **S9c — `facts.category` CHECK lacked 5 newer categories** (`pattern`, `commitment`, `weekly_summary`, `lifetime_profile`, `user_note`) → pattern detection + commitment capture silently failing in prod since they shipped. New `rebuildFactsIfStale()` (folds S9a — rebuilds with the full schema so `last_confirmed_at` + its DEFAULT land too).
  - **Rebuild safety:** generic `rebuildTable()` helper — runs under `PRAGMA foreign_keys=OFF` toggled **outside** the transaction (a no-op inside one), uses **create-new / drop-old / rename-new** (never rename-old) so no child table's FK gets silently rewritten to the temp name, recreates indexes dropped with the old table, and runs `foreign_key_check` after. Guards mean a **fresh or already-converged DB never rebuilds** (zero data churn on the common path). Legacy rows preserved; `valid_from` (NOT NULL in new schema, nullable on legacy) and `last_confirmed_at` backfilled from `learned_at` → freshness semantics, not a fake "now".
  - **S9d — migration runner observability.** `catch` now `console.error('[db] MIGRATION FAILED: …')` on any non-benign error (anything that isn't "duplicate column name" / "already exists"). Still never aborts boot — one bad migration must not block the rest.
  - **Tests:** new `lib/db-schema-drift.test.ts` (+11) — seeds the legacy-prod schema, proves `status='missed'` / `category='pattern'` inserts **fail before** and **succeed after**, rows + facts→briefings FK + all indexes survive both rebuilds, `last_confirmed_at` backfills from `learned_at` and carries its DEFAULT, idempotent on re-run, and a fresh DB is **not** rebuilt. 166 files / 2446 green, tsc + build clean.
  - **Files:** `lib/db.ts` (additive — new helpers + rewritten `applyMigrations`; removed the one broken migration string), `lib/db-schema-drift.test.ts`.
  - **Follow-up (same day):** synced PR #2 (Journaling Mode, `9ae004d`) down — it added `briefings.is_journal` + `audio_url` (both constant-default, safe). Added both to the S9b rebuild CREATE + copy list so a rebuilt legacy table doesn't silently drop them, plus a regression test. Rebuild column lists are now flagged as hand-maintained/must-stay-in-sync. 168 files / 2463 green.
...
- **2026-06-24** — **R20 follow-up — NANP area-code timezone fallback (2196 green).** _(Kevin greenlit the code-level defense)_
  ...
  - **Tests:** new `lib/phoneTimezone.test.ts` (15 assertions: Derrick's 416 → Toronto, representative US/Canada zones, bare/prefixed forms, null cases, fallback ordering, and the exact "7:37 PM Eastern → hour 19 not 16" regression). Cross-zone dup-guard run clean (fixed 434 Virginia mis-assignment). 141 files / 2196 green.
  - ⚠️ **Derrick immediate (prod DB — I can't reach it from the worktree):** `UPDATE users SET current_timezone='America/Toronto' WHERE email='derrickfung87@gmail.com';` — though once this NANP fix deploys, his stored phone number auto-resolves to Toronto, so the manual update is only needed to unblock before deploy.
- **2026-06-24** — **R21 T1+T2 COMPLETE — explicit tool-call failure strings + R33 work_schedule schema (2189 green).** _(synced master first)_
  - **T1 (HIGH) — `app/api/vapi/tool-call/route.ts`:** the four mutating handlers (`createEvent`/`deleteEvent`/`moveEvent`/`editEvent`) now return explicit `ERROR: Event was NOT …` strings on EVERY Google failure path, so the model can never read a failed call as success (the "Done. Locked in 90 minutes" hallucination for an un-created event). Added 4 `ERR_*` constants; wrapped the previously-unguarded `createEvent`/`editEvent` inserts/patch in `.catch`; converted every vague soft-failure return (no `data.id`, patch→null, all-deletes-failed, recurring-retime-failed) to the `ERROR:` string. `FAILURE_RE` now also matches a leading `ERROR:`. Success paths untouched. Exported `executeTool` + `ToolContext` for testing. New `mutation-errors.test.ts` (5 cases: each handler's throw → its ERROR string, via a mock cal client + confirm-token handshake for delete; success path stays clean).
  - **T2 (LOW) — `lib/db.ts`:** added `work_schedule TEXT` column to `users` (JSON, default `{"start":9,"end":18,"days":[1,2,3,4,5]}`) + `getWorkSchedule(user)` helper (parses JSON, falls back to default on null/corrupt). Unblocks Core's R33 settings API/UI. `work_schedule?` added to the `User` interface. New `work-schedule.test.ts` (4 cases). 140 files / 2189 green.
  - **T3 (Trust + Daily-Call QA checklists):** ran every code-verifiable item; logged a dated run in `content/qa-log.md`. All automated guards green; R21 T1 directly hardened the "tool failure → honest explanation" checklist item. Live-path items (real Vapi calls, Railway/Vapi dashboards, prod restore drill) remain ⚠️ EXTERNAL — a worktree can't run them; handed to Derrick/Kevin (same list as the qa-log Morning action list).
- **2026-06-24** — **R20 T1 INVESTIGATED — "Good afternoon" at 7:37 PM is a TIMEZONE bug, not a greeting bug.** _(no code change to greeting logic — it's correct)_
  - **Finding (empirically verified):** the greeting plumbing is correct. `scheduleOpenCall` computes `greet = greetingEn(hour)` and it DOES reach the Vapi `firstMessage` (the non-gratitude opener `${greet}, ${firstName}. It's Edge…` → `briefingContent` → `firstMessage`). The spec's Path A (var unused) and Path B (hardcoded string) are both **disproven** — there is no hardcoded "Good afternoon" in `lib/vapi.ts` or `scheduler.ts`.
  - **Real root cause:** `effectiveTimezone(user)` (`lib/db.ts`) falls back to **`America/Los_Angeles`** when both `current_timezone` and `timezone` are null/invalid. Derrick is Eastern with no valid stored timezone → 7:37 PM ET is computed as 4:37 PM PT → `hour=16` → "Good afternoon". Proven: `node` check shows the same `Date` yields hour 19 ("Good evening") under `America/New_York` and hour 16 ("Good afternoon") under `America/Los_Angeles`.
  - **Why the tz is empty:** the app only sets `current_timezone` via a *manual* Settings action (`POST /api/profile/timezone`); nothing auto-captures the browser's `Intl…resolvedOptions().timeZone`. A user who never touches that setting (and has no `timezone`) silently gets the LA default.
  - **Recommended fixes (routed):** (1) IMMEDIATE — set Derrick's timezone (Settings → timezone, or DB) to unblock tonight. (2) DURABLE (Core-owned, `app/dashboard/**` + `app/api/profile/**`): auto-detect the browser timezone on dashboard load and POST it when `current_timezone` is empty, so no user is ever on the LA default. (3) OPTIONAL Security safety net: infer timezone from the phone number's NANP area code in `effectiveTimezone` before defaulting to LA — offered to Kevin, not built unprompted (new behavior in shared `lib/db.ts`).
- **2026-06-24** — **R19 T3 FOLLOW-UP — gratitude open call said "Good morning" at 5 PM (2177 green).** _(hotfix; reported live by Derrick at 5:27 PM)_
  - **Root cause:** T3 fixed the *plain* open-call greeting (uses `greetingEn`), but the **gratitude** open-call path was separate and hardcoded `"Good morning … How are you doing this morning?"` in **two** places: the `opener`/Vapi `firstMessage` in `scheduleOpenCall` (`lib/scheduler.ts`) AND the OPENER line in `buildGratitudeSystemPrompt` (`lib/vapi.ts`). Derrick has gratitude mode on, so his manual 5:27 PM "Call me now" hit that path. (No hardcoded `firstMessage` in the assistant config — confirmed `firstMessage` is the dynamic opener.)
  - **Fix:** both now use the centralized time-of-day greeting. `scheduleOpenCall` computes `dayPeriod(hour)` + passes the localized greeting (`greet`/`greetYue`) and period word into the opener and into `buildGratitudeSystemPrompt`. The gratitude prompt builder gained two **optional** params (`greeting='Good morning'`, `timeOfDay='morning'` — defaults preserve old behavior for any other caller); its OPENER, Whoop-acknowledgment line, quote instruction, and "morning gratitude check-in" framing are now time-neutral/parameterized. Cantonese branch swapped hardcoded 早晨 for the localized greeting.
  - **Tests:** +4 in `lib/vapi-gratitude.test.ts` (evening greeting/period threads through OPENER + Whoop line; default stays morning; Cantonese uses supplied greeting). 138 files / 2177 green.
  - ⚠️ **Shared-file note:** touched `lib/vapi.ts` `buildGratitudeSystemPrompt` (gratitude *behavior*, normally Core's half of the shared file). Change is additive (optional params) + scoped to the time-correctness bug Kevin dispatched to Security — flag for Core awareness.
...
- **2026-06-24** — **R19 T6 COMPLETE — score-change notification uses `getPrior` not yesterday-only (2148 green).** _(synced master first)_
  ...
  - **Tests:** new `lib/notifications-score.test.ts` (real in-memory DB). 5 cases: prior-row-across-gap fires; no prior row → silent; delta<3 → silent; nearest-prior preferred; today's own row not treated as baseline. 134 files / 2148 green.
  - ⚠️ **Shared-file note:** `lib/notifications.ts` is also listed under Core R25 T7 (score-rise notif/animation). My change is surgical (lookup swap only) — flag for merge-order awareness; if Core also edits `maybeCreateScoreChangeNotif`, PM resolves.
  - **T5 (nightly score cron) still BLOCKED:** Core's `lib/scores.ts` / `computeAndSaveScore` not yet on master; a dynamic import of a missing module fails `tsc`, so deferred until it lands (will wire per spec + add the per-user test then).
- **2026-06-24** — **R19 T4 COMPLETE — gratitude/open call no longer blocks the morning briefing (2143 green).** _(synced master first)_
  - **Root cause:** the two once-a-day guards in `lib/scheduler.ts` (`alreadyCalled` in `checkAndInitiateCalls`, `existing` in `scheduleBriefingCall`) matched any `status='completed'` briefing for today without filtering `is_open_call`. Once the overnight ElevenLabs quota reset, the gratitude call (`is_open_call=1`) fired ~5–7am, completed, and its completed row made the scheduler think the morning briefing already happened → silently skipped it (Derrick's missed briefing 2026-06-24).
  - **Fix:** both guards now exclude open calls via `(is_open_call IS NULL OR is_open_call = 0)`. Legacy NULL rows are treated as morning briefings (conservative — still block). Extracted the (previously duplicated) query into one exported `findTodaysBlockingBriefing(db, userId, dayPrefix, callingCutoff, pendingCutoff)` so the guard lives in a single place and is unit-testable.
  - **Tests:** new `lib/scheduler-blocking-briefing.test.ts` (real in-memory DB, exercises the actual SQL). 6 cases: completed open call → not blocked; completed morning briefing → blocks; NULL → blocks; two open calls → free; open + morning → blocks on morning; vapi_daily_limit on open vs morning. Existing `scheduler.hardening.test.ts` still green (helper routes through its SQL-substring mock). 133 files / 2143 green.
