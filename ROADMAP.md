...
| 🔒 Security | `security` | _(✅ **Esther dispatch DONE (1918 green).** T2: `people_models` added to `GET /api/account/export` (decrypted, +test). T1: privacy email-body disclosure already shipped overnight (verified present). Idle — awaiting next dispatch.)_ | `app/api/account/export/route.ts`, `app/api/account/account.test.ts` | 2026-06-19 |
| 🔧 PM/CoS | `master` | _(✅ **Round 8 FULLY SHIPPED — 1909 green, pushed to origin.** All lanes merged (Core M4-4 + Briefing V2, Security people_models, Design FocusScoreboard + People section). Master at `b0684e7`. **Next lane dispatches ready:** Security T3-1-A ('pattern' DB constraint), Core T3-1-B (factPatterns category fix, gated on Security) + DC3-1 (voice anchor phrases). **⚠️ send_message blocked in unsupervised mode — lanes need /loop kick-off when Derrick is present.** **🚨 CRITICAL — Derrick action:** Railway volume at `/data` is likely ephemeral — DB wiped on every deploy. Check Railway → Volumes NOW. Runbook: `content/durability-runbook.md`.)_ | `ROADMAP*.md` | 2026-06-19 |
