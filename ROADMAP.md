...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Round 8 SHIPPED (both tickets, 1905 green).** T1 = M4-4 social mental models: new `people_models` table + `peopleModelQueries`, sleep-time sync (`derivePersonModelFields`), briefing injection (`buildPeopleModelBlock`, ≤3 on-calendar). T2 = Briefing V2 proactive wins (free-slot offer, tomorrow look-ahead, personal-event warmth, recovery names specific deferrable event). 15 new tests. ⚠️ cross-lane: added `people_models` table to shared `lib/db.ts` per dispatch — Vijay FYI. Earlier: Round 7 email-body reading. Awaiting next dispatch.)_ | `lib/db.ts` (cross-lane), `lib/facts.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/alignment.ts` | 2026-06-19 |
