...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Derrick dashboard-review batch — 9/10 shipped (1818 green)**: 1 Edge-Score "why", 2 Today's-Focus context, 3 gate "what I'd change" panel, 4 focus lock-in, 5 naming, 6 removed avg metric, 7 browsable priority history (+new `/api/priorities/history`), 9 email spam filter (`lib/emailActivityFilter.ts`), 10 expandable threads. **Ticket 8 DEFERRED** (consolidate dup priority sections — needs Core+Design joint structural call; see ROADMAP-CORE changelog). Dashboard claim released. ⚠️ Multi-account Google UI still queued — blocked on Vijay's `GET /api/auth/accounts`. ⚠️ §2: a twin Core session also worked this worktree overnight — PM re-establish single ownership.)_ | `app/dashboard/page.tsx`, `app/api/priorities/history/route.ts`, `lib/emailActivityFilter.ts`, `lib/scoreChange.ts` | 2026-06-18 |
