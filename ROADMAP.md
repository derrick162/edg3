...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R12 T7 — email feature code removal IN PROGRESS.** Removing `draftEmail`/`checkReplies` handlers + imports from tool-call route; deleting `lib/outreach{,.test}.ts` + `lib/replies{,.test}.ts`. ⚠️ **Scope note:** `checkOutreachReplies` was also consumed by `lib/briefing.ts` (REPLIES prompt block) + `app/api/notifications/route.ts` — dispatch file list missed these; removing the orphaned reply-tracking there too so the build stays green + feature is fully gone. Keeping `deleteDraft` (undo.ts compat).)_ | `app/api/vapi/tool-call/route.ts`, `lib/briefing.ts`, `app/api/notifications/route.ts`, `lib/outreach.ts`, `lib/replies.ts` | 2026-06-20 |
