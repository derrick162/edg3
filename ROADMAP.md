...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12 T1–T5 SHIPPED (1991 green); 🔨 building T6 voice-speed control.** T1 delete-token fresh-flow+log, T2 token-language ban, T3 REPLACE PATTERN, T4 open-call Momentum (`is_open_call`), T5 dashboard auto-refresh. **T6 in progress:** `users.voice_speed` column (shared `db.ts`, additive), POST `/api/profile/voice-speed`, Profile-tab 3-button UI, `SPEED_MAP` in `lib/vapi.ts` replacing hardcoded `speed:0.9`. ⚠️ shared `lib/db.ts` + `lib/vapi.ts` (voice config) — additive, Vijay FYI.)_ | `lib/vapi.ts`, `lib/db.ts`, `app/api/profile/**`, `app/dashboard/page.tsx` | 2026-06-20 |
