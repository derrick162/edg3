...
| 🔒 Security | `security` | _(✅ **Night-3 dispatch COMPLETE** (827 green): (1) `facts.statement` encrypted at rest (PM GO) — factQueries enc/dec, openLoops stub swapped to encrypted openLoopQueries, dailyFocus encrypted; (2) open-call double-tap guard + rate limit; (3) nightly backup wired in cron for no-call days; (4) CASA code done, non-code (demo video + inbox consent) pending Derrick. Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
