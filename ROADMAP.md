...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **M4-5 SHIPPED (2239 green) — ready for PM merge** (R28–R34 + R29 Part D also awaiting merge). Hierarchical memory synthesis: `runWeeklySynthesis` + `runLifetimeSynthesis` (`lib/facts.ts`) + lifetime profile injected first in call memory + `facts.category` CHECK expanded (also fixes latent R34 commitment bug). ⚠️ **CRON WIRING IS SECURITY'S** — Vijay adds weekly (Sun 4am) + monthly (first Sun) jobs in `lib/scheduler.ts` calling these helpers (R25 T4→R19 T5 pattern). ⚠️ Additive to Shared `lib/db.ts`. Idle — dispatch queue exhausted.)_ | `lib/facts.ts`, `lib/callMemory.ts`, `lib/db.ts`, `lib/factConfidence.ts`, `lib/memorySalience.ts` | 2026-06-24 |
