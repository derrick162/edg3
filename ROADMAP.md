...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **M5 EPISODE STORE COMPLETE (1418 green, preflight clean)**: `lib/db.ts` (episodes table + queries) + `lib/episodeStore.ts` (tagTopics + persistCallEpisode + buildEpisodeMemoryBlock) + webhook fire-and-forget wiring + briefing EPISODIC MEMORY injection. Previous: M4 Accountability Memory, M3 Pattern Memory, M2 Relationship Memory. Awaiting PM merge.)_ | `lib/db.ts`, `lib/episodeStore.ts`, `lib/episodeStore.test.ts`, `lib/briefing.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-18 |
