...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R25 T1+T2+T3 SHIPPED (2130 green) — ready for PM merge.** T1: calls know people off today's calendar. T2: gratitude high-recovery `WHOOP ACKNOWLEDGMENT` + natural per-item conversation. T3: `lib/alignment.ts` excludes all-day events from the LLM classifier (Conrad Las Vegas 8h false positive). 10 tests across the round. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` — Vijay sync down.** Idle, awaiting next dispatch.)_ | `lib/scheduler.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/alignment.ts` | 2026-06-24 |
