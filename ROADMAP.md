...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Tickets A + C COMPLETE (997 green)**: A — Focus Score recalibrated (ratio×coverage, routine excluded from committed, topUnaligned filters meals). C — Notification center populated: score_change/fact_learned/activity producers in `lib/notifications.ts`, `existsToday` de-dupe in db. Awaiting PM dispatch.)_ | — | 2026-06-16 |
| 🔒 Security | `security` | _(✅ **POST-FLAGSHIP BACKLOG COMPLETE (1156 green)**: (1) Undo audit gap closed — `undo_applied` now logged to audit_log. (2) Encryption-at-rest fully verified — all 14 encrypted fields documented in security-audit.md; accepted-unencrypted fields catalogued. (3) Session/auth hardening review PASS — JWT, bcrypt 12, session versioning, cookie flags, CSRF all clean. (4) npm audit — 2 moderate transitive vulns in Next.js postcss, accepted. (5) data-protection.md updated with missing encrypted fields. (6) Hardened new Core routes `/api/priorities/derive/accept` — rate limit + text length cap. Awaiting PM dispatch.)_ | `app/api/undo/route.ts`, `app/api/priorities/derive/accept/route.ts`, `lib/rateLimit.ts`, `content/security-audit.md`, `content/data-protection.md`, `ROADMAP-SECURITY.md` | 2026-06-17 |
| 🔧 PM | `master` | _(✅ **NIGHT-QUEUE flagships integrating → `45a4c35` (1156 green, deployed).** Integrated: Core FLAGSHIP — Proactive Priority Derivation (`lib/priorityDerivation.ts` + `/api/priorities/derive` + accept) + T3 grounding on live voice path; Security FLAGSHIP — full pre-beta audit of all 78 `app/api/**` routes (10 new rate-limit keys, input-validation fixes, `content/security-audit.md`); Cam FLAGSHIP — full mobile pass + a11y/touch-targets + loading skeletons; Esther — content avalanche (beta-launch-playbook, ICP interview guides, outreach kit, positioning, competitive landscape, pricing analysis, first-10-users, CASA video script). Resolved rateLimit key collision (kept both lanes' keys); cleared stale .next cache. Tonight's fixes (one Edge Score, graceful receipts) verified intact through all merges. Continuing overnight loop to ~9am.)_ | `ROADMAP*.md`, `specs/**` | 2026-06-17 |
| 🎨 Design | `design` | _(idle — ✅ **NIGHT-QUEUE COMPLETE**: (1) Mobile pass flagship — notif panel responsive, Next Call strip on mobile, nav aria-labels, landing/onboarding touch/text responsive. (2) Memory auto-open first cat. (3) Activity + Profile loading skeletons. (4) Touch targets py-2.5, hover-control opacity-30. (5) Accessibility — aria-expanded on accordions, aria-label on notification bell + memory cats. (6) Token consolidation — badge colors use --edg-* vars. 8 commits, preflight clean.)_ | — | 2026-06-18 |
