...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R22 IN PROGRESS (2026-06-22) — Cantonese (`yue`) language support.** `language` col + `setLanguage`/`getLanguage` (shared `lib/db.ts`, additive, **Vijay FYI**); `GET/PATCH /api/settings/language` (validates en/yue); `initiateCall` + `buildGratitudeSystemPrompt` gain `language` param → Whisper STT + Azure `zh-HK-WanLungNeural` voice + Cantonese system/gratitude prompts + Cantonese `endCallPhrases`; `scheduleBriefingCall`/`scheduleOpenCall` read `user.language`. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` — Vijay sync down after merge.** Preflight+commit. (R20 already merged.))_ | `lib/db.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `app/api/settings/language/**` | 2026-06-22 |
