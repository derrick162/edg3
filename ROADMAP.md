...
| 🔒 Security | `security` | _(🔨 **R21 T1+T2 DONE (2189 green) — ready for merge; T3 (QA) next.** T1: tool-call handlers return explicit `ERROR: Event was NOT …` strings on every Google failure (no more "Done" on a failed call) + 5 tests. T2: `work_schedule` column + `getWorkSchedule` helper (unblocks Core R33) + 4 tests. **R20 T1 (greeting):** still a TIMEZONE bug — `effectiveTimezone` defaults to LA when user has no valid tz; awaiting Kevin's call on the fix (offered area-code fallback). Hygiene `a5a34fa` + R20 finding `c27b79b` pending merge.)_ | `app/api/vapi/tool-call/route.ts`, `lib/db.ts` | 2026-06-24 |
