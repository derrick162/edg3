...
| 🔒 Security | `security` | _(✅ **EPISODE STORE SCHEMA + ENCRYPTION (1456 green)**: `episodes` table with AES-256-GCM `content_raw`, compound `(user_id, occurred_at)` idx, `episodeQueries` (insert/recent/search/prune), 18 new tests. Consent-gated (callers must check `isImproveConsented`), user-scoped, retention-bounded (365d). Ready for Core ingestion wiring. Awaiting PM dispatch for Round 4 Ticket 1 (audit log coverage) + Ticket 2 (rate-limit gap check).)_ | `lib/db.ts` | 2026-06-18 |
