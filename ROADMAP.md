...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Round 8 SHIPPED (both tickets, 1905 green).** T1 = M4-4 social mental models: new `people_models` table + `peopleModelQueries`, sleep-time sync (`derivePersonModelFields`), briefing injection (`buildPeopleModelBlock`, ≤3 on-calendar). T2 = Briefing V2 proactive wins (free-slot offer, tomorrow look-ahead, personal-event warmth, recovery names specific deferrable event). 15 new tests. ⚠️ **M4-4 DUP-BUILD RECONCILED (1909 green):** Vijay also built `people_models` (`e118526`) — synced master, kept **his** schema/`peopleModelQueries` (Security owns db.ts), adapted my Core wiring to his **camelCase** `PeopleModelFields` (`derivePersonModelFields`, `buildPeopleModelBlock`), dropped my dup query tests. PM: dispatch said "coordinate with Vijay" — we collided; route schema-side M4-4 solely to Security next time. Earlier: Round 7 email-body. Awaiting next dispatch.)_ | `lib/db.ts` (cross-lane), `lib/facts.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/alignment.ts` | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **Synced master (1880 green) — migration fix reconciled.** All Security pillars done. **Idle — awaiting dispatch.** ⚠️ Review Darren's cross-lane edit to `lib/gmail.ts`.)_ | `lib/db.ts`, `lib/scheduler.ts`, `app/api/admin/**`, `app/api/auth/**` | 2026-06-18 |
| 🔧 PM/CoS | `master` | _(✅ **Round 8 dispatched — 1890 green.** All three lane merges done (security privacy fix + core email body reading). New dispatches written: Core R8 (M4-4 social mental models + Briefing V2 proactive wins), Security R8 (people_models schema + setEnergyLevel route), Design R8 (Focus Scoreboard shell + tab order). **🚨 CRITICAL — Derrick action:** Railway volume at `/data` is likely ephemeral — DB wiped on every deploy. This is why 9am call missed + memory keeps resetting. Check Railway → Volumes NOW. Runbook: `content/durability-runbook.md`.)_ | `ROADMAP*.md` | 2026-06-19 |
| 🎨 Design | `design` | _(✅ **Gmail reconnect slot + briefing word count + sidebar copy fix shipped.** All pillars exhausted. Idle — awaiting dispatch.)_ | `app/dashboard/page.tsx`, `components/ui/Button.tsx`, `components/ui/NotificationCenter.tsx` | 2026-06-19 |
