...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R17 + pillar sweep SHIPPED (2056 green) — ready for PM merge.** R17: sharper opener (`isRoutineEvent`) + post-call feedback (`call_feedback` table, `POST /api/briefing/feedback`, ★★★★★ row). Pillar sweep: Daily Call (Tier 4 gated @50 users) + Memory (M4-3 gated @30 calls; M4-4 already LIVE) had no open Core items; **Trust T3-1 completed** — `factDisplayStatement` renders pattern facts' `.summary` not raw JSON in "What Edge knows". All dispatch + buildable pillar items exhausted. Idle, awaiting next dispatch.)_ | `lib/factDisplay.ts`, `app/dashboard/page.tsx`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `lib/rateLimit.ts`, `app/api/briefing/feedback/**` | 2026-06-20 |
