...
| 🔒 Security | `security` | _(✅ **T0-2+T1-3+T4-1+DC1-1 COMPLETE (1660 green)**: encryption-key-rotation.md; health_log table + 6am health digest cron; calendar auth failure tracking (recordAuthFailure/clearAuthFailures/needsReconnect + checkCalendarTokenHealth); call_attempts log. Next: T4-2 Vapi pre-call health check + DC1-2 retry on transient failure.)_ | `lib/db.ts`, `lib/scheduler.ts`, `lib/google-auth.ts`, `content/encryption-key-rotation.md` | 2026-06-18 |
