...
| 🔒 Security | `security` | _(✅ **ROUNDS 4–6 COMPLETE (1201 green)**: (4) Webhook + briefing task text capped 500 chars; verifyPromises memory note capped 2000 chars. (5) briefing.ts analyzeUserResponse task cap + onboarding/priorities 200-char text cap. (6) Email header injection fix in `lib/gmail.ts` `buildRawMessage` — CRLF stripped from to/cc/bcc/subject before MIME header interpolation; 1 new test. Awaiting PM dispatch.)_ | `lib/gmail.ts`, `lib/briefing.ts`, `lib/verifyPromises.ts`, `app/api/vapi/webhook/route.ts`, `app/api/onboarding/priorities/route.ts` | 2026-06-17 |
