...
| 🔒 Security | `security` | _(✅ **PILLAR-TRUST T1-1 + T1-3 COMPLETE (1594 green)**: `failed_webhooks` dead-letter table + `failedWebhookQueries`; `background_job_failures` table + `backgroundJobFailureQueries` (consecutive alert at ≥3); retry catch → dead-letter; cron jobs wrapped; 3am daily health check. 20 new tests. Awaiting PM merge.)_ | `lib/db.ts`, `lib/scheduler.ts`, `lib/failure-logging.test.ts` | 2026-06-18 |
