...
| 🔒 Security | `security` | _(✅ **R21 T1+T2+T3 DONE (2189 green) — ready for merge; idle.** T1: tool-call handlers return explicit `ERROR: Event was NOT …` on every Google failure (no more "Done" on a failed call) + 5 tests. T2: `work_schedule` column + `getWorkSchedule` (unblocks Core R33) + 4 tests. T3: ran code-verifiable QA checklist items → `content/qa-log.md` dated run; live-path items handed to Derrick/Kevin. **R20 T1 (greeting):** TIMEZONE bug — `effectiveTimezone` defaults to LA when user has no valid tz; awaiting Kevin's call (offered area-code fallback). Pending merge: `cbe9b2a` (R21), `c27b79b` (R20 finding), `a5a34fa` (hygiene).)_ | `app/api/vapi/tool-call/route.ts`, `lib/db.ts`, `content/qa-log.md` | 2026-06-24 |
