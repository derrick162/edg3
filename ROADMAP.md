...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Night-3 queue COMPLETE (939 green)**: Items 1–2 (meeting prep cross-link + deeper email intel) + Items 3–5 (calendar pattern detection [routines/focus windows/trend], time-allocation trends [priority % bucketing], Open Loops refinement [snooze + recurring detection + tier-2 call-surfacing]). All wired into briefing + focus rec + vapi. Plus Whoop intel, memory weighting, actionable focus backend, /api/support. Awaiting PM dispatch.)_ | — | 2026-06-16 |
| 🔒 Security | `security` | _(✅ **Security gap assessment + quick-win hardening COMPLETE** (827 green): H2 verify-promises endpoint gated; M3 cookie flags; M4 postMessage origin; M5 briefingCall rate limit; M6 email normalization; L4 timingSafeEqual; L5 backup filename; L6 parseInt radix. Security headers + content/security.md shipped. ⚠️ NOT fixed (need PM go): H3 OAuth CSRF state, M7 JWT revocation, CSP nonce. Awaiting PM.)_ | — | 2026-06-16 |
