...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **M3 PATTERN MEMORY COMPLETE (1389 green, preflight clean)**: `lib/patternMemory.ts` (4 detectors: productive-day, light-day, meeting-load→recovery, focus-window) + `pattern_cache` table + `GET /api/patterns` + briefing PATTERN INSIGHT block + Memory tab "Patterns" section. Previous: M2 Relationship Memory. Awaiting PM merge.)_ | `lib/db.ts`, `lib/patternMemory.ts`, `lib/briefing.ts`, `app/api/patterns/route.ts`, `app/dashboard/page.tsx` | 2026-06-18 |
