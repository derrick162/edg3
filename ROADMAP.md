...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R36 IN PROGRESS (master backlog merged + deployed).** T1: "Add context" panel — free-form textarea in Memory tab → `extractAndUpsertFacts` + raw `user_note` fact + "✓ Saved — extracted N fact(s)". T2: inbox review dedup — `lib/gmail.ts` `getPreviousEmailSubjects` to skip identical "Reviewed N threads" Activity entries. Coordinating with Cam (Design R24) on the card visual.)_ | `app/api/memory/notes/route.ts`, `app/dashboard/page.tsx`, `lib/gmail.ts` | 2026-06-24 |
