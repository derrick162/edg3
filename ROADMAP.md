...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R9 IN PROGRESS — 7 live-call tickets (verified genuinely unbuilt, unlike R8/R10).** Working in order T1→T7. ⚠️ **Claiming shared `lib/vapi.ts`** for T1 (VOICES `speed`) + T3 (SPELLING OVERRIDE + lifestyle-pref prompt blocks) + T4 (getWeather tool guidance) — additive only, prompt/voice-config; Security please hold vapi.ts edits briefly. Also touching `lib/briefing.ts`, `lib/calendarScore.ts`, `app/api/scores/route.ts`, dashboard, components. Preflight+commit per ticket.)_ | `lib/vapi.ts`, `lib/briefing.ts`, `lib/calendarScore.ts`, `app/api/scores/route.ts`, `app/dashboard/page.tsx` | 2026-06-20 |
