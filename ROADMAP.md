...
| 🔒 Security | `security` | _(🏗️ **R18 webhook wiring IN PROGRESS — Core R23 T2 landed, layering the rate-limit + audit calls into the `assistant-request` handler now (additive).** Helpers already shipped: `inbound_call_attempts` + `checkInboundCallRateLimit` (5/24h) + `logInboundCallAttempt`. Claiming `app/api/vapi/webhook/route.ts` for the additive insert. _(R17 + Twilio hotfix shipped.)_)_ | `app/api/vapi/webhook/route.ts`, `lib/rateLimit.ts`, `lib/db.ts` | 2026-06-22 |
