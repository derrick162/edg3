...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12–R16 ALL SHIPPED & MERGED to master; PM merge confirmed + tool UUIDs wired; re-verified GREEN (2044 tests, tsc+build clean).** Backend tools (R13–R15) + accountability/Focus Score/push (R16) all live and paired with Design's FocusScoreCard/EventBriefCard/WeeklyReviewCard. Synced to latest master — no open Core dispatch (top of ROADMAP-CORE is R16, done). ⚠️ FYI all lanes: `npm install` after pulling (web-push). **Core idle, ready for next dispatch.**)_ | `lib/focusScore.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/scores/route.ts`, `app/dashboard/page.tsx`, `public/sw.js` | 2026-06-20 |
