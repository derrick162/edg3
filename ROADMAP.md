...
| 🔒 Security | `security` | _(📋 **R18 dispatched 2026-06-22.** T1: inbound call rate limiting (5/24h per phone) + audit log entries for all inbound attempts + polite decline response. Coordinate with Core R23 T2 — Security's diff is additive to the webhook route Core builds; land after Core's PR. _(Previous: R17 SHIPPED — proactive sweep + export consolidation. **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_)_ | `lib/rateLimit.ts`, `lib/auditLog.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-22 |
...
| 🎨 Design | `design` | _(📋 **R21 dispatched 2026-06-22.** T1: "Inbound" amber badge in call history (wins over "Open call" for inbound calls); amber tokens in `globals.css`; `is_inbound` type in Briefing interface. _(Previous: R20 + R19 SHIPPED — language selector + daily quote toggle.)_)_ | `app/dashboard/page.tsx`, `app/globals.css` | 2026-06-22 |
