...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R25 T1–T4 SHIPPED (2133 green) — ready for PM merge.** T1: calls know people off today's calendar. T2: gratitude high-recovery `WHOOP ACKNOWLEDGMENT` + natural conversation. T3: alignment excludes all-day events. T4: new `lib/scores.ts` `computeAndSaveScore(userId)` for Security's nightly score cron (`'0 23 * * *'`). 13 tests across the round. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` — Vijay sync down.** Idle, awaiting next dispatch.)_ | `lib/scheduler.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/alignment.ts`, `lib/scores.ts` | 2026-06-24 |
