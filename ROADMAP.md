...
| 🔒 Security | `security` | _(✅ **ROUND 6 COMPLETE — PREDICTIVE CONTEXT LOADING + CONFIDENCE DECAY (1553 green)**: `briefing_context_packs` table + `briefingContextPackQueries` (upsert/get/prune encrypted); 11pm nightly cron pre-warms briefing context (activates when Core exports `buildBriefingContextPack`); `confidence_score`/`last_confirmed_at` on facts; weekly decay cron (volatile −0.1/week, stable −0.02/week); `factQueries.confirmFact` + `decayByCategories`. 24 new tests. Awaiting PM merge.)_ | `lib/db.ts`, `lib/scheduler.ts`, `lib/scheduler.round6.test.ts` | 2026-06-18 |
...
| 🎨 Design | `design` | _(idle — ✅ **Full branding sweep + pillar pass**: T2 Activity tab inbox receipt copy (Edg3); UX-1 landing page call duration copy fix (3–5 min); comprehensive Edge→Edg3 sweep across `app/dashboard/page.tsx` + all 13 `components/ui/` files — zero remaining bare "Edge" in user-visible strings. T3-1 Memory tab fact categories verified complete (all 6 rendered). Pillar-Daily-Call DC3/DC4 gated (Core/Security or Phase 2). Pillar-Memory M4-3b gated on Core `fact_history` table. Awaiting next PM dispatch.)_ | `app/dashboard/page.tsx`, `components/ui/**`, `app/page.tsx` | 2026-06-18 |
