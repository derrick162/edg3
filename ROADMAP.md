...
| 🔒 Security | `security` | _(⏳ **R12 T2 MOSTLY SHIPPED (1993 green); `createDraft` DEFERRED on Core R12 T7.** Deleted the 4 `app/api/auth/google/gmail/**` routes + test; removed the dedicated-Gmail OAuth flow (`getGmailAuthUrl`/`exchangeGmailCode`/`saveGmailTokens`/`disconnectGmailAccount`/`emailFromIdToken`/`GMAIL_ACCOUNT_SCOPES`) + contact ingest (`extractGmailAccountContacts`/`gmail_contacts_fetch`); privacy page → read-only only. **⚠️ BLOCKED: `createDraft` + `gmail.compose` scope removal — `tool-call/route.ts:909` (Core `draftEmail` handler) still imports `createDraft`; lands once Core R12 T7 removes the handler. `deleteDraft` kept (undo).** 🔔 **@Kevin: sequence Core R12 T7 before my createDraft follow-up.**)_ | `lib/gmail.ts`, `lib/google-auth.ts`, `app/privacy/page.tsx`, `content/security-audit.md` | 2026-06-20 |
