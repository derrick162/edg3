...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Derrick dashboard-review batch — 9/10 shipped (1818 green)**: 1 Edge-Score "why", 2 Today's-Focus context, 3 gate "what I'd change" panel, 4 focus lock-in, 5 naming, 6 removed avg metric, 7 browsable priority history, 9 email spam filter, 10 expandable threads. Ticket 8 DEFERRED. Multi-account Google UI queued next.)_ | `app/dashboard/page.tsx`, `lib/scoreChange.ts`, `lib/emailActivityFilter.ts` | 2026-06-18 |
| 🔒 Security | `security` | _(🚨 **PROD FIX READY — MERGE + DEPLOY ASAP (1857 green): migration runner** (+ "never-again" full-initSchema regression guard + admin/facts SQL hardening). `no such column: valid_until`/`retry_after` on deploy was caused by a CREATE INDEX on `facts.valid_until` (migration-added column) sitting in the PRE-migration block → threw on existing DBs → aborted schema init before migrations ran. Fixed: deferred the index after migrations + `applyMigrations()` extracted/tested + deduped array. `lib/db-migrations.test.ts` reproduces it. Kevin: manual ALTERs safe; self-heals after deploy. ✅ Also done: Round 6 T1 (context pack 11pm user-local). Fixed the one R6T1 gap (was 11pm UTC → tz-skewed freshness); rest already shipped. Earlier: multi-account Google linking (separate `gmail_tokens` table; `GET /api/auth/accounts` + connect/disconnect routes ready for Darren), T1-1 webhook retry+DLQ. **All Security-owned pillar items (TRUST Tier 0+1, DAILY-CALL DC1, R6 T1) exhausted/done.** ⚠️ EXTERNAL: register GMAIL_REDIRECT_URI in Google console + set env. ⚠️ AM: export-boundary call + `content/qa-log.md` action list. Commits local-only.)_ | `lib/scheduler.ts`, `lib/google-auth.ts`, `lib/gmail.ts`, `app/api/auth/**` | 2026-06-18 |
