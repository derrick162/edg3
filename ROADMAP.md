...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R17 IN PROGRESS — opener sharpening (T1) + post-call feedback signal (T2).** ⚠️ **Claiming shared `lib/db.ts`** for T2 (additive: `call_feedback` table + `callFeedbackQueries`) — Security/Vijay FYI, merging fast. T1: `isRoutineEvent` helper in briefing.ts + GREETING prompt. R12–R16 all merged + green (2044). Preflight+commit per ticket.)_ | `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `app/api/briefing/feedback/route.ts`, `app/dashboard/page.tsx` | 2026-06-20 |
