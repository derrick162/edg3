...
| 🔒 Security | `security` | _(✅ **T1-1 webhook retry + DLQ done (1809 green).** `lib/retry.ts` `withRetry` (3×, exp backoff) wraps the Vapi transcript fetch; outer catch dead-letters failed call processing to `failed_webhooks` (6am digest surfaces it). Synced master (1803). Prior overnight work (Tier 0+1, durability+restore drill, scheduler lock, account-deletion FK-500 fix, T3-3 activity-log + people export) all merged. ⚠️ **5-min AM call: confirm export boundary** (episodes/fact_history/focus_milestones/support_messages deferred). ⚠️ **Derrick/Kevin AM action list at top of `content/qa-log.md`** (Railway volume, LITESTREAM env, key backup, restore drill). Commits local-only, not pushed.)_ | `app/api/vapi/webhook/route.ts`, `lib/retry.ts`, `lib/db.ts` | 2026-06-18 |
