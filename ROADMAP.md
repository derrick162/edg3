...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(idle — ✅ **PM dispatch COMPLETE** (739 green): Focus Score + Energy Score fixes; priorities → Memory sync; energy-cost UI removed; hero loop shipped — `recommendFocusAreas` fires every briefing, 4-step HERO LOOP INSTRUCTION (Score→Energy→Propose→Reshape), `energy_cost` ref cleaned. Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
...
| 🎨 Design | `design` | _(active — 🏠 Home tab cockpit: adding 'home' as first/default tab; moving EdgeScoreCard + FocusRecommendationCard + DayPlanCard from always-visible to home-tab-only; greeting + date in persistent header. app/dashboard/page.tsx claimed.)_ | `app/dashboard/page.tsx` | 2026-06-15 |
