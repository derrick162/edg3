...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R14 ALL 5 SHIPPED (1986 green) — ready for PM merge.** findFreeTime, createEvent recurrence + attendees, editEventAttendees, addTask/completeTask, forgetFact. New pure modules attendees/taskMatch/factForget + computeFreeSlots/buildRrule; +30 tests. ⚠️ **External (PM):** create 5 Vapi tools (findFreeTime, editEventAttendees, addTask, completeTask, forgetFact) + add recurrence/attendees params to createEvent; paste UUIDs into both toolIds arrays (placeholders in `lib/vapi.ts`). R9/R12/R13/R14 all shipped on `core`. Idle, awaiting next dispatch.)_ | `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts`, `lib/time.ts`, `lib/attendees.ts`, `lib/taskMatch.ts`, `lib/factForget.ts` | 2026-06-20 |
