...
|---|---|---|---|---|
| 📱 Mobile IC | `cloud/*` | _(✅ **PR #2 Journaling Mode merged to master (9ae004d). 2451 green. Deployed.** Journal tab: Start Journal call (listen-first prompt, recorded + transcribed) + desktop screen recording (local download, 5-min cap). Follow-up parked: journaling as default for existing calls. Idle.)_ | — | 2026-07-30 |
...
| 🔒 Security | `security` | _(✅ **S10 — trade-alert trust surface SHIPPED + PUSHED to origin/security (2495 green).** `POST /api/vapi/trade-alert` gates: constant-time key compare (never logged) → body validation (+ optional `alertId`) → target resolve → per-user kill switch → market-hours (DST-aware) → idempotency dedupe (before cap) → ≤3/day cap → dispatch seam + audit. Shared `guardTradeAlertKey()` gates BOTH endpoints (Core's GET watch-list feed must call it). `tradeAlertPoll` bucket for the poll. New `lib/tradeAlert.ts` + route + `claimTradeAlert` + `trade_alerts_enabled` column/audit (all additive). +32 tests. ⚠️ Handoff: Darren exports `initiateTradeAlertCall` from lib/vapi.ts + calls `guardTradeAlertKey` in his GET route. ⚠️ Go-live: `TRADE_ALERT_KEY` on both Railway services. S9 MERGED (4f895fc)+deployed. S1–S8 done.)_ | `app/api/vapi/trade-alert/**`, `lib/tradeAlert.ts`, `lib/db.ts`, `lib/rateLimit.ts`, `lib/idempotency.ts` | 2026-07-31 |
| 🔧 PM/CoS | `master` | _(✅ **Cloud branch `edge-call-feedback-v38s51` merged + pushed (28dbdfc). 2435 green. Deployed.** Fixes: declined/short-call gate (streak), memory freshness (`last_confirmed_at`), broader fact capture. ⚠️ **Vijay (Security) — sync down**: `app/api/vapi/webhook/route.ts` + `lib/vapi.ts` touched. Idle.)_ | `app/api/vapi/webhook/route.ts`, `lib/vapi.ts`, `lib/db.ts`, `lib/facts.ts` | 2026-07-01 |
| 🎨 Design | `design` | _(✅ **Dispatch D20–D30 ALL SHIPPED. Pillar sweep done — no unclaimed Design items remain in any pillar.** QA checklists require live call access (Derrick/PM). Idle — awaiting next dispatch.)_ | — | 2026-06-24 |
