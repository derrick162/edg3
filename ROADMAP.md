...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **OVERNIGHT PILLAR LOOP (1735 green) — merged to master**: M2-1/2 sleep-time consolidation + rememberPreference; M3-1/2/3 briefing context + searchMemory + commitment tracking; M4-2/3b fact versioning + rollback + undo coverage; DC2-0/1/2/3/3b/4 full briefing quality pass; T2-2/3/4 accuracy + honest failure; T4-5 undo coverage. ⚠️ External: create `confirmFact` Vapi tool (param `topic`).)_ | `lib/briefing.ts`, `lib/vapi.ts`, `lib/facts.ts`, `lib/undo.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **OVERNIGHT — Trust Tier 0+1 + bug fixes (1707 green) — merged to master.** T0-1 durability self-check + restore drill, T0-4 scheduler lock, T0-2 startup key check, M2-4 context-pack guard. 🐞 Fixed account-deletion FK-500 bug. ⚠️ Derrick AM action list in `content/qa-log.md`.)_ | `lib/backup.ts`, `lib/db.ts`, `lib/durability.ts`, `lib/scheduler.ts` | 2026-06-18 |
| 🔧 PM/CoS | `master` | _(✅ **All overnight lanes merged (security ✅ core ✅ design ✅). Preflight running.** ⚠️ Derrick action items: Twilio follow-up, 5 design-partner names, CASA video, push to prod + read `content/qa-log.md` AM action list.)_ | `ROADMAP*.md` | 2026-06-18 |
| 🎨 Design | `design` | _(✅ **UX-2/3/4 shipped + merged to master** — People section filters self/Edg3/AI entities + dedupes near-identical facts; global `cursor: pointer` CSS rule added; dynamic collapse init collapses all but first 3 populated categories. Preflight green (1703). Next: PILLAR-TRUST.md UX items.)_ | `app/dashboard/page.tsx`, `app/globals.css` | 2026-06-18 |
