...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Briefing V2 + Episode Dashboard (1418 green, preflight clean)**: Episodes surface in Memory tab (`GET /api/episodes` + dashboard "Call history" section). Briefing V2 #1 (recovery tied to specific event), #4/#5 (free-slot afternoon choice), #7 (tomorrow forward-look), #8 (personal all-day warmth). Episode nightly prune wired. Previous: M5 Episode Store, M4 Accountability Memory. Awaiting PM merge.)_ | `lib/briefing.ts`, `lib/vapi.ts`, `app/api/episodes/route.ts`, `app/dashboard/page.tsx`, `lib/db.ts`, `lib/scheduler.ts` | 2026-06-18 |
