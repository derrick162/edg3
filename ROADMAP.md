...
| 🔒 Security | `security` | _(✅ **DISPATCH S1–S8 ALL DONE (2394 green) — full S-queue complete; ready for merge; idle.** S8: bucket audit (all per-user/IP, none global) + `vapiWebhook` per-IP flood ceiling (1000/min, sheds 200) + `factExtraction` per-user ceiling (10/hr); +4 tests; documented. ⚠️ S5 instrumentation hooks in Core files (briefing/callMemory/facts) still pending Darren sync down. Awaiting next dispatch.)_ | `lib/rateLimit.ts`, `app/api/vapi/webhook/route.ts`, `content/security-audit.md` | 2026-06-25 |
