...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(📋 **R9 DISPATCHED (2026-06-20) — 4 tickets from Derrick's live call feedback.** T1: voice speed -10% (`speed: 0.9` on VOICES in `lib/vapi.ts`). T2: briefing fallback silent fix + priority description truncation in `buildFallbackBriefing`. T3: spelling override prompt rule (`lib/vapi.ts`). T4: `getWeather` Vapi tool (Open-Meteo, no API key). Full specs in `ROADMAP-CORE.md` R9 section. Idle/awaiting Darren.)_ | `lib/vapi.ts`, `lib/briefing.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-20 |
