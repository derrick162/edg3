...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(⚠️ **R8 P0 bugs are ALREADY SHIPPED on master — re-dispatch is stale, nothing to rebuild.** Verified on synced HEAD `596d3b3`: **#1** empty-focus guard `FocusRecommendationCard.tsx:208` (`!recommendation \|\| areas.length===0`); **#2** `classifyEvent`/`needsPrepSuggestion` `eventMatch.ts:38/48`, wired `calendarPlan.ts:215`; **#3** M4-4 `peopleModelQueries.upsert` `facts.ts:746` + `buildPeopleModelBlock` `briefing.ts:796`. Also Gmail-link→facts trigger SHIPPED+merged (1951 green). **Kevin: please confirm against master before re-dispatching R8.** Idle, awaiting NET-NEW work.)_ | `app/api/learned/route.ts`, `app/dashboard/page.tsx` | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **R9 Ticket 1 SHIPPED (1958 green) — full email body support hardened + Gmail-read audit gap closed.** `getRecentEmailSignal({fullBodies})` already existed (Round 7); added `truncateAtSentenceBoundary` (clean sentence-boundary cut, replaces hard slice) + 7 tests (3 fullBodies integration, 4 truncation). **Audit follow-up:** confirmed `gmailIngest` rate-limit (6/hr) present; **closed audit gap** — `extractGmailAccountContacts` now emits encrypted `gmail_contacts_fetch` receipt (was creating person facts with no audit). `content/security-audit.md` updated w/ R9 section. All in `lib/gmail.ts` (Security-owned). **⚠️ EXTERNAL (Derrick) still open:** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, live restore drill. Idle/polling.)_ | `lib/gmail.ts`, `lib/gmail.test.ts`, `content/security-audit.md` | 2026-06-19 |
