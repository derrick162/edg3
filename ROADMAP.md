...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(idle — ✅ **Hero Loop COMPLETE** (702 green): `lib/calendarPlan.ts` (`findFreeSlot` + `buildCalendarPlan`), `applyCalendarPlan` tool (two-step confirm, creates focus block + moves mismatch event, plan-level undo, re-scores after). `worstMismatchEventId/Title` added to `ScoreResult`. NEXT: energy color-coding (`colorByEnergy` via `event_energy_tags`).)_ | — | 2026-06-15 |
