...
| 🔒 Security | `security` | _(🏗️ **R13 IN PROGRESS — T1 already shipped on master (`6abf51c`, verified); building T2 (Gmail-reading indicator) + T3 (`searchEmailsBySubject`).** ⚠️ Claiming `app/dashboard/page.tsx` for the T2 sidebar indicator (small additive diff — restoring `calendarHasGmailScope` read-only state).)_ | `lib/gmail.ts`, `app/api/auth/accounts/route.ts`, `app/dashboard/page.tsx` | 2026-06-20 |
