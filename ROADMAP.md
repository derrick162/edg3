...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(idle — ✅ **Day-scoped Focus Recommendation v1 COMPLETE** (`7264397`, 677 green): `daily_focus` table, `recommendFocusAreas(userId, opts)` (energy+anchor+date), `/api/focus/recommend`, `/api/focus/confirm`, `confirmFocus` tool handler, briefing wired. NEXT slice: ONE Edge Score (collapse Focus%+Energy% → 0–100) + fix Energy-defaults-to-100 bug.)_ | — | 2026-06-15 |
| 🔒 Security | `security` | _(✅ **Overnight queue COMPLETE** (673 green): (1) `applyCalendarPlan` durability — `calendar_plan_executions` idempotency table + plan-level undo group (`undoPlan`) + 22 new tests; (2) Privacy plumbing — `calendar_scores`/`energy_profile`/`event_energy_tags` added to data export + privacy page Gmail disclosure corrected for inbox signal; (3) CASA checklist updated — privacy policy done, self-service deletion done, token revocation + demo video + PM consent decision remain; (4) Durability audit — all new write paths confirmed idempotent. Awaiting PM for next dispatch.)_ | — | 2026-06-14 |
