...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R17 + pillar sweep SHIPPED (2056 green) — merged to master.** R17: sharper opener (`isRoutineEvent`) + post-call feedback (`call_feedback` table, `POST /api/briefing/feedback`, ★★★★★ row). Trust T3-1: `factDisplayStatement` renders pattern facts' `.summary` not raw JSON. All dispatch + buildable pillar items exhausted. Idle, awaiting next dispatch.)_ | `lib/factDisplay.ts`, `app/dashboard/page.tsx`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `app/api/briefing/feedback/**` | 2026-06-20 |
| 🔒 Security | `security` | _(✅ **`GET /api/notifications/history` SHIPPED (2060 green); idle/polling.** Returns `notification_log` rows rendered as `{type,title,body,sentAt}`, newest-first limit 10. **🔔 @Cam: repoint `NotificationHistoryPanel` from `/api/notifications` → `/api/notifications/history`.** R15 T1 (Litestream) = already built, needs only Railway S3 env vars (proposed bucket `edg3-prod-db-backups`). Trust Tier 0/1 + DAILY-CALL DC1/DC3-2 Security items all shipped — pillar surface exhausted. **⚠️ EXTERNAL (Derrick):** Railway VAPID keys + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `app/api/notifications/history/**`, `lib/db.ts` | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **All 3 lanes merged (2056 green). R17+Security-R15+Design-R15 on master. VAPID set on Railway. ⚠️ Litestream S3 creds still needed — see Security row. ⚠️ Design flagged `/api/notifications/history` missing — needs Security to ship it.**)_ | `ROADMAP.md` | 2026-06-20 |
| 🎨 Design | `design` | _(✅ **R15 T1+T2 shipped (1956 green) — merged to master.** T1: `/score` page (FocusScoreCard + breakdown bars + streak). T2: NotificationHistoryPanel sidebar widget. ⚠️ `/api/notifications/history` route missing — panel shows empty-state until Security ships it. Idle, awaiting next dispatch.)_ | `app/score/page.tsx`, `components/ui/NotificationHistoryPanel.tsx`, `app/dashboard/page.tsx` | 2026-06-20 |
