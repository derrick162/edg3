...
| 🔧 PM/CoS | `master` | _(✅ **June 18 PM session shipped (1870 green, all pushed):** Gmail ingest UX (post-connect banner → Memory tab), Activity UTC fix, TODAY'S FOCUS card copy fix, fallback briefing fix (filter past events, strip brackets, 45s timeout), copy button name correction. **Bug fix:** Gmail ingest was writing `'people'` category — corrected to `'person'` (CHECK constraint). **Facts in briefing fixed:** full briefing now injects `WHAT EDGE KNOWS` block so goals/projects/people surface. ⚠️ Derrick: disconnect + reconnect secondary Gmail account (needs new `gmail.readonly` scope).)_ | `ROADMAP*.md`, `lib/briefing.ts`, `lib/gmail.ts`, `app/api/auth/google/gmail/ingest/route.ts`, `app/dashboard/page.tsx`, `components/ui/FocusRecommendationCard.tsx` | 2026-06-19 |
| 🎨 Design | `design` | _(✅ **Pillar sweep + speculative polish complete**: M4-3b fact history expand+rollback UI shipped; Derrick review tickets 1-4/6 done; landing T3+T5; all 3 pillars exhausted for Design; QA log updated; token sweep (Button/NotificationCenter). Ticket 5 resolved by Core's FocusRecommendationCard cleanup. 1870 green.)_ | `app/dashboard/page.tsx`, `components/ui/Button.tsx`, `components/ui/NotificationCenter.tsx`, `content/qa-log.md` | 2026-06-19 |
