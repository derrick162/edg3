...
| 🔒 Security | `security` | _(✅ **S9 — prod DB schema-drift repair SHIPPED local (2463 green) — committed on `security`, awaiting PM merge + deploy. URGENT.** Root cause: migration runner swallowed all failures + SQLite can't ALTER a CHECK/DEFAULT. Fixed S9a (facts.last_confirmed_at non-constant-default ALTER → table rebuild), S9b (briefings.status CHECK +'missed'), S9c (facts.category CHECK +5 categories), S9d (migration runner now logs non-benign failures). Idempotent guarded rebuilds under `foreign_keys=OFF`; rows+FKs+indexes preserved. +12 tests. Synced PR #2 (Journaling Mode) — added is_journal/audio_url to the rebuild schema. S1–S8 previously done.)_ | `lib/db.ts`, `lib/db-schema-drift.test.ts` | 2026-07-30 |
