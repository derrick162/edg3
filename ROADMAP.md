...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Memory-tab P0 batch + Esther dispatch SHIPPED — 1945 green, pushed to Railway.** Fixed: self-entity leak (selfName.ts Soundex), health hallucination guard (factGuards.ts), meetings-with-self attendee filter, preference recall nudge. ⚠️ Gmail-link→no-facts ROUTED TO SECURITY: Vijay to decide if connect should kick one-off extraction. **Next: Round 7 full email body reading (dispatched) → DC3-1 voice anchors (PILLAR-DAILY-CALL.md).**)_ | `lib/selfName.ts`, `lib/factGuards.ts`, `lib/facts.ts` | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **NOT stalled — all dispatched work verified SHIPPED on master; loop is live. Correcting the board.** (1) **R8 T2 `setEnergyLevel` — DONE/LIVE:** handler `app/api/vapi/tool-call/route.ts:1224` (validates red/yellow/green + manual/override, writes `energy_log` via `energyLogQueries.upsert`); runs through the unified `/api/vapi/tool-call` webhook (already enforces `checkVapiSecret`+rate-limit+idempotency for all tools) — deliberately NOT a separate `/api/vapi/energy` route (would break the single-webhook pattern every other tool uses). **Vapi tool UUID `8aac93a3…` already created 2026-06-14 + wired `lib/vapi.ts:276` — Derrick does NOT need to create it.** (2) **R6 T1 predictive context-loading — DONE/LIVE:** `briefing_context_packs` table `lib/db.ts:527` + queries + `runNightlyContextPacks` `lib/scheduler.ts:163` (11pm user-local, idempotency + empty-pack guards). (3) people_models + privacy email-body disclosure confirmed on master. **No phantom Derrick action; no dispatch needed — idle/polling, will merge + execute any genuinely-new ticket on arrival.**)_ | `lib/db.ts`, `lib/scheduler.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-19 |
| 🔧 PM/CoS | `master` | _(✅ **Memory-tab P0 + Design UX overhaul MERGED + PUSHED — 1945 green, master at `9f5400a`, Railway deploying.** Operating model confirmed: product bugs → PM directly; strategy → Esther first. **🚨 DERRICK — still pending:** (A) Railway persistent volume at `/data` + Litestream env vars (durability runbook: `content/durability-runbook.md`). (B) Open Vijay's session and dispatch R8 Security work (setEnergyLevel route + R6 predictive context loading).)_ | `ROADMAP*.md` | 2026-06-19 |
| 🎨 Design | `design` | _(✅ **Memory tab UX overhaul + nav fix SHIPPED (1945 green).** Removed clutter, filtered self, reordered sections; Today tab first, flattened nav (removed broken More dropdown). **Next: landing page redesign (Derrick's feedback: T1–T4 in ROADMAP-DESIGN.md) — dispatched.**)_ | `app/dashboard/page.tsx`, `app/globals.css` | 2026-06-19 |
