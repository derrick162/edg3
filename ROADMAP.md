...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Memory-tab P0 batch DONE (1945 green) — ready for PM merge.** Fixed: (1) self-entity leak under nickname/STT/initial variants — new `lib/selfName.ts` Soundex matcher catches "derek"→Derrick; (2) hallucinated health metrics ("weighs 122 lbs") — new `lib/factGuards.ts` drops body-measurement facts whose number isn't in the source; (4) preference recall prompt nudge; (5) "meetings with self" — attendee filter now name-matches the user too. ⚠️ **[3] Gmail-link→no-facts ROUTED TO SECURITY**: email facts are briefing-triggered not OAuth-triggered (expected until next call); the auth callback is Security-owned — Vijay to decide if connect should kick a one-off extraction. **Committed on `core`, NOT pushed — PM please merge.**)_ | `lib/selfName.ts`, `lib/factGuards.ts`, `lib/facts.ts`, `lib/relationships.ts`, `lib/briefing.ts` | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **T3-1-A DONE (1918 green) — Darren unblocked for T3-1-B.** `'pattern'` added to facts CHECK + `Fact.category` type; +`pattern` key in 2 Core Record maps (forced by type ripple). Also done: people_models export (T2), privacy email-body disclosure (T1). ⚠️ **Existing-DB note:** CHECK change is fresh-DB-only (SQLite can't ALTER CHECK) — flagged a safe rebuild as available if a persistent pre-change prod DB exists. ⚠️ Railway volume P0 = Derrick/Kevin dashboard action (my durability self-check + runbook already shipped). Idle — awaiting dispatch.)_ | `lib/db.ts`, `lib/factConfidence.ts`, `lib/memorySalience.ts`, `app/api/account/export/route.ts` | 2026-06-19 |
| 🔧 PM/CoS | `master` | _(✅ **Round 8 + P0 + Security patches SHIPPED — 1918 green, master at `2ff2dd3`.** `staging` branch created + pushed (origin/staging). **🚨 DERRICK — TWO Railway action items:** (A) **Prod volume** (root cause missed calls + lost memory): Railway → Edg3 prod → Volumes → confirm persistent volume at `/data`. Set `LITESTREAM_S3_BUCKET/ACCESS_KEY_ID/SECRET_ACCESS_KEY`. Runbook: `content/durability-runbook.md`. (B) **Staging env** (Esther request): Railway → Edg3 project → New Environment → name `staging` → add volume at `/data` → copy all prod env vars (update `APP_URL`/`NEXTAUTH_URL` to staging URL) → set deploy branch to `staging`. After setup, report staging URL. **Then:** send `/loop` to Darren/Vijay/Cam for next dispatch.)_ | `ROADMAP*.md` | 2026-06-19 |
| 🎨 Design | `design` | _(✅ **R8 T1+T2+T3 SHIPPED — FocusScoreboard hours bar + tab reorder + People section polish (goals/comms style badges).** Idle — awaiting dispatch.)_ | `components/ui/FocusScoreboard.tsx`, `app/dashboard/page.tsx` | 2026-06-19 |
