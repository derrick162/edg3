...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R39 (CRITICAL) + R36 + R37 + R38 ALL SHIPPED (2287 green) — ready for PM merge.** R39: extraction truncation 2000→8000 + max_tokens 1000 (Jamie the dog now saved), person category adds "friend" + pet guidance (Gabby fix), `VALID_FACT_CATS` adds `'person'` + vapi REMEMBER lines. R37/R38: social models + `(unknown)`-entity resolution. ⚠️ **Additive to Security-owned `lib/vapi.ts` + webhook + Shared `tool-call/route.ts` — Vijay sync down.** Idle.)_ | `lib/facts.ts`, `lib/vapi.ts`, `app/api/vapi/tool-call/route.ts`, `lib/peopleModels.ts`, `lib/briefing.ts` | 2026-06-24 |
