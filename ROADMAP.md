...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R16 ALL 3 SHIPPED (2010 green) — merged to master.** T1 accountability (pickTopCommitment + COMMITMENT CAPTURE/ACCOUNTABILITY CLOSE), T2 daily Focus Score (`lib/focusScore.ts` → `/api/scores` `dailyFocusScore`), T3 push SW + subscribe (VAPID-guarded). Idle, awaiting next dispatch (R14 T1–T5 + R15 T1–T7 queued).)_ | `lib/focusScore.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/scores/route.ts`, `app/dashboard/page.tsx`, `public/sw.js` | 2026-06-20 |
| 🔒 Security | `security` | _(🔎 **R15 T1 (Litestream S3) — FINDINGS: already built + wired on master; needs only Railway env vars.** Build = **NIXPACKS (no Dockerfile)**, but `railway.toml` already sets `startCommand = "sh scripts/start.sh"` → **no Dockerfile needed.** `litestream.yml` (S3 replica, 1s sync, 72h retention, 6h snapshots) + `scripts/start.sh` (restore-on-fresh-volume → `litestream replicate -exec "npm start"`) present + correct — and **more correct than the R15 spec**: uses `/data/edg3.db` (the persistent volume) not `/app`, and Litestream supervises the app. (Spec's `node server.js` Dockerfile wouldn't work — Next is NOT `output:standalone`.) **➡️ PM/Derrick: set `LITESTREAM_S3_BUCKET` + `LITESTREAM_S3_ACCESS_KEY_ID` + `LITESTREAM_S3_SECRET_ACCESS_KEY` (+ optional `_REGION`). Proposed bucket: `edg3-prod-db-backups`.** ⚠️ Robustness note: start.sh downloads the litestream binary at boot (wget/curl) — graceful fallback to plain `npm start` if unavailable; bake-in via nixpacks.toml is optional hardening. Then run the restore drill.)_ | `litestream.yml`, `scripts/start.sh`, `railway.toml` (pre-existing) | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **R17 dispatched to Core (opener sharpening + call feedback signal), Security R15 dispatched (Litestream S3 backup — P0 durability), Design R15 dispatched (Focus Score page + notification history). VAPID keys set on Railway — push active after redeploy.** 2044 green.)_ | `ROADMAP-CORE.md`, `ROADMAP-SECURITY.md`, `ROADMAP-DESIGN.md`, `ROADMAP.md` | 2026-06-20 |
