...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R35 T1 SHIPPED (2243 green) — ready for PM merge** (R28–R34 + R29 Part D + M4-5 also awaiting merge). Browser timezone auto-detect on dashboard load (`pickTimezoneUpdate` + mount effect → `POST /api/profile/timezone`) — closes the unset-tz→LA failure class. Spec said `/api/profile {timezone}` but that 400s; used the correct `/api/profile/timezone {current_timezone}`. M4-5 cron wiring still pending Security (weekly+monthly synthesis jobs). Idle — dispatch queue exhausted.)_ | `lib/timezoneDetect.ts`, `app/dashboard/page.tsx` | 2026-06-24 |
