...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12–R16 ALL MERGED to master + integration verified GREEN (2044 tests, tsc+build clean).** R16: accountability loop, daily Focus Score (`dailyFocusScore` on `/api/scores`), push SW+subscribe — now end-to-end with Security R14 (`/api/notifications/subscribe` live). R14 T6 Gmail indicator reconciled cleanly with Security R13 T2. ⚠️ **FYI all lanes:** after pulling master, run `npm install` — Security's R14 added `web-push` (declared in package.json; not auto-installed). Core idle, awaiting next dispatch.)_ | `lib/focusScore.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/scores/route.ts`, `app/dashboard/page.tsx`, `public/sw.js` | 2026-06-20 |
| 🔒 Security | `security` | _(✅ **R13 + R14 DONE (1990 green); idle/polling — released `lib/db.ts`/`lib/scheduler.ts` claims.** R14 T1: `web-push` + VAPID + `push_subscriptions`/`notification_log` tables + `lib/push.ts` (`sendPushToUser`) + subscribe/unsubscribe routes. R14 T2: `lib/proactiveNotifications.ts` — low-recovery (7:30) + priority-gap (9:00 Tue–Thu) cron. **⚠️ EXTERNAL (Derrick):** set Railway VAPID keys (`VAPID_PUBLIC_KEY`/`NEXT_PUBLIC_VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT`) — push is a silent no-op until then.)_ | `lib/push.ts`, `lib/proactiveNotifications.ts`, `lib/scheduler.ts`, `lib/db.ts`, `app/api/notifications/**` | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **All 3 lanes merged (2044 green). R14+R15+R16+Security-R13+R14+Design-R14+R15-prep all on master. 13 new Vapi tools created + wired (R14 T1–T5 + R15 T1–T7). ⚠️ Darren: sync down `git merge master`; Core row says R14/R15 queued but they shipped — update status. ⚠️ EXTERNAL: VAPID keys needed on Railway for push to activate.)**)_ | `lib/vapi.ts`, `ROADMAP.md` | 2026-06-20 |
