...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(idle — ✅ **5-item PM dispatch COMPLETE (759 green)**: email→memory (Haiku fact extraction from inbox); home page IA (Home tab as default cockpit, Briefings = own tab); sleep score in Whoop APIs; EdgeScoreCard calibrating wired; Fix It button end-to-end. ⚠️ Derrick must reconnect Google for gmail.readonly. Awaiting PM.)_ | — | 2026-06-15 |
| 🔒 Security | `security` | _(✅ **open_loops + performancePct COMPLETE** (787 green): (1) open_loops table + openLoopQueries (list/insert/resolve/dismiss/prune) — description encrypted at rest; (2) Privacy plumbing: deletion in self-service + admin paths, data export coverage, 30-day retention; (3) WhoopSleepDay.performancePct added — unblocks Core's Energy Score. ✅ **Privacy audit COMPLETE** (787 green): all email-derived PII fields encrypted; watched_threads 30-day retention prune + nightly cron; CASA code items all done. ⚠️ **PM DECISION:** `facts.statement` plaintext — LLM-distilled email facts not encrypted (see ROADMAP-SECURITY.md). Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
...
| 🎨 Design | `design` | _(idle — ✅ **Open Loops dashboard surface**: `OpenLoopsSection` 3-bucket component (commitment / awaiting / deadline) + dashboard wiring. Preflight clean. Moving to items 3–5: Fix-It copy, Edge Score live-number, home-cockpit polish.)_ | — | 2026-06-15 |
