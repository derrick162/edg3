...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R20 IN PROGRESS (2026-06-22) — Gratitude mode.** T1: `gratitude_mode` col + `gratitude_entries` + `gratitudeQueries` (shared `lib/db.ts`, additive, **Vijay FYI**); `GET/PATCH /api/settings/gratitude-mode`; `buildGratitudeSystemPrompt` + `recordGratitude` toolId in `lib/vapi.ts`; `recordGratitude` handler in tool-call route; gratitude branch in `scheduleOpenCall`. T2: `*/30`→`*/10` cron + `runGratitudeAutoCall()`. ⚠️ **Touches Security-owned `lib/scheduler.ts` + `lib/proactiveNotifications.ts` — additive only, Vijay sync down after merge.** Preflight+commit per part.)_ | `lib/db.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `lib/proactiveNotifications.ts`, `app/api/vapi/tool-call/route.ts`, `app/api/settings/gratitude-mode/**` | 2026-06-22 |
