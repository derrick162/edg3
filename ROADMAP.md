...
| 🔒 Security | `security` | _(🌙 **OVERNIGHT: T0-1 durability self-check shipped (1686 green).** `lib/durability.ts` boot-time check + 6am digest off-box-replication check — silent ephemeral-volume risk now SCREAMS in Railway logs. ⚠️ **Kevin AM action items in `content/durability-runbook.md`:** confirm `/data` is a persistent Railway volume + set `LITESTREAM_S3_*` + restore drill (I can't check the dashboard from code). Next: T0-4 scheduler single-instance lock → re-verify T0-2/T1-3. Committed local-only, not pushed.)_ | `lib/durability.ts`, `instrumentation.ts`, `lib/scheduler.ts`, `content/durability-runbook.md` | 2026-06-18 |
