...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **COMPLIANCE GATE COMPLETE (1320 green, preflight clean)**: `data_consent` DB column + `userQueries.setDataConsent` + `POST /api/onboarding/consent` + `GET /api/profile` returns consent. CASA consent UI (Security + Design) now fully wired end-to-end. Previous: T4 STT canonicalization (3 write paths); Trust Bug T2. Awaiting PM merge.)_ | `lib/db.ts`, `app/api/profile/route.ts`, `app/api/onboarding/consent/route.ts` | 2026-06-18 |
