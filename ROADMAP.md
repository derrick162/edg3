...
| 🔒 Security | `security` | _(🌙 **OVERNIGHT: T0-1 + T0-4 shipped (1693 green).** T0-1 durability self-check (`lib/durability.ts` + 6am digest) — silent ephemeral-volume risk now SCREAMS in Railway logs. T0-4 single-instance scheduler lock (`scheduler_lock` table + atomic acquire/release) — no double-dial across replicas/overlapping ticks. ⚠️ **Kevin AM action items in `content/durability-runbook.md`:** confirm `/data` is a persistent Railway volume + set `LITESTREAM_S3_*` + restore drill (I can't check the dashboard from code). Next: T0-2 step-3 startup key-presence check → into Tier 1. Committed local-only, not pushed.)_ | `lib/durability.ts`, `lib/db.ts`, `lib/scheduler.ts`, `content/durability-runbook.md` | 2026-06-18 |
