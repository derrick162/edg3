...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **OVERNIGHT PILLAR LOOP (1737 green, 3x clean, preflight clean)**: M4-3b (fact versioning+rollback), T4-5 (undo coverage), M4-1+Round6-T2 (mid-call reconfirmation — `lib/factConfidence.ts`, category-weighted, `confirmFact` tool), DC0-2 (post-call latency), UX-4 (no false hedging), removed DC2-3b dup, **fixed flaky preflight** (calendar mock in facts/call-to-briefing tests). QA log updated. ⚠️ External: create `confirmFact` Vapi tool (param `topic`). ⚠️ Security flag: decay-job category mismatch (see ROADMAP-CORE changelog). Local commits only — awaiting PM merge.)_ | `lib/factConfidence.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/vapi/webhook/route.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-18 |
