...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Second batch (1747 green) — merged to master**: M4-3b fact versioning, mid-call reconfirmation trigger, confirmFact tool, DC0-2 post-call latency, M2-3 priority-drift pattern, M2-4 context-pack wiring, flaky preflight fixed. ⚠️ External: create `confirmFact` Vapi tool (param `topic`). ⚠️ §2 VIOLATION reported: two sessions ran in edg3-core overnight — PM to re-establish single ownership.)_ | `lib/factConfidence.ts`, `lib/patternMemory.ts`, `lib/briefing.ts`, `lib/vapi.ts` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **T3-3 + T1-5 + T3-4 + T4-3 — 1708 green — merged to master.** Activity log added to data export; rate limit sweep clean (37/37 routes); WAL+busy_timeout confirmed; briefing_context_packs CASCADE fix. ⚠️ Derrick AM action list in `content/qa-log.md`. Next: T1-1 webhook retry.)_ | `app/api/account/export/route.ts`, `lib/backup.ts`, `lib/db.ts` | 2026-06-18 |
