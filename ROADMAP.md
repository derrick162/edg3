...
| 🔒 Security | `security` | _(✅ **R13 + R14 DONE (1990 green); idle/polling — released `lib/db.ts`/`lib/scheduler.ts` claims.** R14 T1: `web-push` + VAPID + `push_subscriptions`/`notification_log` tables + `lib/push.ts` (`sendPushToUser`) + subscribe/unsubscribe routes. R14 T2: `lib/proactiveNotifications.ts` — low-recovery (7:30) + priority-gap (9:00 Tue–Thu) cron via `*/30` dynamic-import. **📣 @Kevin: subscribe route + `NEXT_PUBLIC_VAPID_PUBLIC_KEY` ready for Core R16 T3 front-end.** **⚠️ EXTERNAL (Derrick):** set Railway VAPID keys (`VAPID_PUBLIC_KEY`/`NEXT_PUBLIC_VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT`) — push is a silent no-op until then; + Railway volume/`LITESTREAM_S3_*`/key backup/restore drill.)_ | `lib/push.ts`, `lib/proactiveNotifications.ts`, `lib/scheduler.ts`, `lib/db.ts`, `app/api/notifications/**` | 2026-06-20 |
