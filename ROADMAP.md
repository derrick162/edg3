...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(📋 **R20 DISPATCHED (2026-06-22) — T1: Gratitude mode (schema + prompt + recordGratitude handler + scheduleOpenCall branch + background sound). T2: */10 cron + auto-trigger on Whoop score. R19 fully merged.**)_ | `lib/db.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `lib/proactiveNotifications.ts`, `app/api/vapi/tool-call/route.ts`, `app/api/settings/gratitude-mode/**` | 2026-06-22 |
| 🔒 Security | `security` | _(✅ **Twilio outbound-number hotfix — NO code change needed (2072 green); idle/polling.** `lib/vapi.ts` + `app/api/briefing/intro` already read `phoneNumberId` from `process.env.VAPI_PHONE_NUMBER_ID` (no hardcoded id). Documented the var in `.env.example` with the Twilio id. **➡️ DERRICK: set `VAPI_PHONE_NUMBER_ID=5c00719e-b341-463a-8f2d-54859cb927ac` on Railway → redeploy → calls originate from the Twilio number.** (R17 also done.) **⚠️ EXTERNAL (Derrick) still open:** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `.env.example` | 2026-06-22 |
