...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **OVERNIGHT PILLAR LOOP (1735 green, preflight clean)**: M4-3b (fact versioning + rollback), T4-5 (undo coverage), M4-1+Round6-T2 (mid-call reconfirmation — `lib/factConfidence.ts` consumes Security's confidence_score/last_confirmed_at → briefing RECONFIRM block + `confirmFact` tool). Removed DC2-3b duplicate. QA log at `content/qa-log.md`. ⚠️ External: create `confirmFact` Vapi tool (param `topic`). Continuing down pillars. Local commits only — awaiting PM merge.)_ | `lib/factConfidence.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/undo.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-18 |
