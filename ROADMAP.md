...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R24 SHIPPED (2117 green) — ready for PM merge.** Onboarding "Connect your tools" step (`app/onboarding/page.tsx`): Google Calendar+Gmail **required** (Continue gated, shows ✓ + connected email) + Whoop **optional** (Skip for now); pre-checks `/api/auth/accounts` + `/api/whoop/status` on mount, popup+postMessage OAuth. Gating logic in pure `lib/onboardingConnect.ts` + 4 tests. (R23 already merged — rich open-call memory + inbound calls + TOOL CALL DISCIPLINE hotfix.) Idle, awaiting next dispatch.)_ | `app/onboarding/page.tsx`, `lib/onboardingConnect.ts` | 2026-06-23 |
