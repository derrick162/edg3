...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **M2 RELATIONSHIP MEMORY COMPLETE (1353 green, preflight clean)**: `people_profiles` table + `lib/relationships.ts` (pure compute + sync + briefing block) + `GET /api/relationships` + `POST /api/relationships/sync` + briefing wiring (fire-and-forget sync + RELATIONSHIP CONTEXT injected) + Memory tab "People you meet with" section. Awaiting PM merge.)_ | `lib/db.ts`, `lib/relationships.ts`, `lib/briefing.ts`, `app/api/relationships/**`, `app/dashboard/page.tsx` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **DATA CONSENT + CASA COMPLETE (1267 green)**: data_consent UI + export forward-compat; sentinel comments in briefing/facts/outreach LLM paths; full CASA section in security-audit.md (two-setting table, data-flow inventory, OAuth checklist). Round-3 hardening (36 rate-limit types, error-leak fixes, input caps) also in. Awaiting PM dispatch.)_ | — | 2026-06-18 |
| 🔧 PM | `master` | _(✅ **Activation + T4 + T2 + CASA + content integrating → see latest commit.** All lanes' work merged: Core (activation real-data wiring, T4 transcript canonicalization, T2 event-title spelling, briefing gym-credit), Security (CASA data-consent + round-3 hardening), Design (ActivationReveal + ActivationHeroCard + DataConsentScreen), Esther (29-doc GTM/content library). Onboarding conflict resolved to consistent component contract. ⚠️ Derrick action items: push to prod, Twilio follow-up, pick 5 design-partner names, record CASA video.)_ | `ROADMAP*.md`, `specs/**` | 2026-06-17 |
| 🎨 Design | `design` | _(idle — ✅ **ACTIVATION MOMENT + CASA COMPLETE**: ActivationReveal (cycling-scan loading + staggered priority reveal w/ rationale + evidence chips + Accept/Tweak); ActivationHeroCard (first hero-loop, Make-it-happen, Edge Score reveal); DataConsentScreen (onboarding) + DataConsentToggle (settings). prefers-reduced-motion. Awaiting PM dispatch.)_ | `components/ui/ActivationReveal.tsx`, `components/ui/ActivationHeroCard.tsx`, `components/ui/DataConsentCard.tsx` | 2026-06-18 |
