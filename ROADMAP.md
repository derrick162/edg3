...
| 🔒 Security | `security` | _(✅ **open_loops + performancePct + Privacy audit COMPLETE** (787 green): open_loops table + openLoopQueries (encrypted description); deletion/export/30-day-retention; WhoopSleepDay.performancePct (unblocks Energy); all email-derived PII encrypted; watched_threads retention prune + nightly cron; CASA code done. ⚠️ **PM DECISION:** `facts.statement` plaintext — LLM-distilled email facts not encrypted (see ROADMAP-SECURITY.md). Awaiting PM.)_ | — | 2026-06-16 |
