...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R36 SHIPPED (2273 green) — ready for PM merge.** T1: "Add context" panel (`POST /api/memory/notes` → `extractAndUpsertFacts` + `user_note` fact; Memory-tab card). T2: inbox-review dedup (`lib/gmail.ts` `getPreviousEmailSubjects` + `selectNewInboxSubjects` — only logs new threads; corrected the spec's snapshot to store the full set). ⚠️ **`lib/gmail.ts` Security-owned (additive, PM-dispatched) + Shared `lib/db.ts` `user_note` category — Vijay sync down.** Cam (R24) does the card visual pass after merge. Idle.)_ | `app/api/memory/notes/route.ts`, `app/dashboard/page.tsx`, `lib/gmail.ts`, `lib/db.ts` | 2026-06-24 |
