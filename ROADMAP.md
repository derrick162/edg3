...
| 🔒 Security | `security` | _(✅ **DISPATCH S1–S7 DONE (2391 green) — ready for merge.** S7: bounded-concurrency call dispatch (`MAX_CONCURRENT_CALLS=5` batches) in `checkAndInitiateCalls`; verified fires-all-users + dispatch-lock double-dial guard + `call_attempts` intended/actual/outcome log; +4 tests. (S5 instrumentation hooks in Core files still pending Darren sync down.) Next: S8 (rate-limit hardening).)_ | `lib/scheduler.ts`, `content/security-audit.md` | 2026-06-25 |
