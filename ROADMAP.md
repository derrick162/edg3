...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R9 ALL 7 SHIPPED (1975 green) — ready for PM merge.** T1 voice speed 0.9; T2 silent briefing fallback + priority truncation; T3 spelling-override + lifestyle-pref prompt; T4 getWeather tool (`lib/weather.ts`, Open-Meteo); T5 Focus "X of Y hrs" + specific refresh-error; T6 briefing calls Clarity→Momentum (pts → facts/memory depth); T7 local-day call-count fix + weekend Focus Brief card. Done releasing shared `lib/vapi.ts` (additive prompt/voice only). **⚠️ External (Derrick): create `getWeather` Vapi tool (no params) + paste UUID into `lib/vapi.ts`.** Idle, awaiting next dispatch.)_ | `lib/vapi.ts`, `lib/weather.ts`, `lib/briefing.ts`, `lib/calendarScore.ts`, `components/ui/FocusRecommendationCard.tsx` | 2026-06-20 |
