...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Round 7 P0 — full email body reading for memory SHIPPED (1880 green).** `getRecentEmailSignal({fullBodies})` reads inbound bodies (≤10 non-spam threads, 2000 chars ea, in-memory only); briefing extracts facts from bodies not just snippets; new `isLikelySpam`. ⚠️ **Cross-lane: edited Security's `lib/gmail.ts`** per Esther dispatch — Vijay please review. ⚠️ **Security/PM:** update google-verification.md + privacy page (Edge now reads inbox body text in-memory; never stored). Earlier: multi-account UI (P1), dashboard batch 9/10. Pillars exhausted (gated/blocked remain). ⚠️ Design: ticket 8 + multi-account polish.)_ | `lib/gmail.ts` (cross-lane), `lib/facts.ts`, `lib/briefing.ts`, `lib/emailActivityFilter.ts` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **Synced master (1880 green) — migration fix reconciled.** Prod migration bug fixed in prod + hardened version in `applyMigrations()` (extracted, tested, full-initSchema regression guard). All Security pillars + dispatches done. **Idle — awaiting dispatch.** ⚠️ EXTERNAL: register GMAIL_REDIRECT_URI in Google console + set env. ⚠️ Review Darren's cross-lane edit to `lib/gmail.ts` (Round 7).)_ | `lib/db.ts`, `lib/scheduler.ts`, `app/api/admin/**`, `app/api/auth/**` | 2026-06-18 |
