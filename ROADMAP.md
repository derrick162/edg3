...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Round 8 + P0 bugs SHIPPED (1917 green).** M4-4 social models + Briefing V2 wins (earlier) + **both P0 bugs**: empty focus-card CTA gated; `classifyEvent`/`needsPrepSuggestion` in `lib/eventMatch.ts` → prep only for work-meetings (PRP/gym/lunch no longer get prep). 16 new tests. Awaiting next dispatch.)_ | `components/ui/FocusRecommendationCard.tsx`, `lib/eventMatch.ts`, `lib/calendarPlan.ts`, `lib/vapi.ts` | 2026-06-19 |
