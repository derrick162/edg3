...
| 🔒 Security | `security` | _(✅ **AUDIT LOG COVERAGE + ROUND 4 COMPLETE (1362 green)**: Closed audit_log gaps (priorities_set/accepted, loop_resolve/dismiss/snooze). Full coverage doc in security-audit.md. Backup 20-table verification fixed (milestones bug). POST /api/auth/consent shipped with audit. data_consent migration live. Awaiting PM dispatch.)_ | — | 2026-06-18 |
...
| 🎨 Design | `design` | _(idle — ✅ **Memory moat ROUND 3 COMPLETE**: Goals elevated to numbered anchor cards (indigo #1, accent border); Decision + Commitments/Outcomes placeholders (L4+L7 from spec); all placeholders auto-hide when Core ships data. 1353 green. Awaiting PM dispatch.)_ | `app/dashboard/page.tsx` | 2026-06-18 |
