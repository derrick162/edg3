...
| 🔒 Security | `security` | _(🔨 **R19 T1 SHIPPED, T2 IN PROGRESS.** **T1 — quota-error retry cascade fix (done, 2116 green):** `isQuotaError` guard in `app/api/vapi/webhook/route.ts` — quota errors mark missed but never schedule a retry; transient pipeline/no-answer retries preserved. **T2 — gratitude re-fire loop (in progress):** pre-insert null-item gratitude row before `scheduleOpenCall` so the self-gate fires even if the call hangs up. **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `app/api/vapi/webhook/route.ts`, `lib/proactiveNotifications.ts` | 2026-06-23 |
