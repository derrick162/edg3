...
| 🔒 Security | `security` | _(✅ **ROUND 4 COMPLETE (1200 green)**: LLM-output storage caps — task text `.slice(0, 500)` in both webhook task-extraction paths; missed-promises memory note `.slice(0, 2000)` in `verifyPromises.ts`. All known LLM-extracted content paths to DB now uniformly capped. Awaiting PM dispatch.)_ | `app/api/vapi/webhook/route.ts`, `lib/verifyPromises.ts`, `content/security-audit.md`, `ROADMAP-SECURITY.md` | 2026-06-17 |
