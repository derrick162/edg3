...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R15 ALL 7 SHIPPED (1993 green) — ready for PM merge.** searchEvents, checkConflict, getNextEvents (new pure `lib/calendarQuery.ts`), setEventReminder, blockFocusTime, briefEvent + generateWeeklyReview (Haiku, degrade-safe). +9 tests. R14 (all 6) also shipped + awaiting merge. ⚠️ **External (PM):** create 7 R15 Vapi tools + the 5 R14 tools + createEvent recurrence/attendees params; paste UUIDs into both toolIds arrays (placeholders in `lib/vapi.ts`). R9/R12/R13/R14/R15 all on `core`. Idle, awaiting next dispatch.)_ | `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts`, `lib/calendarQuery.ts` | 2026-06-20 |
