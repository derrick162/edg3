...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R40 shipped; R41 T0+T1 SHIPPED (2300 green); T2/T3 next.** T0: memory date tz fix (`parseDbTimestamp` — UTC parse of SQLite timestamps). T1: Conversation State Engine L3 (`lib/transcriptSignals.ts` `detectTranscriptSignals` + webhook-wired `recordTranscriptSignals` → pattern facts). T2: PILLAR-DAILY-CALL Tier 2. T3: PILLAR-MEMORY M4-3 (≥30-call gate). ⚠️ **Additive to Security webhook — Vijay sync down.**)_ | `lib/factSourceLabel.ts`, `lib/transcriptSignals.ts`, `app/api/vapi/webhook/route.ts`, `app/dashboard/page.tsx` | 2026-06-24 |
