...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12 T1–T5 SHIPPED (1991 green); 🔨 building T6 voice-speed control.** T1 delete-token fresh-flow+log, T2 token-language ban, T3 REPLACE PATTERN, T4 open-call Momentum (`is_open_call`), T5 dashboard auto-refresh. **T6 in progress:** `users.voice_speed` column, POST `/api/profile/voice-speed`, Profile-tab 3-button UI, `SPEED_MAP` in `lib/vapi.ts`.)_ | `lib/vapi.ts`, `lib/db.ts`, `app/api/profile/**`, `app/dashboard/page.tsx` | 2026-06-20 |
| 🔒 Security | `security` | _(✅ **R12 T1 SHIPPED (1990 green); idle/polling.** `getRecentEmailSignal` 24h cache gate + empty fetches write no audit entry. Fixes Activity-tab flooding. **⚠️ Note:** `fullBodies` (briefing) is EXEMPT from cache so body fact-extraction gets fresh data.)_ | `lib/gmail.ts`, `lib/gmail.test.ts` | 2026-06-20 |
