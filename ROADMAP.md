...
| 🔒 Security | `security` | _(✅ **DISPATCH S1+S2+S3 DONE (2373 green) — ready for merge.** S1: tests/e2e/ suite. S2: web push for DEGRADED health. S3: multi-user audit — db.ts query scoping clean (no cross-user leak, documented in security-audit.md), deletion cascade drift-guarded, admin users +`total_facts`, scheduler loops all users; new isolation test. Next: S4 (OWASP sweep).)_ | `app/api/admin/users/route.ts`, `lib/db.ts`, `content/security-audit.md` | 2026-06-24 |
