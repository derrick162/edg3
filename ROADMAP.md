...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R41 T0+T1+T4 SHIPPED (2300 green) — ready for PM merge.** T0: memory date tz fix. T1: Conversation State Engine L3. T4: energy false-confirm fix — Part B (setEnergyLevel tool + energy_log + /api/energy/today) already existed; added Part A prompt guard ("never promise to change the Whoop indicator") + dashboard "(self-reported)" label. T2/T3: assessed → nothing buildable (Daily-Call Tier 2 shipped; M4-3 gated on ≥30 calls). ⚠️ Additive to Security `lib/vapi.ts` + webhook — Vijay sync down. Idle.)_ | `lib/vapi.ts`, `lib/factSourceLabel.ts`, `lib/transcriptSignals.ts`, `app/dashboard/page.tsx`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
