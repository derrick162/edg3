...
| 🔒 Security | `security` | _(🔨 **R19 T3 done (ready for merge), T4 IN PROGRESS.** T1: quota retry fix (merged). T2: gratitude re-fire fix (merged). T3: greeting boundary — evening 5 PM not 6 PM, centralized into `lib/greeting.ts` + 13 tests (committed `5b5b74f`). **T4 [HIGH] in progress:** gratitude completed call blocking morning briefing — adding `(is_open_call IS NULL OR is_open_call = 0)` guard to both `alreadyCalled`/`existing` queries in `lib/scheduler.ts`. **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `lib/scheduler.ts`, `lib/greeting.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
