...
| 🔒 Security | `security` | _(✅ **EMAIL SIGNAL QUALITY FIX + ROUND 5 (1529 green)**: `getRecentEmailSignal` now excludes Promotions/Social/Forums via query filter + label safety-net — stops marketing mail from entering fact extraction/meeting-prep/priority derivation. Round 5: bi-temporal facts + M3 encryption. Awaiting PM merge.)_ | `lib/gmail.ts`, `lib/gmail.test.ts`, `lib/db.ts`, `lib/episodeStore.ts` | 2026-06-18 |
