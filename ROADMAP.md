...
| 🔒 Security | `security` | _(✅ **R21 (T1+T2+T3) + R20 NANP timezone fallback ALL DONE (2196 green) — ready for merge; idle.** R21 T1: explicit `ERROR:` tool-call failure strings. T2: `work_schedule` schema. T3: QA sweep → `qa-log.md`. **R20:** Kevin greenlit the code-level fix — new `lib/phoneTimezone.ts` infers tz from NANP area code in `effectiveTimezone` before the LA default (Toronto 416 → America/Toronto), 15 tests. Core owns the durable browser-detect (R35). ⚠️ Derrick prod one-liner to set tz now (worktree can't reach prod). Pending merge in order: `a5a34fa` → `c27b79b` → `cbe9b2a` → `5fe8a22` → NANP commit.)_ | `lib/phoneTimezone.ts`, `lib/db.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-24 |
