...
| 🔒 Security | `security` | _(✅ **Token revocation + security audit COMPLETE** (739 green): (1) Whoop token revocation on disconnect (REVOKE_URL, clearUserCaches, revokeWhoopAccess + 6 tests) — OAuth hygiene done for both providers (CASA ✅); (2) Security audit of Core's new write paths — `daily_focus` added to deletion routes + data export; all paths confirmed user-scoped + idempotent + consistent encryption policy. Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
