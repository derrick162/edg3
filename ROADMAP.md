...
| 🔒 Security | `security` | _(✅ **All dispatched work verified SHIPPED on master (board correction merged `d63a6b9`); idle/polling.** R8 T2 `setEnergyLevel` (handler `tool-call/route.ts:1224`, Vapi UUID `8aac93a3…` live) + R6 T1 predictive context-loading (`briefing_context_packs` + `runNightlyContextPacks`) + people_models + privacy email-body disclosure all confirmed on master. ➡️ **DECISION for Core — "Gmail-link→no-facts" (routed to me): do NOT add extraction to the OAuth callback.** The one-off pass already exists in `app/api/learned/route.ts` (fire-and-forget `getRecentEmailSignal → extractAndUpsertFactsFromEmail`, guarded on thin-facts `<10` + calendar token; readonly resolves via the PRIMARY account, so a dedicated-Gmail-callback kick would no-op without the calendar token). Gap is trigger *timing*, not capability. **Minimal single-source fix = Core one-liner: call `/api/learned` on the `gmail_connected` postMessage / `?gmail_linked=1` so facts populate immediately post-connect.** No Security change needed.)_ | `ROADMAP.md` | 2026-06-19 |
