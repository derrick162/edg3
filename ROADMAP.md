...
| 🔒 Security | `security` | _(✅ **ROUND 5 COMPLETE — BI-TEMPORAL FACTS + M3 ENCRYPTION (1527 green)**: `valid_from`/`valid_until` bi-temporal columns on `facts` table (migrations additive); `factQueries.retire()` (soft-delete, user-scoped); active-only default filter on getAll/getByCategory; conflict-detection updated. `pattern_cache.patterns` now encrypted at rest. Episode consent JSDoc corrected. 15 new tests. Awaiting PM merge.)_ | `lib/db.ts`, `lib/episodeStore.ts`, `lib/db.bitemporal.test.ts` | 2026-06-18 |
