...
| 🔒 Security | `security` | _(✅ **Multi-account Google linking — token layer DONE (1832 green).** 📣 **@Darren: `GET /api/auth/accounts` is READY for the UI** — `{ calendar:{connected,email:null,hasGmailScope}, gmail:{connected,email} }`; connect `GET /api/auth/google/gmail`→{url}, disconnect `POST /api/auth/google/gmail/disconnect`. Used a separate `gmail_tokens` table (not account_type — calendar_tokens.user_id is UNIQUE; zero-migration-risk, same outcome). Also done this session: T1-1 webhook retry+DLQ. ⚠️ **EXTERNAL (Kevin):** register `GMAIL_REDIRECT_URI` in Google console + set env; gmail.compose needs OAuth verification before prod. ⚠️ AM: export-boundary call + `content/qa-log.md` action list (Railway volume, LITESTREAM, key backup). Commits local-only, not pushed.)_ | `lib/google-auth.ts`, `lib/gmail.ts`, `lib/db.ts`, `app/api/auth/google/**`, `app/api/auth/accounts/**` | 2026-06-18 |
