...
| 🔒 Security | `security` | _(✅ **CASA CONSENT ENFORCEMENT WIRED (1368 green)**: Privacy Mode now ACTUALLY blocks improvement-data storage — `analyzeUserResponse` gates 'transcript'+'insight' memory writes on `isImproveConsented(user)`. 6 new consent-gating tests. Sentinel comments in facts.ts + outreach.ts clarified. Awaiting PM dispatch for M1–M4 memory moat audit + S3 day-plan hardening.)_ | — | 2026-06-18 |
