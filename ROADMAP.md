...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R17 SHIPPED (2052 green) — ready for PM merge.** T1 sharper opener: `isRoutineEvent` helper in briefing.ts + GREETING prompt. T2 post-call feedback: additive `call_feedback` table + `callFeedbackQueries` on shared `lib/db.ts` (Vijay FYI — merged fast), new `callFeedback` rate-limit key, `POST /api/briefing/feedback`, ★★★★★ row on expanded Call Summary card. Released `lib/db.ts` claim. Moving to pillar work (Daily Call → Memory → Trust).)_ | `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `lib/rateLimit.ts`, `app/api/briefing/feedback/**`, `app/dashboard/page.tsx` | 2026-06-20 |
