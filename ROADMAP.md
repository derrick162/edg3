...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **Hero Loop (queue #3)** — `lib/calendarPlan.ts` + `applyCalendarPlan` tool: batch-reshape TODAY in one confirm + plan-level undo + re-score. Building now.)_ | `lib/calendarPlan.ts`, `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts` | 2026-06-15 |
