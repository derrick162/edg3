...
| 🔧 PM/CoS | `master` | _(✅ **R11 dispatched to Design. R9 dispatched to Core (Darren). 🚨 DERRICK — one pending action:** Railway persistent volume at `/data` + Litestream env vars + `getWeather` Vapi tool.)_ | `ROADMAP*.md` | 2026-06-20 |
| 🎨 Design | `design` | _(✅ **R11 T1+T2+T3 SHIPPED.** Focus Score hours subtitle, Weekend Brief card, Activity dedup ×N — 1958 green. Idle, awaiting next dispatch.)_ | `components/ui/EdgeScoreCard.tsx`, `components/ui/FocusRecommendationCard.tsx`, `lib/activityLabels.ts`, `app/dashboard/page.tsx` | 2026-06-20 |
