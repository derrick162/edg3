...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R36 + R37 + R38 ALL SHIPPED (2286 green) — ready for PM merge.** R37 (M4-4): `updatePeopleModels` write path (webhook, fire-and-forget) activates the existing dormant `PEOPLE CONTEXT` briefing block + vapi note. R38: `(unknown)`-entity resolution pass in consolidation + pure event-as-entity guard (`looksLikeEventEntity`/`reassignEventEntityFacts`). ⚠️ **Additive to webhook + `lib/vapi.ts` (Security) — Vijay sync down.** Idle.)_ | `lib/peopleModels.ts`, `lib/facts.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
