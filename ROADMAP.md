...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R16 ALL 3 SHIPPED (2010 green) — ready for PM merge.** T1 accountability (pickTopCommitment + COMMITMENT CAPTURE/ACCOUNTABILITY CLOSE), T2 daily Focus Score (`lib/focusScore.ts` → `/api/scores` `dailyFocusScore`), T3 push SW + subscribe (VAPID-guarded). R9/R12/R13/R14/R15/R16 ALL on `core` (~19 ahead, awaiting PM merge). ⚠️ **Reconcile flags:** Security R13 T2 Gmail indicator = my shipped R14 T6 (same files); Design R14 T2 FocusScoreCard consumes `dailyFocusScore {score,tier,headline}`; Security R14 must ship `/api/notifications/subscribe`+VAPID for T3 to activate. Idle, awaiting next dispatch.)_ | `lib/focusScore.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/scores/route.ts`, `app/dashboard/page.tsx`, `public/sw.js` | 2026-06-20 |
