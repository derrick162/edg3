...
| 🔒 Security | `security` | _(✅ **R12 T1 SHIPPED (1990 green); idle/polling.** `getRecentEmailSignal` 24h cache gate (returns cached subjects from the last `email_signal_fetch` receipt if < 24h old — no Gmail call, no new audit row) + empty fetches write no receipt. Fixes Activity-tab flooding. **⚠️ Guarded beyond spec:** `fullBodies` (briefing) is EXEMPT from the cache so Round-7 body fact-extraction still gets fresh bodies — flagged to Kevin. 5 tests. **⚠️ EXTERNAL (Derrick) still open:** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, restore drill.)_ | `lib/gmail.ts`, `lib/gmail.test.ts` | 2026-06-20 |
