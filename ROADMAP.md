...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12 ALL 5 SHIPPED (1991 green) — ready for PM merge.** T1 delete-token fresh-flow + mismatch log; T2 ban token language from voice; T3 REPLACE PATTERN (replace at deleted event's time); T4 Momentum counts open calls (`is_open_call` column + 50/20/30 weighting); T5 dashboard `visibilitychange` auto-refresh + celebrate. ⚠️ **Done with shared files** `lib/db.ts` (additive column) + 1 line in Security's `lib/scheduler.ts::scheduleOpenCall` (Vijay FYI — released). Idle, awaiting next dispatch.)_ | `lib/vapi.ts`, `lib/calendarScore.ts`, `lib/db.ts`, `lib/scheduler.ts`, `app/dashboard/page.tsx` | 2026-06-20 |
