...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R18 T1 SHIPPED (2066 green) — ready for PM merge; ⚠️ T2 BLOCKED.** T1: `findNextFreeSlot` in `lib/calendar.ts` + `PRIORITY BLOCKING SLOT` injection in `lib/briefing.ts` + `lib/vapi.ts` guidance — Edge now names an exact day+time slot for under-scheduled priorities (6 tests). **T2 NOT BUILT — needs Kevin's decision:** it depends on `checkOutreachReplies`/`formatRepliesForVoice`/`outreach_tracking`, all DELETED in R12 T7 (email-feature removal). Building it resurrects the removed feature — won't reverse that decision autonomously. Confirm: revive email outreach, or drop T2 as obsolete? Details in ROADMAP-CORE changelog.)_ | `lib/calendar.ts`, `lib/briefing.ts`, `lib/vapi.ts` | 2026-06-21 |
