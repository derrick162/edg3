...
| 🔒 Security | `security` | _(✅ **H3+M7+CSP+FAQ§3+backups COMPLETE** (989 green): H3 OAuth CSRF state (oauth_state table, connect+callback routes, atomic consume); M7 JWT session revocation (session_version + logout invalidation, legacy tokens grandfathered); CSP nonce (proxy.ts, Next.js 16 Proxy API, script-src nonce+strict-dynamic); FAQ §3 encryption claims corrected; backup table list extended. All 5 dispatched items shipped. Awaiting next PM dispatch.)_ | — | 2026-06-16 |
