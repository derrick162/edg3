...
| 🔒 Security | `security` | _(✅ **R19 FULLY SHIPPED (2120 green); idle/polling.** **T1 — quota-error retry cascade fix:** `isQuotaError` guard in `app/api/vapi/webhook/route.ts` — quota errors mark missed but never schedule a retry; transient pipeline/no-answer retries preserved. **T2 — gratitude re-fire loop:** pre-insert null-item gratitude row before `scheduleOpenCall` in `lib/proactiveNotifications.ts` so the self-gate fires even if the call hangs up; hardened `gratitudeQueries.getByDate` with `id DESC` tiebreak (shared `lib/db.ts`, additive — Core FYI). New `gratitude-autocall.test.ts` (4 cases). **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `app/api/vapi/webhook/route.ts`, `lib/proactiveNotifications.ts`, `lib/db.ts` | 2026-06-23 |
| 🔧 PM/CoS | `master` | _(📋 **Design R22+R23 dispatched. Core R24 dispatched. Security R19 dispatched (T1: quota retry cascade fix + T2: gratitude re-call fix).**  R23 (Design, do first): sidebar cleanup + sparkline fix. R22: briefings date grouping + score trust signals. R24 (Core): onboarding account linking. R19 (Security): quota retry cascade + gratitude re-fire loop.)_ | `ROADMAP.md`, `ROADMAP-CORE.md`, `ROADMAP-SECURITY.md`, `ROADMAP-DESIGN.md` | 2026-06-23 |
