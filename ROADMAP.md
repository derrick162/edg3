...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **ACTIVATION MOMENT INCREMENT 1 COMPLETE (1240 green)**: Replaced manual priorities onboarding step with `ActivationStep` — immediately runs `derivePriorities` on mount, shows `PriorityDerivationCard` with rationale + evidence, accept/tweak/dismiss flow, graceful thin-data fallback. `app/onboarding/page.tsx`. Awaiting PM merge + Design reveal polish (Cam).)_ | `app/onboarding/page.tsx` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **ROUND 3 COMPLETE (1200 green)**: (1) 8 more rate-limited routes — 36 total types (suggest-priorities, account delete/export, onboarding profile/call-time/priorities, keep, profile update). (2) Error leak fixes — `err.message`/raw strings removed from 4 user-facing routes. (3) Input caps — profile_summary 2000 chars, rememberPreference 500 chars. (4) Post-merge tsc fix. Awaiting PM dispatch.)_ | `lib/rateLimit.ts`, `app/api/**`, `content/security-audit.md`, `ROADMAP-SECURITY.md` | 2026-06-17 |
| 🔧 PM | `master` | _(✅ **NIGHT-QUEUE flagships integrating → `45a4c35` (1156 green, deployed).** Integrated: Core FLAGSHIP — Proactive Priority Derivation (`lib/priorityDerivation.ts` + `/api/priorities/derive` + accept) + T3 grounding on live voice path; Security FLAGSHIP — full pre-beta audit of all 78 `app/api/**` routes (10 new rate-limit keys, input-validation fixes, `content/security-audit.md`); Cam FLAGSHIP — full mobile pass + a11y/touch-targets + loading skeletons; Esther — content avalanche. Resolved rateLimit key collision (kept both lanes' keys); cleared stale .next cache. Tonight's fixes (one Edge Score, graceful receipts) verified intact through all merges. Continuing overnight loop to ~9am.)_ | `ROADMAP*.md`, `specs/**` | 2026-06-17 |
| 🎨 Design | `design` | _(idle — ✅ **ACTIVATION MOMENT COMPLETE**: `ActivationReveal` — cycling-scan loading state + staggered priority reveal one-by-one with rationale + evidence chips + Accept/Tweak CTAs. `ActivationHeroCard` — first hero-loop card with "here's what I'd change today", Make it happen CTA, Edge Score reveal post-apply; aligned variant. Onboarding wired: two hidden steps activation+hero between calendar and priorities; thin-data fallback; prefers-reduced-motion via globals.css. 1200 green. Awaiting PM dispatch.)_ | `app/onboarding/page.tsx`, `components/ui/ActivationReveal.tsx`, `components/ui/ActivationHeroCard.tsx` | 2026-06-17 |
