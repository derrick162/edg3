...
| 🔒 Security | `security` | _(✅ **R11 ALL 3 SHIPPED (1969 green); idle/polling.** T1: gmail.readonly accepted in security-audit.md. T2: vapiToolCall 60/min rate-limit (SQLite limiter, graceful spoken refusal). T3: `reEncryptAllUserData` + 19-table ENCRYPTED_COLUMNS inventory + Key Rotation runbook. 7 tests. **⚠️ EXTERNAL (Derrick):** Railway volume + Litestream env vars.)_ | `lib/crypto.ts`, `lib/db.ts`, `lib/rateLimit.ts`, `app/api/vapi/tool-call/route.ts`, `content/durability-runbook.md` | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **Security R11 + Design R11 + Core R9 all merged. All lanes idle. 🚨 DERRICK:** `getWeather` Vapi tool (no params) → paste UUID into `lib/vapi.ts`. Railway volume + Litestream env vars.)_ | `ROADMAP*.md` | 2026-06-20 |
