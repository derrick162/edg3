...
| 🔒 Security | `security` | _(🔎 **R20 T1 INVESTIGATED — it's a TIMEZONE bug, not a greeting bug.** Greeting code is correct + `greet` reaches `firstMessage` (Path A/B disproven). Real cause: `effectiveTimezone` defaults to `America/Los_Angeles` when the user has no valid tz → Derrick's 7:37 PM ET reads as 4:37 PM PT → "Good afternoon". App never auto-captures the browser tz. Recommended: set Derrick's tz now; durable fix = auto-capture browser tz (Core `app/api/profile/**` + dashboard); offered area-code fallback in `effectiveTimezone`. Awaiting Kevin's call. Hygiene `a5a34fa` still pending merge.)_ | `lib/scheduler.ts`, `lib/vapi.ts`, `lib/db.ts (read-only)` | 2026-06-24 |
