...
| 🔧 PM/CoS | `master` | _(📋 **R23 (Core) + R18 (Security) + R21 (Design) shipped. R19 (Security) dispatched 2026-06-23 — quota-error retry cascade fix (30m ticket). Hotfixes shipped to master: transcript display (scrollable box + skip duplicate first AI bubble). ⚠️ Derrick (external): inbound webhook URL set ✓; `recordGratitude` UUID wired ✓.)_ | `ROADMAP.md`, `ROADMAP-CORE.md`, `ROADMAP-SECURITY.md`, `ROADMAP-DESIGN.md` | 2026-06-23 |
| 🎨 Design | `design` | _(✅ **R21 SHIPPED (2094 green).** Inbound badge: --edg-amber-20/08 tokens + .badge-inbound class in globals.css; is_inbound field + badge priority logic in dashboard. All dispatch exhausted. Idle.)_ | `app/globals.css`, `app/dashboard/page.tsx` | 2026-06-22 |
