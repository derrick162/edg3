...
| 🔒 Security | `security` | _(🌙 **OVERNIGHT — Trust Tier 0+1 done + bug fixes + reliability hardening, 14 commits, 1708 green.** Shipped: T0-1 durability self-check + **automated restore drill**, T0-4 scheduler lock (+contention warning), T0-2 startup key check, M2-4 empty-pack guard, verifyBackup derived from canonical table list. **🐞 Fixed account-deletion FK-500 bug** (`support_messages`+`fact_history`) + drift-guard test (verified every `REFERENCES users(id)` is via `user_id`). **🐞 T3-3: activity log + relationship-memory (people) were MISSING from data export** — both added (no encrypted leaks). ⚠️ **5-min morning call needed: confirm export boundary** — episodes/fact_history/focus_milestones/support_messages deferred (need decryption + per-table judgment); see ROADMAP-SECURITY changelog. Esther dispatches all addressed/verified. **Idle/looping — re-checking board for new PM direction.** ⚠️ **Derrick/Kevin AM action list at top of `content/qa-log.md`** (Railway volume, LITESTREAM env, key backup, live restore drill, post-deploy `[durability]` logs). T0-3 e2e remains Core/Darren. Commits local-only, not pushed.)_ | `app/api/account/export/route.ts`, `lib/backup.ts`, `lib/db.ts`, `lib/durability.ts`, `content/qa-log.md` | 2026-06-18 |
