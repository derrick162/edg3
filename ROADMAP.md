...
| 🔒 Security | `security` | _(✅ **R9 T1 SHIPPED (1958 green) — full email body hardened + Gmail-read audit gap closed.** `getRecentEmailSignal({fullBodies})` already existed (R7); added `truncateAtSentenceBoundary` + 7 tests. Closed audit gap: `extractGmailAccountContacts` now emits encrypted `gmail_contacts_fetch` receipt. `content/security-audit.md` updated. **⚠️ EXTERNAL (Derrick):** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, live restore drill. Idle/polling.)_ | `lib/gmail.ts`, `lib/gmail.test.ts`, `content/security-audit.md` | 2026-06-20 |
