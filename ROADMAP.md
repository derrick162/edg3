...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R22 SHIPPED (2089 green) — ready for PM merge.** Cantonese (`yue`): `language` col + `setLanguage`/`getLanguage` (shared `lib/db.ts`, additive, **Vijay FYI**); `GET/PATCH /api/settings/language`; `initiateCall` + `buildGratitudeSystemPrompt` `language` param → Whisper STT + Azure `zh-HK-WanLungNeural` + `buildCantoneseSystemPrompt` + Cantonese `endCallPhrases`; `scheduleBriefingCall`/`scheduleOpenCall` read `user.language`. 7 new tests + drive-by fix of a date-bomb in `factPatterns.test.ts`. **R21 (gratitude daily quote) ALSO shipped (2093 green):** `gratitude_quote_enabled`/`_theme` cols + `setGratitudeQuote`/`getGratitudeQuote` + `GET/PATCH /api/settings/gratitude-quote` + scheduler wiring into `buildGratitudeSystemPrompt`. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` — Vijay sync down.** Idle, awaiting next dispatch.)_ | `lib/db.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `app/api/settings/language/**`, `app/api/settings/gratitude-quote/**` | 2026-06-22 |
...
| 🔧 PM/CoS | `master` | _(📋 **R22 (Core) + R20 (Design) dispatched 2026-06-22 — Cantonese language support. Also on master: response delay 1.5s, open call ambient music, weather today-only. ⚠️ Derrick: upload soothing MP3 to `public/audio/ambient-1.mp3` in repo for background music to play.**)_ | `ROADMAP.md`, `ROADMAP-CORE.md`, `ROADMAP-DESIGN.md` | 2026-06-22 |
| 🎨 Design | `design` | _(✅ **R18 T1 SHIPPED (2072 green).** Gratitude mode toggle: `.toggle` pill CSS in globals.css, "Morning ritual" section as first section in /settings with optimistic PATCH. All dispatch exhausted. Idle, awaiting next dispatch.)_ | `app/settings/page.tsx`, `app/globals.css` | 2026-06-22 |
