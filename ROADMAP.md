...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Pillars exhausted + QA pass (1869 green).** Shipped: multi-account Google UI (P1), dashboard batch 9/10, +route tests for priority-history & fact-rollback. **Re-audited both pillars: all DC0-DC3 + M1-M4-2/M4-3b LIVE.** Only remaining are **gated/blocked**, not skipped: DC4 (50-user gate), M4-3 (30+-call gate), M4-4 (people-data block), M2-3#4 (no overwhelm signal). ⚠️ For Design: ticket 8 (dup priority sections) + multi-account look polish. **Core idle — awaiting next dispatch or an unblock.**)_ | `app/dashboard/page.tsx`, `app/api/priorities/history/**`, `app/api/memory/facts/**` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **Synced master (1880 green) — migration fix reconciled.** Prod migration bug already fixed in prod (`fbc496f`, per PM); my branch carries the cleaner version (`applyMigrations()` extracted + exported + tested in `lib/db-migrations.test.ts` + deduped + full-initSchema regression guard). Merge kept my tested version **and** master's `oauth_state` gmail-CHECK migration. All Security pillars + dispatches done: multi-account Google token layer, Round 6 T1 (11pm user-local), T1-1 webhook retry+DLQ, admin/facts SQL hardening. **Idle — awaiting dispatch.** ⚠️ EXTERNAL: register GMAIL_REDIRECT_URI in Google console + set env. ⚠️ AM: export-boundary call + `content/qa-log.md` action list.)_ | `lib/db.ts`, `lib/scheduler.ts`, `app/api/admin/**`, `app/api/auth/**` | 2026-06-18 |
| 🔧 PM/CoS | `master` | _(✅ **June 18 PM session shipped (1870 green, all pushed):** Gmail ingest UX (post-connect banner → Memory tab), Activity UTC fix, TODAY'S FOCUS card copy fix, fallback briefing fix (filter past events, strip brackets, 45s timeout), copy button name correction. **Bug fix:** Gmail ingest was writing `'people'` category — corrected to `'person'` (CHECK constraint). **Facts in briefing fixed:** full briefing now injects `WHAT EDGE KNOWS` block so goals/projects/people surface. **Esther's items:** (1) Migration runner already deployed — `valid_until`/`valid_from` auto-applied on Railway restart, no manual SQL needed. (2) Facts block added to briefing prompt. (3) Migration fix was `fbc496f`, already in prod. (4) ⚠️ Darren: confirm multi-account UI shows linked Gmail in sidebar + full email body reading ETA? **⚠️ Derrick action:** disconnect + reconnect secondary Gmail account (needs new `gmail.readonly` scope) — then ingestion will pull contacts into Memory.)_ | `ROADMAP*.md`, `lib/briefing.ts`, `lib/gmail.ts`, `app/api/auth/google/gmail/ingest/route.ts`, `app/dashboard/page.tsx`, `components/ui/FocusRecommendationCard.tsx` | 2026-06-19 |
| 🎨 Design | `design` | _(✅ **Derrick review batch (tickets 1-4, 6) shipped**: removed stale email footer copy, "Detail — Expand" redundant line, trend arrow tooltips, milestone inline edit (pencil icon + PATCH title), "+ N more threads" as styled button. Aria voice fix (eleven_flash_v2 + speed:1.2). Ticket 5 (Today's Focus note) blocked on Darren data shape. 1803 green.)_ | `app/dashboard/page.tsx`, `lib/activityLabels.ts`, `app/api/milestones/[id]/route.ts`, `lib/db.ts`, `lib/vapi.ts` | 2026-06-18 |
