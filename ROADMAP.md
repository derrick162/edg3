...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Full-week alignment fix COMPLETE (989 green)**: `getFullWeekEvents` (Mon→Sun in user tz) replaces future-only `getWeekEvents` for `computeAlignment` + hygiene flags. Completed gym/meetings now count toward per-priority hours. Edge Score focus% also fixed. Awaiting PM dispatch.)_ | — | 2026-06-16 |
| 🔒 Security | `security` | _(✅ **CSP decision documented; new Core routes audited (all clean)**. Strict nonce parked — `'self' 'unsafe-inline'` is the right pre-beta baseline (cross-origin scripts blocked; strict-dynamic needs local `next build && next start` verify before re-attempting). New routes `/api/scores`, `/api/focus/recommend`, `app/page.tsx` all auth-gated + user-scoped + rate-limited. ⚠️ Note for Core: `/api/waitlist` route missing — landing page form will 404. Awaiting next PM dispatch.)_ | — | 2026-06-16 |
| 🔧 PM | `master` | _(✅ **Night-3 fully integrated → `788791b` (989 green, build clean, pushed).** Merged 3 lanes: Core call-quality batch (`8c9269b` — 5 morning-call fixes) + gym trust-bug fix (`ba40bbb`); Security encryption (`03e117d` facts.statement+dailyFocus) + H3 CSRF/M7 session-revocation/CSP-nonce (`824437b`); Design landing-page rebuild + Whoop Intelligence + TimeAllocationViz + mobile (`5e44a16`–`6ad6b0c`). All lanes level with master, idle. Railway auto-deploying. Holding for Derrick's decisions (pricing, design partners, Vapi-key rotation) + Esther's launch playbook.)_ | `ROADMAP*.md`, `specs/**` | 2026-06-16 |
