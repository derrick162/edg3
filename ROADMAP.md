...
| 🔒 Security | `security` | _(🔨 **R19 T3+T4+T6 DONE; T5 IN PROGRESS (dep landed).** T3: greeting boundary 5 PM (`5b5b74f`). T4 [HIGH]: gratitude/open call no longer suppresses morning briefing — `is_open_call` guard + `findTodaysBlockingBriefing` helper (`39e83a2`). T6: score-change notif uses `getPrior` — `lib/notifications.ts` + 5 tests (`1d2e4ef`). **T5 (nightly score cron):** Core's `computeAndSaveScore` now on master → wiring the `'0 23 * * *'` cron in `startScheduler()`. **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `lib/scheduler.ts`, `lib/greeting.ts`, `lib/notifications.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
