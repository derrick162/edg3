...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(⚠️ **DISPATCH QUEUE EXHAUSTED — all of R8 (P0 bugs + "Briefing V2 quick wins" + R7 email-body) verified ALREADY SHIPPED on master `596d3b3`; not rebuilt.** Evidence: in-code tags `briefing.ts:1194 "(Round 8 win 1)"`, `briefing.ts:1197 "(Round 8 win 2)"`, personal-warmth `:1189`, recovery-name `:862`; `eventMatch.ts:38/48`+`calendarPlan.ts:215`; M4-4 `facts.ts:746`+`briefing.ts:796`. Pillars have **no ungated Core item left**: M4-3 gated (30+ calls), DC4-* gated (50 users), M4-4 done (doc stale), UX-4/T3-1 are ongoing-audit mandates. **Kevin/Derrick: need NET-NEW direction, or authorize a Memory-pipeline quality/bug-hunt sweep (UX-4).** 1951 green. Idle.)_ | _(none — verification only)_ | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **R10 ALL 4 TICKETS SHIPPED (1959 green); idle/polling.** **T1:** data export now complete — added `episodes`/`factHistory`/`focusMilestones`/`supportMessages` (safeDecrypt, `version:'3'`) +tests. **T2:** Gmail second-flow review — CSRF/RL/audit verified; `gmail.readonly` on dedicated flow is REQUIRED by contact ingest (not a regression) — **kept it, flagged keep-vs-tighten decision to Kevin** in `security-audit.md`. **T3:** fixed contradictory Gmail body-reading claims (`privacy/page.tsx:110` + 4 in `google-verification.md`) — CASA-relevant; calendar write-scope already accurate. **T4:** QA sweep logged (`qa-log.md`) — e2e 19/19, export-completeness item closed; live-call/Railway items still ⚠️ external. **⚠️ EXTERNAL (Derrick):** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, restore drill.)_ | `app/api/account/export/route.ts`, `app/privacy/page.tsx`, `specs/google-verification.md`, `content/security-audit.md` | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **1945 green + all status board corrections merged, master at `dbaf35f`+, Railway deploying.** Operating model live. **🚨 DERRICK — one pending action:** Railway persistent volume at `/data` + Litestream env vars (`LITESTREAM_S3_BUCKET/ACCESS_KEY_ID/SECRET_ACCESS_KEY`). Runbook: `content/durability-runbook.md`. (Security + Design dispatches resolved — all work confirmed shipped.))_ | `ROADMAP*.md` | 2026-06-19 |
| 🎨 Design | `design` | _(✅ **R9 T1 Activation Moment copy polish SHIPPED (1951 green).** Screen 4 header + CTA corrected to Esther's spec. Screen 6 default 07:30. Screen 7 banner now shows next-call day + time. Screens 2, 3, 3b, 5, 5b confirmed matching spec — no changes needed. Idle — awaiting next R9 dispatch.)_ | `app/onboarding/page.tsx`, `app/dashboard/page.tsx` | 2026-06-19 |
