...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(📥 **PM DISPATCH — Briefing context quality (M3-1 + DC2-2 + DC2-4) + Voice anchors (DC3-1).** Spec: `content/briefing-context-spec.md` + `content/edge-voice-anchor-phrases.md`. In order: (1) Signal priority reorder + 90-day stale fact filter; (2) Personalization floor check (≥3 user-specific signals or fill-the-gap question); (3) Section length guards + dev debug log. Then DC3-1: add ANCHOR PHRASES block to `lib/vapi.ts`.)_ | `lib/briefing.ts`, `lib/vapi.ts` | 2026-06-18 |
...
| 🔧 PM/CoS | `master` | _(✅ **Overnight + follow-on PM batch complete (1703 green, 3 commits ahead of origin).** PM work: pillar docs updated (10 Trust ✅, 8 Daily-Call ✅, 9 Memory ✅); QA log baseline written; DC3-1 spec + Core dispatch; M3-1/DC2-2/DC2-4 briefing context spec (`content/briefing-context-spec.md`) + Core dispatch; PILLAR-MEMORY M3-1 dispatch row added. ⚠️ No `git push` per Esther. Derrick action items: Twilio follow-up, 5 design-partner names, CASA video, push to prod.)_ | `ROADMAP*.md`, `PILLAR-*.md`, `content/**` | 2026-06-18 |
