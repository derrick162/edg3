...
| 🔒 Security | `security` | _(✅ **R19 T3+T4+T6 DONE (2148 green) — ready for PM merge. T5 BLOCKED on Core.** T3: greeting boundary 5 PM (`5b5b74f`). T4 [HIGH]: gratitude/open call no longer suppresses morning briefing — `is_open_call` guard + tested `findTodaysBlockingBriefing` helper (`39e83a2`). T6: score-change notif now uses `getPrior` (fires across dashboard-gap days) — `lib/notifications.ts` + 5 tests. **T5 (nightly score cron):** waiting on Core's `computeAndSaveScore` export from `lib/scores.ts` (not on master; missing-module dynamic import breaks build) — will wire the moment it lands. ⚠️ T6 touched `lib/notifications.ts` (also in Core R25 T7) — surgical lookup swap, flag for merge order. **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `lib/scheduler.ts`, `lib/greeting.ts`, `lib/notifications.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
