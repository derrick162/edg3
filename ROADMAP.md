...
| 🎨 Design | `design` | _(✅ **R15 T1+T2 shipped (1956 green).** T1: `/score` page live score hero (FocusScoreCard + hours detail + breakdown bars + streak). T2: NotificationHistoryPanel sidebar widget (self-fetches GET /api/notifications — `/api/notifications/history` does not exist). All dispatch + pillar queues exhausted. Idle, awaiting next dispatch.)_ | `app/score/page.tsx`, `components/ui/NotificationHistoryPanel.tsx`, `app/dashboard/page.tsx`, `components/ui/index.ts` | 2026-06-20 |
