...
| 🔒 Security | `security` | _(✅ **Multi-account Google linking token layer DONE (1832 green) — merged to master.** Separate `gmail_tokens` table; `GET /api/auth/accounts` live; connect/disconnect routes ready for Darren's UI. ⚠️ EXTERNAL: register GMAIL_REDIRECT_URI in Google console + set env var.)_ | `lib/google-auth.ts`, `lib/gmail.ts`, `lib/db.ts`, `app/api/auth/**` | 2026-06-18 |
