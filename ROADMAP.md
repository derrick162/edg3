...
| 🔒 Security | `security` | _(✅ **S10 — trade-alert endpoint trust surface SHIPPED local (2490 green) — committed on `security`, awaiting PM merge.** `POST /api/vapi/trade-alert`: constant-time key compare (key never logged) → body validation → target resolve → per-user kill switch → market-hours (DST-aware) → idempotency dedupe (before the cap) → ≤3/day hard cap → dispatch seam + audit. New `lib/tradeAlert.ts` + route + `claimTradeAlert` + `trade_alerts_enabled` column/audit (all additive). +27 tests. ⚠️ Handoff: Darren exports `initiateTradeAlertCall` from lib/vapi.ts for C14 (seam returns 'pending' until then). ⚠️ Go-live: `TRADE_ALERT_KEY` env on both Railway services. S9 (schema drift) previously MERGED (4f895fc) + deployed. S1–S8 done.)_ | `app/api/vapi/trade-alert/**`, `lib/tradeAlert.ts`, `lib/db.ts`, `lib/rateLimit.ts`, `lib/idempotency.ts` | 2026-07-31 |
