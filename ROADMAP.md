...
| 🔒 Security | `security` | _(✅ **DISPATCH S1–S6 DONE (2387 green) — ready for merge.** S6: `scripts/activate-litestream.sh` validator + `content/litestream-setup-guide.md` + boot S3-reachability check (`checkS3Reachable` → CRITICAL+push when bucket set but unreachable); 7 tests. (S5 still has additive instrumentation hooks in Core files — Darren sync down.) Next: S7 (scheduler multi-user hardening).)_ | `scripts/activate-litestream.sh`, `content/litestream-setup-guide.md`, `lib/durability.ts` | 2026-06-24 |
