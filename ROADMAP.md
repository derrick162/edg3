...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Memory-tab P0 + Esther dispatch SHIPPED (1945 green).** ➡️ **NEW Core ticket (from Vijay):** Gmail-link→no-facts gap = one-liner — call `/api/learned` on `gmail_connected` postMessage / `?gmail_linked=1` so facts populate immediately post-connect. (Existing fire-and-forget in `app/api/learned/route.ts`; no Security change needed.) **Next: Round 7 full email body reading → Gmail-link one-liner → DC3-1 voice anchors.**)_ | `lib/selfName.ts`, `lib/factGuards.ts`, `lib/facts.ts` | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **Pillar exhaustion sweep DONE (Kevin dispatch) — all three pillars' Security surfaces SHIPPED; idle/polling. Preflight green (1945).** Reconciled stale pillar markers that caused re-dispatch churn: TRUST T0-1/T0-4/T1-1 marked shipped w/ refs, T1-2 Security side resolved (declined redundant `call_health_events` — daily digest covers it); DAILY-CALL DC1/DC3-2 all ✅ (DC4 Phase-2-gated); MEMORY M4-4 `people_models` schema shipped → Darren unblocked. QA refresh logged in `content/qa-log.md`. **Only open items are ⚠️ EXTERNAL (Derrick/Kevin):** Railway persistent volume + `LITESTREAM_S3_*` activation, `DATA_ENCRYPTION_KEY` backup, live restore drill, live-call/Vapi/Whoop QA. Code shipped & waiting. No new Security code work remains — awaiting dispatch.)_ | `PILLAR-TRUST.md`, `PILLAR-MEMORY.md`, `content/qa-log.md` | 2026-06-19 |
