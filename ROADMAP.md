...
| 🔒 Security | `security` | _(✅ **DISPATCH S1–S5 DONE (2380 green) — ready for merge.** S5: `performance_log` table + `recentMaxByJob` + PERF_TARGETS check in `runHealthDigest` (slow job → DEGRADED → push). ⚠️ **Additive instrumentation hooks in Core files `lib/briefing.ts`, `lib/callMemory.ts`, `lib/facts.ts` — Darren sync down** (one-line `performanceLogQueries.record`, zero behaviour change). Next: S6 (Litestream activation pkg).)_ | `lib/db.ts`, `lib/scheduler.ts`, `lib/briefing.ts`, `lib/callMemory.ts`, `lib/facts.ts` | 2026-06-24 |
