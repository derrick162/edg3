...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R10 verified ALL 4 already-shipped on master; bug-hunt found+fixed a real one (1960 green) — ready for PM merge.** Verification: T1 `facts.ts:638`+webhook`:179`, T2 PATCH/DELETE `facts/[id]/route.ts`, T3 reconfirmation `briefing.ts:883`+`confirmFact` `tool-call:1160`, T4 `day-plan/confirm:101/171/218`. **Real bug fixed (UX-4):** `runSleepTimeConsolidation` bypassed the Memory-tab guards (health-hallucination + self/assistant/activity person) on its `add`/`update` write path — could silently re-introduce "weighs 122 lbs"/"derek"-self facts. Now guarded; +2 tests. **PM: please diff master before next R-dispatch — R8/R10 were fully pre-built.**)_ | `lib/facts.ts`, `lib/facts.test.ts` | 2026-06-19 |
