...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R25 T1–T4 SHIPPED (2133 green); T5+T6 IN PROGRESS.** T1: off-calendar people. T2: gratitude UX. T3: alignment all-day filter. T4: `lib/scores.ts` nightly score cron. T5: open/gratitude call source label. T6: goal re-learn preserves oldest learned date. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` — Vijay sync down.**)_ | `lib/briefing.ts`, `lib/scheduler.ts`, `lib/vapi.ts`, `lib/alignment.ts`, `lib/scores.ts`, `lib/facts.ts`, `app/api/memory/route.ts` | 2026-06-24 |
