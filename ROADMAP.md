...
| 🔒 Security | `security` | _(✅ **BACKUP COVERAGE + CONSENT ROUTE + DATA_CONSENT MIGRATION (1340 green)**: Fixed backup milestones bug + added 5 new tables to verifyBackup (20 total). POST /api/auth/consent — auth + rate-limit + audit log, 7 tests. data_consent column now migrates automatically on startup (no Core deploy needed). Awaiting PM dispatch.)_ | — | 2026-06-18 |
