...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R29 Part D SHIPPED (2233 green) — ready for PM merge** (R28–R34 also awaiting merge). Structured grounding contract: `currentOpenCallMemoryText` now labelled ALL-CAPS sections (PEOPLE/GOALS/PREFERENCES/OPEN COMMITMENTS/PROJECTS/OTHER/CONSTRAINTS), one item per line, person facts on one line, ~600-char budget. `currentPreferencesText` already removed (R25 T1) so this covers both call types. Idle.)_ | `lib/callMemory.ts` | 2026-06-24 |
