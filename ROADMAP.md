...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(📋 **R23 dispatched 2026-06-22.** T1: enrich open call memory — `currentOpenCallMemoryText()` with all fact categories + open loops + recent call notes, passed to `initiateCall` for open calls. T2: inbound call handler — `assistant-request` webhook branch, `findByPhoneNumber`, `is_inbound` DB column, Edge greets "Hey, it's Edge — [firstName], how can I help?". ⚠️ Coordinate with Security R18 — land Core's T2 PR first so Security can add the rate-limit layer. _(Previous: R22 + R21 SHIPPED — Cantonese + gratitude daily quote.)_)_ | `lib/db.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-22 |
| 🔒 Security | `security` | _(✅ **R18 T1 helpers SHIPPED (2103 green); ⏳ webhook wiring deferred on Core R23 T2.** `inbound_call_attempts` table + `checkInboundCallRateLimit` (5/24h per phone) + `auditLogQueries.logInboundCallAttempt` (unknown caller → sentinel user_id 0 since audit_log.user_id is NOT NULL). 8 tests. **➡️ Once Core's `assistant-request` handler lands in `webhook/route.ts`, I'll layer in the rate-limit + audit calls (additive) + the 8s polite-decline response.** _(R17 + Twilio hotfix also shipped.)_)_ | `lib/rateLimit.ts`, `lib/db.ts`, `lib/inbound-call.test.ts` | 2026-06-22 |
