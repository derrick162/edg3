...
| 🔒 Security | `security` | _(🔎 **R15 T1 FINDINGS: Litestream already built + wired. Nixpacks build; `railway.toml` sets `startCommand = "sh scripts/start.sh"` — no Dockerfile needed.** `litestream.yml` + `scripts/start.sh` correct (uses `/data/edg3.db` on persistent volume). **➡️ PM/Derrick: set `LITESTREAM_S3_BUCKET` + `LITESTREAM_S3_ACCESS_KEY_ID` + `LITESTREAM_S3_SECRET_ACCESS_KEY`. Proposed bucket: `edg3-prod-db-backups`.**)_ | `litestream.yml`, `scripts/start.sh` | 2026-06-20 |
