...
| 🔒 Security | `security` | _(✅ **R22 DONE (2298 green) — ready for merge; idle.** Weekly synthesis cron already wired (M4-5). Added MONTHLY consolidation cron: `runMonthlyConsolidationSweep` self-gates to the first Sunday of month, runs `runLifetimeSynthesis` per user; 4 tests. Pillar-Trust scan: no open `(Security)` code items (live-QA only). R21+R20 merged/deployed. Awaiting next dispatch.)_ | `lib/scheduler.ts` | 2026-06-24 |
