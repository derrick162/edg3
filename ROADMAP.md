...
| 🔒 Security | `security` | _(🔎 **R15 T1 FINDINGS: Litestream already built + wired. Nixpacks; `railway.toml` → `start.sh`. ➡️ PM: set `LITESTREAM_S3_BUCKET` + `LITESTREAM_S3_ACCESS_KEY_ID` + `LITESTREAM_S3_SECRET_ACCESS_KEY`. Bucket: `edg3-prod-db-backups`.**)_ | `litestream.yml`, `scripts/start.sh` | 2026-06-20 |
