...
| 🔒 Security | `security` | _(✅ **T3-3 + reliability hardening — 1708 green.** T0-1 durability self-check + restore drill, T0-4 scheduler lock, T0-2 startup key check, M2-4 context-pack guard. 🐞 Fixed account-deletion FK-500 bug. 🐞 T3-3: activity log added to data export. ⚠️ Derrick AM action list in `content/qa-log.md`. Next: T1-1 webhook retry.)_ | `app/api/account/export/route.ts`, `lib/backup.ts`, `lib/db.ts`, `lib/durability.ts` | 2026-06-18 |
| 🔧 PM/CoS | `master` | _(🔄 **Merging second batch — security + core + design in progress.** 1790 green prior merge. ⚠️ Derrick: push to prod + read `content/qa-log.md` AM action list.)_ | `ROADMAP*.md` | 2026-06-18 |
