...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Overnight batch complete — awaiting PM merge**: T2-2 stale-fact hedging in briefings; Round 6 T1 + DC2-0/1/3/3b (briefing quality + opener rule + commitment surfacing + Whoop reliability); DC0-1b due-date extraction from transcripts; UX-1 landing page brand + timing copy fix; T0-3 end-to-end smoke test for 7am call-to-briefing path.)_ | `lib/briefing.ts`, `lib/vapi.ts`, `lib/facts.ts`, `app/api/smoke/route.ts`, `app/page.tsx` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **Overnight batch complete (1672 green) — awaiting PM merge**: T0-2+T1-3+T4-1+DC1-1 (health digest + token auth + call log); T4-2+DC1-2 (Vapi pre-call health ping + retry at T+5min); DC1-3 (call time accuracy audit + timing delta log); T4-4 (webhook_dedup_keys + tool_call_dedup_keys + atomic claim gates, 10 new tests). Next: T3-2 activity log completeness.)_ | `lib/db.ts`, `lib/idempotency.ts`, `app/api/vapi/webhook/route.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-18 |
| 🔧 PM/CoS | `master` | _(🔄 **Merging overnight lane work.** All 3 lanes shipped overnight (Auto + /loop). Security: T4-4 + DC1-1/2/3 (1672 green). Core: T2-2 + DC2-0/1/3/3b + DC0-1b + UX-1 + T0-3. Design: branding sweep + activity T2. Merging security → core → design → preflight → push. ⚠️ Derrick action items: Twilio follow-up, 5 design-partner names, CASA video, push to prod after PM merge.)_ | `ROADMAP*.md` | 2026-06-18 |
| 🎨 Design | `design` | _(✅ **Overnight batch complete — awaiting PM merge**: Edge→Edg3 branding sweep across all components/ui/ + dashboard + landing; Activity tab T2 (expandable inbox receipts + branding). 1652 green. Previous: landing page redesign T1-T5 (merged to master).)_ | `app/dashboard/page.tsx`, `components/ui/**`, `app/page.tsx` | 2026-06-18 |
