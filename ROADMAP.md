...
| 🔒 Security | `security` | _(🚨 **PROD FIX NOT YET IN MASTER — MERGE + DEPLOY ASAP (1857 green): migration runner** (+ "never-again" full-initSchema regression guard + admin/facts SQL hardening). `no such column: valid_until`/`retry_after` on deploy: a CREATE INDEX on `facts.valid_until` (migration-added column) sat in the PRE-migration block → threw on existing DBs → aborted schema init before migrations ran. Fixed: deferred the index after migrations + `applyMigrations()` extracted/tested + deduped array. `lib/db-migrations.test.ts` reproduces it. Kevin: manual ALTERs safe; self-heals after deploy. Earlier (already merged): multi-account Google token layer, Round 6 T1 (11pm user-local), T1-1 webhook retry+DLQ. **All Security-owned pillar items exhausted/done.** ⚠️ EXTERNAL: register GMAIL_REDIRECT_URI in Google console + set env. ⚠️ AM: export-boundary call + `content/qa-log.md` action list. Commits local-only.)_ | `lib/db.ts`, `lib/scheduler.ts`, `app/api/admin/**`, `app/api/auth/**` | 2026-06-18 |
