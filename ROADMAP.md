...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Overnight batch merged to master**: T2-2 stale-fact hedging in briefings; Round 6 T1 + DC2-0/1/3/3b (briefing quality + opener rule + commitment surfacing + Whoop reliability); DC0-1b due-date extraction from transcripts; UX-1 landing page brand + timing copy fix; T0-3 end-to-end smoke test for 7am call-to-briefing path.)_ | `lib/briefing.ts`, `lib/vapi.ts`, `lib/facts.ts`, `app/api/smoke/route.ts`, `app/page.tsx` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **Overnight batch merged to master (1672 green)**: T0-2+T1-3+T4-1+DC1-1 (health digest + token auth + call log); T4-2+DC1-2 (Vapi pre-call health ping + retry at T+5min); DC1-3 (call time accuracy audit + timing delta log); T4-4 (webhook_dedup_keys + tool_call_dedup_keys + atomic claim gates, 10 new tests). Next: T3-2 activity log completeness.)_ | `lib/db.ts`, `lib/idempotency.ts`, `app/api/vapi/webhook/route.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-18 |
| 🔧 PM/CoS | `master` | _(✅ **18-commit overnight batch merged + pushed (1703 green).** Overnight PM batch: memory-moat-one-pager updated (5/7 layers live, Outcomes now Live); Round 6 spec written (`content/round-6-spec.md`); Cam dispatched UX-2/3/4 (`content/cam-dispatch-ux-trust.md` + Status Board); CASA submission draft written (`content/casa-submission-draft.md`); privacy policy accuracy pass (AES-256-GCM, date, rights updated); all 3 pillar docs updated with shipped items (10 Trust ✅, 8 Daily-Call ✅, 7 Memory ✅). ⚠️ Derrick action items: Twilio follow-up, 5 design-partner names, CASA video, push to prod.)_ | `ROADMAP.md`, `PILLAR-*.md`, `content/**`, `app/privacy/page.tsx` | 2026-06-18 |
| 🎨 Design | `design` | _(📥 **PM DISPATCH — UX Trust fixes (UX-2/UX-3/UX-4).** Full spec in `content/cam-dispatch-ux-trust.md`. In order: (1) Dedup display + entity grounding in "What Edge knows" People section — filter self/Edge/Edg3 entities, collapse near-identical facts; (2) Cursor fix + name accuracy — `cursor: pointer` global CSS rule, isSelf guard on People render; (3) Collapsible memory sections — category headers show count + chevron, default first 3 expanded. All three needed before launch. `git merge master` first.)_ | `app/dashboard/page.tsx`, `app/globals.css` | 2026-06-18 |
