...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R18 T1+T3 SHIPPED (2067 green) — ready for PM merge; ⚠️ T2 BLOCKED.** T1: `findNextFreeSlot` (`lib/calendar.ts`) + `PRIORITY BLOCKING SLOT` injection (`lib/briefing.ts`) + `lib/vapi.ts` guidance — Edge names an exact slot for under-scheduled priorities. T3: `calendar_tokens.email` (additive, **Vijay FYI**) + `fetchGoogleAccountEmail` + callback/connect `select_account` + `/api/auth/accounts` returns `calendar.email` (Security file, additive, **Vijay FYI**) + dashboard shows linked account + "Switch account →"; fixed dead `/api/auth/google` re-auth links. **T2 NOT BUILT — needs Kevin's decision:** depends on `checkOutreachReplies`/`outreach_tracking`, all DELETED in R12 T7 (email removal). Won't resurrect autonomously — confirm revive vs drop. Details in ROADMAP-CORE changelog.)_ | `lib/calendar.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `app/api/calendar/**`, `app/api/auth/accounts/**`, `app/dashboard/page.tsx` | 2026-06-21 |
...
| 🎨 Design | `design` | _(📋 **R16 DISPATCHED (2026-06-21) — T1: 7-day ScoreSparkline on /score page, T2: onboarding step indicator mobile polish.**)_ | `components/ui/ScoreSparkline.tsx`, `app/score/page.tsx`, `app/onboarding/**` | 2026-06-21 |
