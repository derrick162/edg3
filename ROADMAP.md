...
| 🔧 PM/CoS | `master` | _(✅ **Round 8 + P0 bugs FULLY SHIPPED — 1917 green, master at `9f53901`.** Code is live on Railway. **🚨 DERRICK — DO THIS FIRST (root cause of missed calls + disappearing memory):** (1) Railway dashboard → Edg3 service → Volumes tab → confirm persistent volume at `/data`. If missing, add one NOW. (2) Check Railway logs for `[durability]` line. (3) Set env vars: `LITESTREAM_S3_BUCKET`, `LITESTREAM_S3_ACCESS_KEY_ID`, `LITESTREAM_S3_SECRET_ACCESS_KEY`. Full runbook: `content/durability-runbook.md`. **After volume confirmed:** send `/loop` to Darren/Vijay/Cam to kick off next dispatch (T3-1-A pattern constraint, DC3-1 voice anchors).)_ | `ROADMAP*.md` | 2026-06-19 |
