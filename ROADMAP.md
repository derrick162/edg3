...
| 🔒 Security | `security` | _(✅ **R19 T3+T4 DONE (2143 green) — ready for PM merge. T5 BLOCKED on Core R25 T4.** T1+T2 merged. T3: greeting boundary 5 PM, centralized `lib/greeting.ts` (`5b5b74f`). T4 [HIGH]: completed gratitude/open call no longer suppresses the morning briefing — both once-a-day guards in `lib/scheduler.ts` exclude `is_open_call`; extracted to tested `findTodaysBlockingBriefing` helper (`39e83a2`). **T5 (nightly score cron) waiting on Core's `computeAndSaveScore` export from `lib/scores.ts` — not on master yet; will wire the moment it lands.** **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `lib/scheduler.ts`, `lib/greeting.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
