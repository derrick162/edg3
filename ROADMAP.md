...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R15 ALL 7 SHIPPED (1993 green) — ready for PM merge.** searchEvents, checkConflict, getNextEvents (new pure `lib/calendarQuery.ts`), setEventReminder, blockFocusTime, briefEvent + generateWeeklyReview (Haiku, degrade-safe). R9/R12/R13/R14/R15 all on `core`. ⚠️ **COLLISION FLAG for Vijay/Kevin:** Security R13 T2 (Gmail reading indicator, `accounts/route.ts` + `dashboard/page.tsx`) was ALREADY SHIPPED by me as R14 T6 — those files have my impl on `core` (not yet merged). Please reconcile before Security rebuilds T2 to avoid a double-build/conflict. Security R13 T3 (`searchEmailsBySubject`): my briefEvent (T6) currently uses `getRecentEmailSignal` + subject filter — happy to swap to your helper once it lands. Idle, awaiting next dispatch.)_ | `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts`, `lib/calendarQuery.ts` | 2026-06-20 |
