...
- `lib/gmail.ts` + `lib/google-auth.ts` — Gmail **read primitive** (`gmail.readonly` scope only; `deleteDraft` for undo compat). _(Email drafting removed R12; `createDraft`/compose scope deleted.)_
...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R23 SHIPPED (2102 green) — ready for PM merge.** **Hotfix:** TOOL CALL DISCIPLINE prompt block (Edge no longer claims "Done" without calling the tool). **T1:** `lib/callMemory.ts` `currentOpenCallMemoryText` (all fact cats + open loops + recent call notes) → open calls get briefing-quality recall. **T2:** inbound `assistant-request` webhook branch (`findByPhoneNumber`, `is_inbound` col, `buildOpenCallSystemPrompt`, `CALENDAR_TOOL_IDS`) — unknown caller polite decline / registered caller full personalized open call; `call-started` links the call.id. 8 new tests. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` + `app/api/vapi/webhook/route.ts` — Vijay sync down + add inbound rate-limit layer.** ⚠️ External (Derrick): point Vapi inbound handler at `/api/vapi/webhook`. Idle, awaiting next dispatch.)_ | `lib/db.ts`, `lib/vapi.ts`, `lib/callMemory.ts`, `lib/scheduler.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-22 |
| 🔒 Security | `security` | _(✅ **R18 FULLY SHIPPED (2113 green); idle/polling.** After Core R23 T2 landed, layered the inbound rate-limit + audit calls into the `assistant-request` handler (additive): `checkInboundCallRateLimit` before the user lookup → 8s polite-decline on breach; `logInboundCallAttempt` for allowed/unknown/rate_limited. +2 integration tests. Inbound calls now capped at 5/24h per phone + fully audited. **⚠️ EXTERNAL (Derrick):** point the Vapi inbound webhook at `/api/vapi/webhook`; Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `app/api/vapi/webhook/route.ts`, `lib/rateLimit.ts`, `lib/db.ts` | 2026-06-22 |
| 🔧 PM/CoS | `master` | _(📋 **Design R22+R23 dispatched. Core R24 dispatched. Security R19 dispatched.** R23 (Design, do first): sidebar cleanup (remove duplicate Gmail, hide Whoop when unconnected, remove Recent Alerts, connect email shown) + score sparkline (continuous line to today + date labels). R22: briefings date grouping + transcript polish + score trust signals. R24 (Core): onboarding "Connect your tools" step (Calendar required, Whoop optional). R19 (Security): quota retry cascade fix.)_ | `ROADMAP.md`, `ROADMAP-CORE.md`, `ROADMAP-SECURITY.md`, `ROADMAP-DESIGN.md` | 2026-06-23 |
| 🎨 Design | `design` | _(✅ **R23 + R22 SHIPPED.** R23: sidebar cleanup + sparkline to today. R22: briefings date grouping + transcript polish + score trust labels. All dispatch exhausted. Moving to pillars.)_ | `app/dashboard/page.tsx`, `components/ui/EdgeScoreCard.tsx`, `app/score/page.tsx`, `app/globals.css` | 2026-06-23 |
