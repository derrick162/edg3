...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **ROUND 5 COMPLETE — Memory self-learning flywheel (1506 green, preflight clean)**: T1 bi-temporal facts, T2 sleep-time consolidation agent, T3 in-call memory triggers, T4 historical pattern detection (weekly Haiku pass → commitment/priority patterns → briefing §3). Voice naming reconciliation (daniel/aria). Awaiting PM merge.)_ | `lib/facts.ts`, `lib/db.ts`, `lib/patternMemory.ts`, `lib/factPatterns.ts`, `lib/briefing.ts`, `app/api/vapi/tool-call/route.ts`, `app/api/profile/voice/route.ts` | 2026-06-18 |
