...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Night-2 queue COMPLETE (813 green)**: ★ Open Loops extraction + `/api/open-loops` + briefing/recs/vapi wiring (DB stub, swap on Vijay's migration); Edge Score calibrating fix (renormalize blend); email-relevance tuning; memory dedup + consolidation; STT name correction. ⚠️ Cam: GET `/api/open-loops` returns 3 buckets — design surface dispatched. ⚠️ Vijay: swap comment in `lib/openLoops.ts`. Awaiting next dispatch.)_ | — | 2026-06-16 |
| 🔒 Security | `security` | _(✅ **Security gap assessment + quick-win hardening COMPLETE** (827 green): H2 verify-promises unauthenticated endpoint fixed (direct call + checkVapiSecret gate); M3 clearSessionCookie flags; M4 postMessage origin; M5 briefingCall rate limit; M6 email normalization; L4 timingSafeEqual Vapi; L5 backup filename regex; L6 parseInt radix. Security headers + content/security.md shipped. NOT fixed: H3 OAuth CSRF state (needs PM go), M7 JWT revocation, CSP nonce. Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
...
| 🎨 Design | `design` | _(idle — ✅ **Today's Focus + FAQ + Help & Support hub COMPLETE**: Today's Focus actionable (✓ Complete + ✕ Dismiss + replacement animation); FAQ accordion (6 sections, placeholder copy awaiting Esther); Feedback/contact form → `/api/support`. Dashboard "Help" tab live. Preflight clean. ⚠️ `git push --force origin design` still needed. Awaiting PM.)_ | `components/ui/FocusRecommendationCard.tsx`, `components/ui/HelpSupportSection.tsx`, `app/dashboard/page.tsx` | 2026-06-16 |
