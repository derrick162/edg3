...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R32 + R33 SHIPPED (2221 green) — ready for PM merge.** R32 (CRITICAL): Edge can't false-confirm a calendar action — prompt rule + explicit "ERROR: NOT created" returns + `friendlyError`→pure `lib/calendarToolErrors.ts`. R33: work-hours setting (`users.work_schedule` + `lib/workHours.ts` + `/api/profile/work-hours` + Settings UI) and Edge defers after-hours work blocks (WORKING HOURS block + briefing prompt). ⚠️ **Additive to Shared `lib/db.ts` + Security-owned `lib/vapi.ts`/`lib/scheduler.ts` (new `initiateCall` param at 2 sites) — Vijay sync down.** Idle.)_ | `lib/db.ts`, `lib/workHours.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `lib/briefing.ts`, `app/settings/page.tsx`, `app/api/profile/work-hours/route.ts` | 2026-06-24 |
