...
| 🔒 Security | `security` | _(🏗️ **R18 IN PROGRESS (2026-06-22) — inbound call rate limit + audit.** Building `checkInboundCallRateLimit(phone)` (5/24h) + `logInboundCallAttempt()` + `inbound_call_attempts` table standalone with tests; will layer the calls into the `assistant-request` handler once Core R23 T2 lands (additive). _(R17 + Twilio hotfix shipped.)_)_ | `lib/rateLimit.ts`, `lib/db.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-22 |
