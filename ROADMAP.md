...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R13 ALL 4 SHIPPED (1955 green) — ready for PM merge.** T1 batchReschedule (window move/clear, 1 token), T2 skip/end recurring, T3 blockTravelTime (+90min warn), T4 conflict surfacing (names event+time). New `lib/batchSchedule.ts` + `applyRruleUntil` in time.ts; +16 tests. ⚠️ **External (PM):** create 4 Vapi tools (batchReschedule, skipRecurringOccurrence, endRecurringSeries, blockTravelTime) + paste UUIDs into both toolIds arrays (placeholders in `lib/vapi.ts`). All R9/R12/R13 now shipped on `core`. Idle, awaiting next dispatch.)_ | `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts`, `lib/batchSchedule.ts`, `lib/time.ts` | 2026-06-20 |
| 🔒 Security | `security` | _(✅ **R12 T2 done + PILLAR-TRUST T3-3 completed (1940 green); idle/polling — Security pillar surface fully exhausted.** T3-3: export now includes retired facts + `status`/`confidence`/`confidenceScore`/`lastConfirmedAt` + `undoHistory`; `version` → '4'. (Earlier: full email-feature removal — `gmail.readonly` is the only Gmail scope now.) Remaining pillar items are all Core/Design-owned or Phase-2-gated. **⚠️ EXTERNAL (Derrick):** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, restore drill. **FYI cleanup candidate (non-blocking):** vestigial `gmail_tokens`/`gmailTokenQueries` + `/api/auth/accounts` gmail field now have no writer.)_ | `app/api/account/export/route.ts`, `PILLAR-TRUST.md` | 2026-06-20 |
