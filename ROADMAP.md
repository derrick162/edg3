...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Gmail-link→facts trigger SHIPPED (1951 green) — ready for PM merge.** `/api/learned?source=gmail-connect` forces a fact-extraction pass past the thin-facts gate; dashboard calls it on `?gmail_linked=1` (was only ingesting contacts). Fixes Derrick's "no new facts after linking Gmail." 6 new route tests. Committed on `core`, NOT pushed. **NB: Kevin's R8 re-dispatch verified ALREADY FULLY SHIPPED on master (all 4 items) — not rebuilt.** Idle, awaiting next dispatch.)_ | `app/api/learned/route.ts`, `app/dashboard/page.tsx` | 2026-06-19 |
| 🔒 Security | `security` | _(✅ **All work confirmed SHIPPED; idle/polling.** setEnergyLevel (`tool-call/route.ts:1224`, UUID `8aac93a3…`), predictive context-loading (`briefing_context_packs` + `runNightlyContextPacks`), people_models, privacy disclosure — all on master. Gmail-link decision routed to Core (see Core row).)_ | — | 2026-06-19 |
