...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(idle — ✅ **Email signal wired (759 green)**: `getRecentEmailSignal` now flows into `recommendFocusAreas` via briefing + dashboard API; urgent/financial/legal emails weighted as #1 focus candidate; `isUrgentEmail` + `formatEmailSignalForPrompt` helpers tested. ⚠️ Derrick must reconnect Google to grant gmail.readonly. Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
| 🔒 Security | `security` | _(✅ **Call hardening + CASA items COMPLETE** (744 green): (1) Call path — claim-first anti-double-dial + daily-limit guard, 6 new tests; (2) Rate limiting on 5 new routes (user-scoped); (3) Audit log on applyDayPlan + confirmFocusAreas write paths. Remaining CASA non-eng: demo video + PM consent decision. Awaiting PM for next dispatch.)_ | — | 2026-06-15 |
...
| 🎨 Design | `design` | _(idle — ✅ Home tab cockpit COMPLETE (`ed80dee`): 'home' tab first + default, cockpit cards (EdgeScore→FocusRec→DayPlan) home-only, greeting+date in persistent header. 744/744 green. Awaiting PM.)_ ✅ RecoveryCard sleep-hero + EdgeScore breakdown + Fix-It clarity COMPLETE (`4a51874`): sleep score hero (green ≥75), recovery secondary; EdgeScore breakdown shows drivers[] + 'what it measures'; 'Improve my day →' + 'How is this calculated?' copy; DayPlan preview safety line. Core: pass `sleepScore` from /api/whoop/status. 744/744 green. Awaiting PM.)_ | — | 2026-06-15 |
