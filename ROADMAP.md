...
| 🔒 Security | `security` | _(✅ **R19 SHIPPED (2116 green); idle/polling.** **R19 T1 — quota-error retry cascade fix:** ElevenLabs `pipeline-error-eleven-labs-quota-exceeded` matched `MISSED_CALL_REASONS` → infinite retry loop (14 failed briefings 2026-06-22). Added `isQuotaError` guard in `app/api/vapi/webhook/route.ts`: quota errors mark missed but **never** schedule a retry; transient pipeline/no-answer retries preserved. New `quota-retry.test.ts` (3 cases). _(R18 inbound rate-limit + audit shipped prior.)_ **⚠️ EXTERNAL (Derrick):** point the Vapi inbound webhook at `/api/vapi/webhook`; Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `app/api/vapi/webhook/route.ts` | 2026-06-23 |
| 🔧 PM/CoS | `master` | _(📋 **R22 (Design) + R19 (Security) dispatched 2026-06-23.** R22: briefings date grouping + transcript polish + score trust signals. R19: quota-error retry cascade fix. Hotfixes on master: transcript scrollable box + skip duplicate first AI bubble.)_ | `ROADMAP.md`, `ROADMAP-CORE.md`, `ROADMAP-SECURITY.md`, `ROADMAP-DESIGN.md` | 2026-06-23 |
| 🎨 Design | `design` | _(📋 **R22 DISPATCHED 2026-06-23.** T1: briefings date grouping + transcript polish (Copy button visibility, empty-transcript placeholder, thin scrollbar token). T2: score trust signals ("Recalculates on page load" label on dashboard + /score page subtitle + score-high/medium/low colour tokens). git merge master first.)_ | `app/globals.css`, `app/dashboard/page.tsx`, `app/score/page.tsx` | 2026-06-23 |
