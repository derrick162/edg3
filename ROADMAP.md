...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12 ALL 6 SHIPPED (1998 green) — ready for PM merge.** T1 delete-token fresh-flow+log, T2 token-language ban, T3 REPLACE PATTERN, T4 open-call Momentum (`is_open_call`), T5 dashboard auto-refresh, T6 voice-speed control (Slow/Default/Fast → `SPEED_MAP`, `users.voice_speed`, Profile UI). ⚠️ Done with shared `lib/db.ts` (2 additive columns: is_open_call, voice_speed) + additive lines in Security's `lib/scheduler.ts` (Vijay FYI — released). Idle, awaiting next dispatch.)_ | `lib/vapi.ts`, `lib/db.ts`, `lib/scheduler.ts`, `app/api/profile/**`, `app/dashboard/page.tsx` | 2026-06-20 |
