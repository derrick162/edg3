...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **T3-1-B SHIPPED (1918 green)** — pattern facts now `category='pattern'` (Patterns tab populates); also fixed the dead `source`-based read/retire filters (no `source` column existed → patterns were write-only). Earlier today: Round 8 (M4-4 + Briefing V2) + both P0 bugs (focus-card CTA, event classifier). Awaiting next dispatch.)_ | `lib/factPatterns.ts` | 2026-06-19 |
