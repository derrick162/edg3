...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R34 SHIPPED (2232 green) — ready for PM merge** (R32+R33 also awaiting merge). T1: commitment tracking (`commitment` fact category + OPEN COMMITMENTS briefing block, <72h). T2: PEOPLE DEEPENING (main + gratitude prompts). T3: hard OPENER RULE (no meals/gym/routine). T4: prior-call CONTINUITY (`pickContinuitySource` <48h + Haiku signal + callback instruction). ⚠️ **Additive to Shared `lib/db.ts` (Fact type) + Security `lib/vapi.ts` — Vijay sync down.** Idle.)_ | `lib/facts.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `lib/factConfidence.ts`, `lib/memorySalience.ts` | 2026-06-24 |
