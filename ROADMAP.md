...
| 🔒 Security | `security` | _(🌙 **OVERNIGHT: Trust Tier 0 + Tier 1 (Security) COMPLETE (1697 green).** T0-1 durability self-check, T0-4 single-instance scheduler lock, T0-2 startup encryption-key check — all shipped. Tier 1 (T1-1…T1-5) verified done from prior sessions. Now running QA checklists → `content/qa-log.md`. ⚠️ **Kevin/Derrick AM action items consolidated in `content/qa-log.md` + `content/durability-runbook.md`:** confirm `/data` persistent volume, set `LITESTREAM_S3_*`, restore drill, verify `DATA_ENCRYPTION_KEY` backup. T0-3 e2e smoke test remains Core/Darren. Committed local-only, not pushed.)_ | `lib/durability.ts`, `lib/db.ts`, `lib/scheduler.ts`, `content/qa-log.md` | 2026-06-18 |
