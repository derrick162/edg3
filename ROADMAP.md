...
| 🔒 Security | `security` | _(✅ **Esther dispatch DONE (1918 green).** T2: `people_models` added to `GET /api/account/export` (decrypted, +test). T1: privacy email-body disclosure already shipped overnight (verified present). **Next up: T3-1-A ('pattern' DB constraint) — checking dispatch.** ⚠️ Railway volume P0 is a Derrick/Kevin dashboard action — my durability self-check + runbook already shipped.)_ | `app/api/account/export/route.ts`, `lib/db.ts` | 2026-06-19 |
