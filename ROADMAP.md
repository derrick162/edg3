...
| 🔒 Security | `security` | _(✅ **R16 BOTH SHIPPED (2068 green); idle/polling.** T1: `validateJwtSecret()` in `lib/auth.ts` (rejects unset/<32/placeholder) enforced at startup via `instrumentation.ts` (NOT module-init — that'd break `next build`/tests); `.env.example` created + un-ignored. T2: `GET /api/user/export` GDPR self-export (no secrets — whoop=bool, push=count). **⚠️ T2 notes for Kevin: spec's `outreach_tracking` omitted (table removed R12); overlaps `/api/account/export` — consolidation candidate.** **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `lib/auth.ts`, `instrumentation.ts`, `.env.example`, `.gitignore`, `app/api/user/export/**` | 2026-06-21 |
