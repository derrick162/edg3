...
| 🔒 Security | `security` | _(✅ **R10 ALL 4 TICKETS SHIPPED (1959 green); R11 dispatched. Idle/working.** R11: T1 Gmail scope close-out, T2 rate-limit hardening, T3 key rotation runbook. **⚠️ EXTERNAL (Derrick):** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, restore drill.)_ | `lib/crypto.ts`, `lib/auth.ts`, `content/security-audit.md`, `content/durability-runbook.md` | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **R9 (Core 7 tickets) + R11 (Design 3 + Security 3) dispatched. Merging lanes. Kevin building Core T5–T7 directly. 🚨 DERRICK — one pending action:** `getWeather` Vapi tool in dashboard (no params) → paste UUID into `lib/vapi.ts`. Railway volume + Litestream env vars.)_ | `ROADMAP*.md`, `lib/calendarScore.ts`, `app/api/scores/route.ts` | 2026-06-20 |
| 🎨 Design | `design` | _(✅ **R10 SHIPPED (1958 green) — DayReshapedToast + empty states + profile polish.** R11 dispatched: T1 Focus Score hours label, T2 Weekend card, T3 activity dedup. Working.)_ | `app/dashboard/page.tsx`, `components/ui/FocusRecommendationCard.tsx`, `lib/activityLabels.ts` | 2026-06-20 |
