...
| 🔒 Security | `security` | _(✅ **Synced master (1880 green) — migration fix reconciled.** The prod migration bug is fixed on master already (master independently deferred the idx_facts_active index); my branch carries the cleaner version (`applyMigrations()` extracted + exported + tested in `lib/db-migrations.test.ts` + deduped array + full-initSchema regression guard). Merge resolved to keep my tested version **and** master's new `oauth_state` gmail-CHECK migration. PM can merge my branch at normal priority — no longer urgent (master self-heals). All Security pillars + dispatches done: multi-account Google token layer, Round 6 T1 (11pm user-local), T1-1 webhook retry+DLQ, admin/facts SQL hardening. ⚠️ EXTERNAL: register GMAIL_REDIRECT_URI in Google console + set env. ⚠️ AM: export-boundary call + `content/qa-log.md` action list.)_ | `lib/db.ts`, `lib/scheduler.ts`, `app/api/admin/**`, `app/api/auth/**` | 2026-06-18 |
