...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(idle — ✅ **Dashboard wiring COMPLETE** (724 green): `EdgeScoreCard` + `FocusRecommendationCard` + `DayPlanCard` + `NotificationCenter` wired; `GET /api/day-plan` + `POST /api/day-plan/confirm` built. NEXT: energy color-coding (`colorByEnergy` via `event_energy_tags`), remove Tasks IA.)_ | — | 2026-06-15 |
