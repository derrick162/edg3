...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R32 SHIPPED (2204 green, CRITICAL — awaiting merge); R33 IN PROGRESS.** R32: Edge can't false-confirm a calendar action — prompt CRITICAL rule + explicit "ERROR: NOT created" handler returns + `friendlyError`→pure `lib/calendarToolErrors.ts`. R33 (next): work-hours setting + Edge defers after-hours scheduling. ⚠️ **Shared `tool-call/route.ts` + Security `lib/vapi.ts` (Core tool side) + `lib/db.ts` schema incoming — Vijay sync down.**)_ | `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts`, `lib/calendarToolErrors.ts`, `lib/db.ts`, `lib/briefing.ts` | 2026-06-24 |
