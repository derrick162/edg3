...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(📋 **R9 DISPATCHED (2026-06-20) — 7 tickets from Derrick's live call + dashboard review.** T1: voice speed -10%. T2: briefing fallback silent + priority truncation. T3: spelling override prompt. T4: getWeather tool. T5: Focus Score detail + error fix. T6: move briefing calls Clarity→Momentum. T7: Momentum call count audit + weekend focus brief. Full specs in `ROADMAP-CORE.md` R9 section. Idle/awaiting Darren.)_ | `lib/vapi.ts`, `lib/briefing.ts`, `lib/calendarScore.ts`, `app/api/scores/route.ts`, `components/ui/FocusRecommendationCard.tsx` | 2026-06-20 |
| 🔒 Security | `security` | _(✅ **R11 ALL 3 TICKETS SHIPPED (1969 green); idle/polling.** **T1:** Gmail `gmail.readonly` scope decision closed → ✅ ACCEPTED in `security-audit.md` (required for contact ingest). **T2:** rate-limit gap — added `vapiToolCall` 60/min on the tool-call webhook (the one real gap; graceful spoken refusal, not raw 429); memory-writes + briefing-regen already covered. Reused the SQLite limiter (better than the proposed in-memory Map — survives restart/multi-instance). **T3:** key rotation — `reEncryptAllUserData(old,new,{dryRun})` in `crypto.ts` (per-user txn, resumable, fail-loud) + `ENCRYPTED_COLUMNS` 19-table inventory in `db.ts` (drift guard caught a real `pattern_cache` PK bug) + "Key Rotation" runbook. 7 tests. **⚠️ EXTERNAL (Derrick):** Railway volume + `LITESTREAM_S3_*`, `DATA_ENCRYPTION_KEY` backup, restore drill.)_ | `lib/crypto.ts`, `lib/db.ts`, `lib/rateLimit.ts`, `app/api/vapi/tool-call/route.ts`, `content/durability-runbook.md` | 2026-06-20 |
| 🔧 PM/CoS | `master` | _(✅ **R9 dispatched to Darren (Core) — 4 live call quality tickets written.** Voice speed, briefing fallback fix, spelling override, weather search. **🚨 DERRICK — one pending action:** Railway persistent volume at `/data` + Litestream env vars + `getWeather` Vapi tool (no params) → paste UUID into `lib/vapi.ts`.)_ | `ROADMAP*.md` | 2026-06-20 |
