...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R14 ALL 6 SHIPPED (1986 green) — ready for PM merge.** T1 findFreeTime, T2 createEvent recurrence, T3 attendees + editEventAttendees, T4 addTask/completeTask, T5 forgetFact, T6 Gmail-reading sidebar indicator. New pure modules attendees/taskMatch/factForget + computeFreeSlots/buildRrule; +30 tests. ⚠️ **External (PM):** create 5 Vapi tools (findFreeTime, editEventAttendees, addTask, completeTask, forgetFact) + add recurrence/attendees params to createEvent; paste UUIDs into both toolIds arrays. R9/R12/R13/R14 all shipped on `core`. Idle, awaiting next dispatch.)_ | `app/api/vapi/tool-call/route.ts`, `lib/vapi.ts`, `lib/time.ts`, `app/api/auth/accounts/route.ts`, `app/dashboard/page.tsx` | 2026-06-20 |
