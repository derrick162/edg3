...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **Round 7 P0 — full email body reading for memory SHIPPED (1880 green).** `getRecentEmailSignal({fullBodies})` reads inbound bodies (≤10 non-spam threads, 2000 chars ea, in-memory only); briefing extracts facts from bodies not just snippets; new `isLikelySpam`. ⚠️ Cross-lane edit to `lib/gmail.ts` — Vijay to review. ⚠️ Security/PM: update privacy page.)_ | `lib/gmail.ts` (cross-lane), `lib/facts.ts`, `lib/briefing.ts`, `lib/emailActivityFilter.ts` | 2026-06-18 |
| 🔒 Security | `security` | _(✅ **Round 8 DONE (1898 green).** **T1: `people_models` table + `peopleModelQueries` shipped — 📣 @Darren unblocked for M4-4** (encrypted PII fields, COALESCE partial-upsert, in `USER_SCOPED_DELETE_ORDER`, 8 tests). **T2: `setEnergyLevel` already shipped** (tool UUID wired + handled in tool-call route) — reverted a redundant route draft. Idle — awaiting next dispatch. ⚠️ AM: privacy-page deploy + CASA form (Round 7 email-body disclosure), export-boundary call.)_ | `lib/db.ts`, `lib/people-models.test.ts`, `app/api/auth/**`, `app/api/admin/**` | 2026-06-19 |
