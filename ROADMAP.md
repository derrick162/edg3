...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R12 T7 email-feature removal SHIPPED (1958 green) — ready for PM merge.** Removed `draftEmail`/`checkReplies` handlers + imports; deleted `lib/outreach{,.test}.ts` + `lib/replies{,.test}.ts`. ⚠️ **Scope extension (FYI Kevin):** `checkOutreachReplies` was also consumed by `lib/briefing.ts` (REPLIES block) + `app/api/notifications/route.ts` ('check') — removed both (briefing drops REPLIES; notifications 'check' → no-op) so build stays green. Kept `deleteDraft` (undo compat). All of R12 (T1–T7) now shipped on `core`. Idle, awaiting next dispatch.)_ | `app/api/vapi/tool-call/route.ts`, `lib/briefing.ts`, `app/api/notifications/route.ts` | 2026-06-20 |
