...
| 🔒 Security | `security` | _(🔨 **R19 T3+T4 DONE (2143 green, committed). T5 BLOCKED on Core (`lib/scores.ts` not on master). T6 next.** T3: greeting boundary 5 PM (`5b5b74f`). T4 [HIGH]: gratitude/open call no longer suppresses morning briefing — `is_open_call` guard + tested `findTodaysBlockingBriefing` helper (`39e83a2`). **T5 (nightly score cron):** waiting on Core's `computeAndSaveScore` export — dynamic import of a missing module breaks the build, so deferred until it lands. **T6 (score-change notif uses `getPrior`):** picking up next. **⚠️ EXTERNAL (Derrick):** Railway VAPID + `LITESTREAM_S3_*` + volume + `DATA_ENCRYPTION_KEY` backup + restore drill.)_ | `lib/scheduler.ts`, `lib/greeting.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-24 |
