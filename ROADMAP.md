...
| 🔒 Security | `security` | _(🌙 **OVERNIGHT — Trust Tier 0+1 done + bug fixes, 8 commits, 1705 green.** Shipped: T0-1 durability self-check, T0-4 scheduler lock (+holder-named warning on contention), T0-2 startup key check, M2-4 context-pack empty guard, QA log. **🐞 Fixed account-deletion bug:** `support_messages`+`fact_history` were missing from the deletion route → FK 500 under `foreign_keys=ON`; added `deleteUserData` + drift-guard test so it can't recur. Esther dispatch items #1-#5 all addressed. **Idle/looping — re-checking board for new PM direction.** ⚠️ **Derrick/Kevin AM action list at top of `content/qa-log.md`** (Railway volume, LITESTREAM env, key backup, restore drill, post-deploy `[durability]` logs). T0-3 e2e remains Core/Darren. Commits local-only, not pushed.)_ | `lib/db.ts`, `lib/durability.ts`, `lib/scheduler.ts`, `lib/db-account-deletion.test.ts`, `content/qa-log.md` | 2026-06-18 |
