...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **FOCUS SCOREBOARD COMPLETE (1335 green, preflight clean)**: `computeWeeklyBreakdown` pure helper + `GET /api/scoreboard` endpoint + `FocusScoreboardPanel` component wired into Priorities tab (hours bars + 4-week trend table + milestone counts). Previous: compliance gate (data_consent). Awaiting PM merge.)_ | `lib/timeAllocation.ts`, `app/api/scoreboard/route.ts`, `app/dashboard/page.tsx` | 2026-06-18 |
