...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(🔨 **R29+R28 SHIPPED (2193 green, awaiting merge); R30+R31 IN PROGRESS.** R30: "Call me now" lag (reuse nightly context pack) + call-initiated notification. R31: briefing score mismatch (briefing `computeCalendarFit` 4-arg, missing Clarity+Momentum → shows 41 vs dashboard 56). ⚠️ **Additive to Shared `lib/db.ts`, Security `lib/scheduler.ts`/`lib/notifications.ts`/`lib/vapi.ts` — Vijay sync down.**)_ | `lib/briefing.ts`, `lib/scheduler.ts`, `lib/notifications.ts`, `app/api/briefing/call/route.ts` | 2026-06-24 |
