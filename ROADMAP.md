...
| 🔧 PM | `master` | _(✅ **Core + Security merged → 1133 green.** T1 fact API + T3 STT grounding live. Security: endpoint hardening, audit sweep, retention TTL, prompt-injection sanitization. Esther: full content library (ICP profiles, beta-readiness, trust page, design-partner-profiles, state-of-edge) + specs/trust-features.md. Dispatched: T1 API spec + T3 undo toast → Core (ROADMAP-CORE.md); T2 inbox receipts UI + fact-edit interaction → Design (ROADMAP-DESIGN.md). **Awaiting:** Cam fact-edit UI + T2 expand; Core T3 undo toast wire-up; Derrick `git push origin master`.)_ | `ROADMAP*.md`, `specs/**` | 2026-06-18 |
| 🎨 Design | `design` | _(idle — ✅ **T2 inbox receipts UI COMPLETE**: enhanced receipt panel on top of Core's emailSubjects/handleExpandItem — URGENT/invoice/legal/etc. subjects surface in flagged section (amber border + ⚑ icon); regular subjects listed below; max 10 + "+ N more" overflow; footer "Edge reads subject lines only — never message content." privacy line; graceful empty/error/loading states. Preflight clean.)_ | `app/dashboard/page.tsx` | 2026-06-18 |
