...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R20 SHIPPED (2080 green) — ready for PM merge.** Gratitude mode: `gratitude_mode` col + `gratitude_entries` + `gratitudeQueries` (shared `lib/db.ts`, additive, **Vijay FYI**); `GET/PATCH /api/settings/gratitude-mode`; `buildGratitudeSystemPrompt` + `recordGratitude` toolId + `initiateCall` gratitude override (`'office'` backgroundSound, gratitude only) in `lib/vapi.ts`; `recordGratitude` handler; gratitude branch in `scheduleOpenCall`; T2 `*/30`→`*/10` cron + `runGratitudeAutoCall()` auto-fires on Whoop score (5–11am). 8 new tests. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` + `lib/proactiveNotifications.ts` — Vijay sync down after merge.** ⚠️ External (Derrick): create `recordGratitude` Vapi tool + paste UUID. Idle, awaiting next dispatch.)_ | `lib/db.ts`, `lib/vapi.ts`, `lib/scheduler.ts`, `lib/proactiveNotifications.ts`, `app/api/vapi/tool-call/route.ts`, `app/api/settings/gratitude-mode/**` | 2026-06-22 |
...
| 🔧 PM/CoS | `master` | _(📋 **R21 (Core) + R19 (Design) dispatched 2026-06-22 — Daily quote feature. Gratitude prompt fix shipped to master (weather brevity + pacing + quote param). Security idle. ⚠️ Litestream S3 creds still needed from Derrick. ⚠️ CALL_GRACE_MINUTES fix needs Railway deploy.**)_ | `ROADMAP.md`, `ROADMAP-CORE.md`, `ROADMAP-DESIGN.md` | 2026-06-22 |
