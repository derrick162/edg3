...
| 🔒 Security | `security` | _(✅ **T4-4 COMPLETE (1672 green)**: webhook_dedup_keys + tool_call_dedup_keys tables; claimWebhookEvent atomic gate in webhook; claimToolCall + recordResult in tool-call loop; 10 new tests. Next: T3-2 activity log completeness → T3-3 data export accuracy.)_ | `lib/db.ts`, `lib/idempotency.ts`, `app/api/vapi/webhook/route.ts`, `app/api/vapi/tool-call/route.ts` | 2026-06-18 |
