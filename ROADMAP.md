...
| 🔒 Security | `security` | _(🔨 **T1-1 webhook retry + DLQ in progress** (synced master, 1803 green). Prior overnight work (Tier 0+1, durability self-check + restore drill, scheduler lock, account-deletion FK-500 fix, T3-3 activity-log + people export) merged to master. ⚠️ **5-min AM call: confirm export boundary** (episodes/fact_history/focus_milestones/support_messages deferred). ⚠️ **Derrick/Kevin AM action list at top of `content/qa-log.md`** (Railway volume, LITESTREAM env, key backup, live restore drill). Commits local-only, not pushed.)_ | `app/api/vapi/webhook/route.ts`, `lib/db.ts` | 2026-06-18 |
