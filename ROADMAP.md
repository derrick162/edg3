...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **OVERNIGHT PILLAR LOOP (1747 green, preflight clean)**: M4-3b, T4-5, M4-1+Round6-T2 (mid-call reconfirmation, category-weighted, fires on fresh STT-garbled facts, `confirmFact` tool), DC0-2 (post-call latency), UX-4 (no false hedging), M2-3 #5 (priority-drift pattern), M2-4 (context-pack wiring), UX-4 403-msg fix, removed DC2-3b dup, **fixed flaky preflight**. ⚠️⚠️ **§2 VIOLATION — TWO sessions ran in this one `edg3-core` worktree overnight**: a twin committed M2-3+M2-4+UX-4 as `c14ce31` (swept up this loop's in-progress M2-3 code too). Nothing lost; PM re-establish single ownership. ⚠️ External: create `confirmFact` Vapi tool (param `topic`). ⚠️ Security: decay-job category mismatch (ROADMAP-CORE changelog). Unblocked Core backlog now ~exhausted (remaining: data-gated M4-3/M2-3#4, blocked M4-4, Security items). Local commits only.)_ | `lib/factConfidence.ts`, `lib/patternMemory.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `app/api/vapi/webhook/route.ts` | 2026-06-18 |
