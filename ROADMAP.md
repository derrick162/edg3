...
| 🔒 Security | `security` | _(✅ **Overnight queue COMPLETE (1090 green)**. (1) Facts endpoint hardened: rate limit, 500-char statement cap, getById ownership check, 404 on missing, audit log (fact_update/fact_delete), priority-sync block on DELETE. Email-receipt endpoint: rate limit added. (2) Audit sweep: calendar/day-plan/facts all covered; undo gap noted. (3) Email subject retention: pruneEmailSubjects() + nightly cron wired. (4) Prompt-injection defense: newline-strip on alignment.ts titles + calendar.ts formatEventsForBriefing. (5) data-protection.md drafted for Esther. 45 new tests. Awaiting PM dispatch.)_ | `app/api/memory/facts/[id]/route.ts`, `app/api/activity/email-receipt/[id]/route.ts`, `lib/rateLimit.ts`, `lib/db.ts`, `lib/scheduler.ts`, `lib/alignment.ts`, `lib/calendar.ts`, `content/data-protection.md` | 2026-06-17 |
| 🔧 PM | `master` | _(✅ **Integrated Ticket H → `1235323` (1051 green, deployed).** Hero loop now DEEP: `buildCalendarPlan` richer diagnosis (real actions, not just focus blocks), `/api/day-plan` real 4-component projected scores (no more +12/action), always-on card. +21 tests. Earlier tonight: Design card-prominence + spark + Unsplash photos, Security S3 apply-path hardening, notification "Book a time" gated to reply-only, trust bugs T1/T2/T3 logged. IN FLIGHT (trust theme): Core T3 calendar/memory grounding (Gym→Jim) + T2 fact-correction + T1 receipts; Cam fact-edit UI + receipts UI; Vijay S4 (encrypted email-subject receipts); Esther State-of-Edge + Trust Roadmap doc. Auto-mode monitoring.)_ | `ROADMAP*.md`, `specs/**` | 2026-06-17 |
| 🎨 Design | `design` | _(idle — ✅ **Hero-loop prominence + D+E COMPLETE**: DayPlanCard moved to top of home tab, always visible, well-aligned greeting state; D spark fires on Apply→rescore (same mechanism as focus-confirm); E Unsplash photos on content cards. ⚠️ `git push --force origin design` still needed — Derrick must run manually.)_ | `app/dashboard/page.tsx`, `components/ui/DayPlanCard.tsx` | 2026-06-17 |
