...
|---|---|---|---|---|
| 🛠️ Core | `core` | _(✅ **R18 T1+T3 SHIPPED (2067 green) — ready for PM merge; T2 DROPPED (PM-confirmed).** T1: `findNextFreeSlot` (`lib/calendar.ts`) + `PRIORITY BLOCKING SLOT` injection (`lib/briefing.ts`) + `lib/vapi.ts` guidance — Edge names an exact slot for under-scheduled priorities. T3: `calendar_tokens.email` (additive, **Vijay FYI**) + `fetchGoogleAccountEmail` + callback/connect `select_account` + `/api/auth/accounts` returns `calendar.email` (Security file, additive, **Vijay FYI**) + dashboard shows linked account + "Switch account →"; fixed dead `/api/auth/google` re-auth links. **T2 dropped** — depended on `outreach_tracking`/`checkOutreachReplies`, removed R12 T7 (PM agreed). All dispatch + pillars exhausted. Idle.)_ | `lib/calendar.ts`, `lib/briefing.ts`, `lib/vapi.ts`, `lib/db.ts`, `app/api/calendar/**`, `app/api/auth/accounts/**`, `app/dashboard/page.tsx` | 2026-06-21 |
