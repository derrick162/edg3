...
| 🔒 Security | `security` | _(🌙 **OVERNIGHT — Trust Tier 0+1 done + bug fixes + reliability hardening, 11 commits, 1707 green.** Shipped: T0-1 durability self-check + **automated restore drill** (real-SQLite create→verify→reopen), T0-4 scheduler lock (+holder-named contention warning), T0-2 startup key check, M2-4 context-pack empty guard, verifyBackup table list now derived from canonical `USER_SCOPED_DELETE_ORDER`. **🐞 Fixed account-deletion FK-500 bug** (`support_messages`+`fact_history` missing from deletion) + drift-guard test; verified EVERY `REFERENCES users(id)` is via `user_id` (no other gaps). Esther dispatch #1-#5 all addressed. **Idle/looping — re-checking board for new PM direction.** ⚠️ **Derrick/Kevin AM action list at top of `content/qa-log.md`** (Railway volume, LITESTREAM env, key backup, live restore drill, post-deploy `[durability]` logs). T0-3 e2e remains Core/Darren. Commits local-only, not pushed.)_ | `lib/backup.ts`, `lib/db.ts`, `lib/durability.ts`, `lib/scheduler.ts`, `content/qa-log.md` | 2026-06-18 |
