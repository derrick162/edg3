...
export function initSchema(db: Database.Database) {
  db.exec(`
    ...
    -- C14 — voice-set custom trade alerts. Derrick SPEAKS these to Edge ("call me if SOXX breaks
    -- 501.30"); the trade-monitor service polls the active definitions and fires an outbound call
    -- when a condition triggers. Constant DEFAULTs only (S9 lesson: SQLite can't ALTER-add a
    -- non-constant default), so this ships as a fresh CREATE, never an ALTER.
    ...
    CREATE INDEX IF NOT EXISTS idx_trade_alerts_active ON trade_alerts(status, user_id);
...
export const tradeAlertQueries = {
  ...
  listActive: (userId: number): TradeAlert[] => {
    return getDb().prepare(
      "SELECT * FROM trade_alerts WHERE user_id = ? AND status = 'active' ORDER BY created_at DESC"
    ).all(userId) as TradeAlert[];
  },
  // All active alerts across users — the definitions feed the trade-monitor watcher polls.
  listAllActive: (): TradeAlert[] => {
    return getDb().prepare("SELECT * FROM trade_alerts WHERE status = 'active' ORDER BY id").all() as TradeAlert[];
  },
  cancel: (userId: number, id: number): void => {
    getDb().prepare("UPDATE trade_alerts SET status = 'cancelled' WHERE id = ? AND user_id = ? AND status = 'active'").run(id, userId);
  },
  // Fire path (Security-gated): mark an alert fired when the watcher triggers it. Scoped by user
  // where known; the fire endpoint validates ownership before calling this.
  markFired: (id: number): void => {
    getDb().prepare("UPDATE trade_alerts SET status = 'fired', fired_at = datetime('now') WHERE id = ? AND status = 'active'").run(id);
  },
};
