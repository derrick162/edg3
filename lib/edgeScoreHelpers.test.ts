...
describe('prepareSparklineData', () => {
  ...
  it('returns null when history has 0 entries (crash guard)', () => {
    expect(prepareSparklineData([], 72)).toBeNull();
  });

  it('returns null when history has 1 entry', () => {
    expect(prepareSparklineData([day('2026-06-23', 60)], 65)).toBeNull();
  });

  it('returns null when history has 1 entry and todayScore is null', () => {
    expect(prepareSparklineData([day('2026-06-23', 60)], null)).toBeNull();
  });

  it('returns data when history has 2+ entries', () => {
    const history = [day('2026-06-22', 55), day('2026-06-23', 70)];
    const result = prepareSparklineData(history, null);
  ...
  it('overrides last slot with todayScore', () => {
    const history = [day('2026-06-22', 55), day('2026-06-23', 70)];
    const result = prepareSparklineData(history, 80);
    expect(result!.extended[1].score).toBe(80);
    expect(result!.delta).toBe(25); // 80 - 55
  });

  it('returns flat stroke when delta is 0', () => {
    const history = [day('2026-06-22', 60), day('2026-06-23', 60)];
    const result = prepareSparklineData(history, null);
  ...
  it('returns low stroke when trending down', () => {
    const history = [day('2026-06-22', 80), day('2026-06-23', 50)];
    const result = prepareSparklineData(history, null);
  ...
  it('does not mutate the original history array', () => {
    const history = [day('2026-06-22', 55), day('2026-06-23', 70)];
    prepareSparklineData(history, 80);
