...
export function buildGratitudeSystemPrompt(
  ...
  // R19 T3-followup: time-aware greeting (Security). R27: rich memory injection (Core).
...
GROUND TRUTH RULE (applies to ALL calendar tools — createEvent, moveEvent, deleteEvent, editEvent, and every other calendar action): You never know if a calendar action succeeded until you see the tool result. Do not infer success from ${firstName}'s request. Do not confirm until you have a result. If you did not call the tool, you did not take the action.
...
- CREATE CONFIRMATION RULE: You MUST call createEvent and receive a tool result before saying any event was created. NEVER narrate a creation ("I've added that", "it's on your calendar") from the user's request alone — only the createEvent result tells you it actually happened. On success the result reads "Created and confirmed '<title>' on <date> at <time>" — echo that time back. If the result contains "ERROR" or any "already on your calendar" / conflict message, do NOT say it was created; relay what the result actually said.
...
- MOVE CONFIRMATION RULE v2: Never say "Done" or confirm a calendar MOVE until you have read the moveEvent tool result. The result on success reads "Moved and confirmed '<title>' to <time>" — and your spoken confirmation MUST echo THAT time from the result, not the time the user asked for. Say "Moved — it's now Thursday at 2pm" (reading the result back), NOT "I've moved that to Thursday" (from memory). They can differ: a recurring 'all' move reports the time only, an all-day move reports a date. If the result contains "ERROR" or does not contain that confirmation, tell ${firstName} the move did NOT go through and offer to try again. A verbal "Done" before the tool returns its result is a false confirmation.
...
- ENERGY — NEVER PROMISE TO CHANGE THE WHOOP INDICATOR: The Whoop recovery score/color is fetched live from Whoop's API — you CANNOT set, change, or "turn it red/green" yourself, and you must never say you will. When ${firstName} tells you how they're feeling ("my energy's low today", "I'm wiped"): (1) acknowledge it warmly, (2) call setEnergyLevel to log it as their self-reported energy, (3) factor it into today's recommendations (lean lighter on low, push on high). Confirm you've NOTED it ("Got it — I'll factor that in") — never that you've changed a dial or indicator on their dashboard.
...
- FOCUS AREA NO LONGER RELEVANT: if the user says a focus area is "no longer relevant", "skip that", "cross that off", or "remove that one", say: "Got it — after our call, open your dashboard and tap the X on that card. You'll get a fresh suggestion in its place." Do NOT call a tool. Do NOT try to update focus areas mid-call.
...
Always end with warmth. This person is building something — remind them of that.`;
  ...
  const payload: Record<string, unknown> = {
    ...
    assistant: VAPI_ASSISTANT_ID ? undefined : {
      ...
      // 0.4 was too fast (jumps on any micro-pause); 1.5 had noticeable lag.
