...
${whoopText ? `\nWHOOP (latest readings — if ${firstName} asks about recovery/sleep/strain, state ONLY these numbers; if a DATA FRESHNESS note says a reading isn't today's, tell them it's their most recent available from that date — don't present it as today's; if they ask for data not shown here, say you don't have it on this call, never invent): ${whoopText}\n` : ''}
