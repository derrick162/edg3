...
- **2026-06-23** — **R24 SHIPPED (2117 green) — onboarding "Connect your tools" step.**
  ...
  - **Notes:** used the existing popup+postMessage OAuth pattern (keeps the user on the onboarding page), so the spec's `?redirect=` full-page param wasn't needed and Security-owned OAuth callback routes were left untouched. The early skippable `'calendar'` step stays (the activation preview depends on it); this new step is the hard gate and shows ✓ when already connected (no double-prompt).
- **2026-06-22** — **R23 SHIPPED (2102 green) — rich open-call memory + inbound calls (+ trust hotfix).**
  - **Hotfix (TOOL CALL DISCIPLINE):** live-call trust bug — Edge said "Done. Locked in…" but never called `createEvent`. Prompt-only block added to the assistant system prompt forbidding completion language before a successful tool response; every calendar change must go through a tool call. Committed `3c369e8`.
  - **T1 — rich open-call memory:** new `lib/callMemory.ts` `currentOpenCallMemoryText(userId)` — all fact categories (≤5 each) + open commitments (`openLoopQueries.list`, top 5) + last 2 completed call notes; degrades section-by-section. `scheduleOpenCall` passes it as `preferencesText` so open calls get briefing-quality recall, not 10 preference facts. 4 tests. Committed `61a814b`.
  - **T2 — inbound call handler:** `briefings.is_inbound` col + `userQueries.findByPhoneNumber` + `briefingQueries.markInbound` (`lib/db.ts`, additive). `lib/vapi.ts`: exported `buildOpenCallSystemPrompt` (EN + 廣東話, incl. TOOL CALL DISCIPLINE), `resolveWebhookUrl`, `CALENDAR_TOOL_IDS`. `app/api/vapi/webhook/route.ts` (Security-owned, additive): `assistant-request` branch → unknown caller polite 15s decline / registered caller full personalized open-call assistant (Cantonese-aware, ambient sound, "Hey, it's Edge — [firstName]…") + inbound briefing row; `call-started` links the Vapi call.id to the caller's pending inbound briefing. `currentPrioritiesText` moved into `lib/callMemory.ts` so the webhook avoids the cron-laden scheduler. 3 integration tests. Committed `91d8302`.
  - 2102/2102 green, tsc + next build clean. ⚠️ **Additive edits to Security-owned `lib/scheduler.ts` + `app/api/vapi/webhook/route.ts` — Vijay sync down + add the inbound rate-limit layer on top.** ⚠️ **External (Derrick):** point the Vapi number's inbound handler at `https://www.edg3.ai/api/vapi/webhook`.
...
- **2026-06-21** — **R19 SHIPPED (2077 green) — first-call intro + new-user Briefings empty state.**
  ...
  - 2077/2077 green, tsc + next build clean. Committed `e398d19`.
- **2026-06-21** — **R18 T3 SHIPPED (2067 green) — connected Google account email + Switch account.**
  - `lib/db.ts` (additive, Vijay FYI): `calendar_tokens.email` column (ALTER migration; NULL on pre-existing rows) + `CalendarToken.email` + optional `email` param on `calendarQueries.upsert` (COALESCE-preserved). `lib/calendar.ts`: `getAuthUrl(state, {selectAccount})` → `prompt:'select_account consent'`; new best-effort `fetchGoogleAccountEmail(token)` (userinfo lookup, null on failure). `calendar/callback` captures + stores the account email after exchange; `calendar/connect` honors `?select_account=1`. `/api/auth/accounts` (Security-owned — additive, Vijay FYI) now returns `calendar.email`. Dashboard shows the linked email under "Calendar connected" + a "Switch account →" link via the select-account flow. **Fixed a pre-existing bug:** the dashboard's "re-authorize →" links pointed at `/api/auth/google`, which **does not exist** (dead 404) — repointed to the working `/api/calendar/connect` flow. +1 accounts test. Committed `c0bee87`.
  - ⚠️ **Spec deviation:** the dispatch's `/api/auth/google?prompt=select_account` URL doesn't exist; used the real `/api/calendar/connect?select_account=1` flow instead (same effect — forces Google's account picker).
- **2026-06-21** — **R18 T1 SHIPPED (2066 green); ⚠️ R18 T2 BLOCKED — depends on removed email feature.**
  - **T1 — priority-blocking names an actual slot:** new pure `findNextFreeSlot(events, durationMins, preferredHour?, opts)` in `lib/calendar.ts` (next-3-weekday 9–6 gap scan, peak-hour preference, tz/DST-safe via existing time helpers, injectable today/tz for tests; 6 new tests). Wired into `lib/briefing.ts`: under-1h priority → `PRIORITY BLOCKING SLOT` line with exact day+time so Edge offers "Want me to block Tuesday at 2 PM for fundraising?" `lib/vapi.ts` PRIORITY BLOCKING guidance added. 2066/2066 green, tsc + next build clean. Committed `8dc06b8`.
  - **⚠️ T2 — surface checkReplies in briefing — BLOCKED, NOT BUILT (needs PM decision).** The spec assumes `checkOutreachReplies(userId)` + `formatRepliesForVoice` + the `outreach_tracking` table "run at briefing time anyway." **They don't exist** — all of `lib/outreach.ts`, `lib/replies.ts`, `checkOutreachReplies`, `formatRepliesForVoice`, and the `outreach_tracking` table were **deleted in R12 T7** (the email-feature removal, a CEO/PM scope-reduction decision). Gmail is now `gmail.readonly` only; `createDraft`/compose scope was removed. Building T2 would mean **resurrecting the entire removed outreach/reply-tracking feature**, reversing that decision — not done autonomously. **PM (Kevin): confirm intent** — (a) genuinely revive email outreach + reply tracking (re-add lib + table + Gmail thread-read; larger Security-gated effort), or (b) drop T2 as obsolete. Left untouched pending your decision.
- **2026-06-20** — **Pillar work (post-R17): Trust T3-1 — pattern facts render readably (2056 green).**
  - Dispatch (R12–R17) exhausted; swept all three pillars. Daily Call (Tier 4 gated until 50 users) and Memory (M4-3 gated until 30+ calls; M4-4 social mental models already LIVE — sleep-time sync + `buildPeopleModelBlock` briefing injection) had no open Core items.
  - **Trust T3-1 completed:** pattern-category facts store a JSON-serialized PatternInsight in `statement`; the "What Edge knows" memory tab rendered it raw (`{"type":"priority_drift",…}`) — a trust/transparency bug. New pure `factDisplayStatement(category, statement)` (`lib/factDisplay.ts`) renders the human `.summary` for pattern facts, falls back to raw on non-JSON/legacy; wired into all 3 fact-display sites in `app/dashboard/page.tsx`. 5 new tests. 2056/2056 green, tsc + next build clean. Committed on `core` — ready for PM merge.
- **2026-06-20** — **R17 — sharper briefing opener + post-call feedback — SHIPPED (2052 green).**
  - **T1 sharper opener:** new pure `isRoutineEvent(summary, isRecurring)` in `lib/briefing.ts` — filters breakfast/lunch/dinner/coffee/gym/workout/walk/run/commute/morning-routine (always) + recurring daily habits (stand-up/meditation/stretch/journal/vitamins/etc. only when recurring). `lib/vapi.ts` GREETING ANCHOR PHRASE updated: lead with the first NON-routine event; if everything today is routine, open with the top priority + yesterday's commitment. +6 assertions (`isRoutineEvent` describe block in `lib/briefing.test.ts`).
  - **T2 post-call feedback signal:** new `call_feedback` table + `callFeedbackQueries` (create/recent/existsForBriefing) in `lib/db.ts` (additive; `'call_feedback'` added to `USER_SCOPED_DELETE_ORDER`). New `POST /api/briefing/feedback` — auth-gated, rate-limited (new `callFeedback` key in `lib/rateLimit.ts`, 30/hr), validates rating integer 1–5, one-per-briefing idempotency (`existsForBriefing` → `{ok,duplicate:true}`). Dashboard: one-tap ★★★★★ row on the expanded Call Summary card (`ratedBriefings` session set, disabled after submit). +4 route tests.
  - 2052/2052 green, tsc + next build clean. Committed on `core` — ready for PM merge.
...
- **2026-06-20** — **R12 — 5 live-call/dashboard tickets — SHIPPED (1991 green).** From Derrick's afternoon call with Aria; each verified unbuilt first.
  ...
  - **T6 voice speed control:** `users.voice_speed` column (shared `db.ts`, additive) + `setVoiceSpeed`; `SPEED_MAP {slow:0.75, default:0.9, fast:1.1}` + `voiceSpeedPref` param on `initiateCall` (per-call `voiceConfig` overrides the VOICES base); both `scheduler.ts` call sites pass `user.voice_speed` (additive, Security FYI); POST `/api/profile/voice-speed` (enum-validated) + `/api/profile` returns it; Profile-tab "Speaking speed" 3-button row. +10 tests. **1998 green.**
  - ⚠️ Cross-lane: `lib/db.ts` (shared additive, 2 columns) + additive lines in Security's `lib/scheduler.ts` (PM-authorized, Vijay FYI). tsc + next build clean. Committed on `core` — ready for PM merge.
